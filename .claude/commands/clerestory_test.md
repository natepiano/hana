...
Run automated integration tests for bevy_clerestory using `tests/scripts/run_test.py`.

<WorkingDirectory>
All script paths below (`tests/scripts/...`, `tests/config/...`) are relative to the crate root. Before doing anything else, `cd` into `crates/bevy_clerestory` (relative to the workspace root) so every command resolves. If already inside that directory, stay put.
</WorkingDirectory>
...
**Usage**: `/clerestory_test [flags]`

**Examples**:
- `/clerestory_test` - Auto-detect OS and run all tests
- `/clerestory_test single-monitor` - Force single-monitor mode
...
If false, STOP and display the monitor layout mismatch message.
</WindowsMonitorValidation>

<SingleMonitorFiltering>
Skip tests requiring multiple monitors:
...
<WindowsZedMove>
Use the PowerShell move/detect scripts with Bevy monitor parameters. Use `dangerouslyDisableSandbox: true`.
</WindowsZedMove>
...
- Do NOT set `CLERESTORY_TEST_MODE` — the human needs keyboard input
