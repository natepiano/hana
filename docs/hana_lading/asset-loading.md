...
- Runtime transition tests now cover ordinary-first default installation,
  explicit default installation, a custom root, and a later operation without
  duplicate baseline plugins. Test builds disable Winit and logging while
  retaining the asset runtime, avoiding macOS event-loop recreation and trace
  artifacts.
- The degraded policy now enters `Ready` only when application-owned evidence
  names the exact image applied to the scene, global resolution reports one
  failure, and the generic record identifies `OptionalAssets`. Every mismatch
  remains in `Loading` behind an opaque decision panel.
- The slow trybuild case is ignored during routine suites and run explicitly for
  ...
  the final direct custom-root pass completed in about 42 seconds from that
  cache.
...
- The independent behavior review caught an unconditional degraded transition;
  the final code verifies applied-image evidence and the exact optional failure
  before entering `Ready`. Its final rereview found no remaining issue.
- The independent API review added executed baseline-transition coverage,
  corrected deferred-installation wording, and caught generated tracing output.
  Test-only plugin selection prevents recurrence, and its final rereview found
  no remaining issue.
- The external pass fixture directly constructs one custom-root builder and
  proves `app_mut()` is available afterward. The ignored-only trybuild test also
  passes both pending-state and late-root rejection cases.
