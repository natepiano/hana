...
- On the first valid membership revision, resolve `FoldEndpoint::Unfolded` to boundary zero with initial direction `Folding`, or `FoldEndpoint::Folded` to the derived terminal boundary with initial direction `Unfolding`. Later membership growth preserves numeric position/target. Membership removal clamps them only when the new terminal boundary is lower. The fixed root normally carries no `FoldMember` and consumes no stage.
...
- Implement the command observer with these transitions:
  ...
  - Idle `Play` at boundary zero selects `Folding` and targets `stages`; idle `Play` at boundary `stages` selects `Unfolding` and targets zero. At an idle interior position, `Play` uses the direction established by the latest directional step.
  - `Play` during `FoldMotion::Step` keeps the step direction and promotes the one-stage motion to terminal playback. `Play` during `FoldMotion::Play` reverses direction, targets the opposite terminal boundary, remains `FoldMotion::Play`, and preserves the continuous position.
  - Any step records its requested direction. At a matching terminal boundary, that step is a no-op. Only a same-direction command during `FoldMotion::Step` extends an existing queued target. Step reversal from queued steps or terminal playback moves immediately toward the adjacent boundary from the current position and must not snap.
- Advance `position` toward the integer target at one stage per `step_seconds`, clamp exactly at the target, and set motion to `Idle` when settled. Reaching an intermediate boundary does not change the direction established by the latest step.
...
- Exhaustively test integer/fractional steps in both directions, queued same-direction steps, direction reversal from step and play, reversal at exact boundaries, both endpoint Play directions, repeated-Play reversal from fractional positions in both directions, step promotion, idle interior continuation, grouped fractions, easing, empty/not-ready sequences, and large deltas that would cross the target.
...
- The transition matrix tests cover fractional reversal, queued steps, exact-boundary behavior, endpoint-aware Play, repeated-Play reversal, step promotion, large deltas, and grouped easing.
...
- Follow-up live use replaced remembered-direction terminal no-ops with endpoint-aware Play: terminal presses select the other endpoint, active Play reverses continuously, and Step promotion preserves its direction.
...
- Motion and direction accessors are now the sole source for Fairy Dust chip activation and directional-step presentation.
...
- Define private BEI input context/actions for one folding step, one unfolding step, and endpoint-aware terminal playback. Bind bare `Space` only when no modifier is held, `Shift+Space` for unfolding with either Shift key normalized by BEI, and bare `P` for play/reverse. Reserve conflicting bare keys through the existing shortcut registry. Do not read `ButtonInput<KeyCode>` directly.
...
- The example starts idle. One `Space` advances one panel stage and `Shift+Space` reverses one. At either terminal boundary, `P` plays to the opposite endpoint; pressing `P` again during playback reverses immediately, while an idle interior `P` continues the latest step direction.
...
- The orbit camera now runs TAA with `Msaa::Off`; the example compiled and ran with the existing stable-transparency path.
...
- Route the standard three controls through `.with_fold_controls()`. The example starts idle and one step moves one crease. At either terminal boundary, `P` plays to the opposite endpoint; pressing `P` again during playback reverses immediately, while an idle interior `P` continues the latest step direction.
...
- The example starts idle. `Space` advances lid then walls and `Shift+Space` reverses walls then lid. At either terminal boundary, `P` plays to the opposite endpoint; pressing `P` again during playback reverses immediately, while an idle interior `P` continues the latest step direction.
...
**Acceptance gate:** `cargo +nightly fmt --all -- --check`; `cargo check -p hana_valence --example box --all-features`; `cargo nextest run -p hana_valence --all-features`; launch the example and verify fold-control installation without a reservation panic, successful builder completion, idle startup, lid-only stage zero, four-wall stage one, reverse order, Play from both endpoints, repeated-Play reversal without a position jump, idle interior continuation after either step direction, fixed center exclusion, BRP port display, orbit camera, home view, teaching-panel content, no `FoldControlDiagnostic` during normal startup and standard fold inputs, and no `FoldAngleDiagnostic` during normal startup, stepping, reversal, or play.
...
- One-second stages keep the lid/wall grouping legible while reducing a complete box Play traversal from five seconds to about two.
...
- The parallel-wave join now explicitly exercises box idle state, lid-only then grouped-wall motion, reverse order, Play from both endpoints, repeated-Play reversal, idle interior continuation, fixed-center behavior, and clean fold-control/fold-angle diagnostics.
- Follow-up runtime verification on an isolated BRP port confirmed endpoint-aware terminal playback and immediate repeated-Play reversal without snapping in all three examples.
...
- Static, lint, and isolated-port runtime checks cover the updated implementation. No user decision remains.
...
For `triangles`, the root interaction pass must exercise Accordion and Wrap, one-step folding and unfolding, Play from both endpoints, repeated-Play reversal, idle interior continuation in both step directions, and a mid-fold T selection that stays queued until fully unfolded. Activation at position zero must not visibly jump or change playback position, target, direction, membership, or grouping.

For `box`, the root interaction pass must verify idle startup, a first Space that moves only the lid, a second Space that moves all four walls while carrying the lid, Shift+Space reversing walls before lid, Play from both endpoints, repeated-Play reversal, and idle interior continuation in both step directions. The center must remain fixed and normal interaction must leave fold-control and fold-angle diagnostics empty.
...
- Prior root-visible runs confirmed presentation, camera home/orbit controls, BRP, stable transparency, and step/reverse sequencing across `staggered_unfold`, `triangles`, and `box`; a follow-up isolated-port pass confirmed Play from both endpoints and repeated-Play reversal in every example.
- `staggered_unfold` now configures TAA on its orbit camera with MSAA disabled and retains stable transparency; an isolated BRP screenshot confirmed the rendered scene and expanded nine-line teaching panel fit cleanly.
...
- `box` raised only the lid in stage zero, carried it with all four walls in stage one, reversed walls before the lid, kept the center fixed, and now uses one-second stages.
