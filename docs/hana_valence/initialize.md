...
- Phase 9 box_net: tween is now a **default** feature. The `[[example]] required-features = ["tween"]` block is still needed, but its purpose is keeping `--no-default-features --examples` green, not the default build (which now has tween on).
...
- Box net is a **tree** (each tile anchors to its single parent edge, using top/bottom AND left/right edges), not a linear chain; `QuadTiling::next_edge` returns a fixed `(TOP, BOTTOM)` pair and `ArrangementMembers::predecessor` yields a single linear predecessor, so the shipped arrangement cannot express a cross-net tree — box_net must hand-wire its `AnchoredTo`+`Hinge` relations directly.
...
- Confirmed still accurate (no edit): box_net stays hand-wired (tree topology the linear-chain arrangement can't express); `ArrangementMembers` has no public constructor so examples must spawn `Member` entities; example tests can't live in `examples/*.rs` under nextest.
...
- box_net uses a manual hinge driver, not `tween` — so **no** `[[example]]` block was added to `crates/hana_valence/Cargo.toml` (spec allowed either; the block is only needed when the example imports tween-gated symbols). `--no-default-features --examples` stays green without it.
