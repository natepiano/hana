...
Example: `crates/bevy_diegetic/examples/ime.rs` (world-panel numeric editing plus
an app-owned popup).

## Module layout

`crates/bevy_diegetic/src/ime/` is a private module; the approved public API is
...
- `field_spec` is `ImeEditableFieldSpec::BuiltIn(ImeBuiltInFieldSpec)` (parsed and
  written back by `bevy_diegetic`) or `AppOwned(ImeAppOwnedFieldSpec { key })`
