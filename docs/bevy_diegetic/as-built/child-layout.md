...
`bevy_diegetic` has a type-safe child-layout API that separates row, column, and
...
`crates/bevy_diegetic/src/layout/builder.rs`. Its layout state is one of the
...
`crates/bevy_diegetic/src/layout/child_layout.rs`:
