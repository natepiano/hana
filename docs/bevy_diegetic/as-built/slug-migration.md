...
> **Archived 2026-06-07 — migration complete.** Slug is hana's sole text
> renderer; the SDF/MSDF/MTSDF glyph path is gone while the unrelated panel
> SDF subsystem (the scope boundary below) is intact. Unticked checkboxes in
> the phase checklists reflect that the doc was retired once the work landed,
> not unexecuted work. Later developments: Phase 9 (cascade unification) grew
> into its own design, [`cascade.md`](cascade.md), and was implemented; Phase
> 10's OIT/transparency-stack removal was later reversed for bevy 0.19 — see
> [`stable-transparency.md`](stable-transparency.md). The original slug design
> doc, whose Phase 8 disposition was "delete as obsolete", is now archived
> alongside as [`slug.md`](slug.md); the motivating effect analysis is live at
> [`../slug_fx.md`](../slug_fx.md).
