...
> **Reference manual** for the `bevy_diegetic` layout engine and panel-drawing API.
...
The engine is a custom flexbox-like solver in `crates/bevy_diegetic/src/layout/`. It is **not** Taffy. Its sizing rules mirror [Clay](https://github.com/nicbarker/clay) (see `engine/clay_parity.rs` for the parity tests). Everything in this manual is re-exported from `bevy_diegetic` (see `layout/mod.rs`), so `use bevy_diegetic::El;` etc.
...
```rust
use bevy_diegetic::{El, LayoutBuilder, Sizing, Px, TextStyle};
...
- Sizing rules & helpers: `crates/bevy_diegetic/src/layout/sizing.rs`
- Two-pass solver: `crates/bevy_diegetic/src/layout/engine/sizing.rs`
- Builder & `El`: `crates/bevy_diegetic/src/layout/builder.rs`
- Draw / lines / coords: `crates/bevy_diegetic/src/layout/draw.rs`, `line.rs`
- Caps: `crates/bevy_diegetic/src/callouts/caps.rs`
