...
> **Archived 2026-06-07 — implemented.** The plan landed: the `TextProps<ForLayout>` /
> `TextProps<ForStandalone>` typestate split was collapsed into the single
> `TextStyle` struct that lives in `layout/text_props.rs` today. The
> `WorldText` / `ScreenText` authoring pair this doc introduces was itself
> superseded shortly after by `DiegeticText::world()` / `DiegeticText::screen()`
> (`src/fluent.rs`) — see [`diegetic-text.md`](diegetic-text.md), the follow-on
> design. Module paths below describe the tree at execution time (May 2026);
> the panel/world text render code now lives under `render/panel_text/` and
> `render/world_text/`.
