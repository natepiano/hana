...
bevy_diegetic opting in. Any consumer that enables one of these bevy features
breaks bevy_diegetic's build:
...
1. Enable the three features on bevy_diegetic's own bevy dependency
   (`crates/bevy_diegetic/Cargo.toml`), so the gated fields exist
...
Forcing the features on means every bevy_diegetic consumer compiles bevy's
