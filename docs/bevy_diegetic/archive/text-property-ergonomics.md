...
> **Archived 2026-06-07 — implemented.** The two-surface model is the current
> API: plain fields mutate via direct setters, cascade attributes via the
> `override_*` / `inherit_*` / `resolved_*` triple. The `cascade_attr!`
> generator lives in `cascade/resolved.rs`, the wrapper triple per attribute in
> `cascade/attributes.rs`, per-attribute `CascadeDefault<A>` resources in
> `cascade/defaults.rs`. This doc predates the text-style unification
> ([`unify-text.md`](unify-text.md)): the `WorldTextStyle` / `LayoutTextStyle`
> aliases of `TextProps<C>` it references were since collapsed into the single
> `TextStyle` (`layout/text_props.rs`); the cascade-attribute design itself was
> later generalized as [`cascade.md`](cascade.md), the parent-walking
> unification.
