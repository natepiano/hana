...
- **Texture boundary.** The material table cannot vary sampled texture resources inside one draw. Texture handles and texture-present pipeline/bind-group requirements stay in the batch key. Diegetic fills are solid SDF surfaces; image leaves are not batched by this plan.
...
- Add the classification as code-adjacent documentation before implementing projection. The initial classification must be:
  ...
  - deferred until texture-backed batching is intentionally designed: `uv_transform`, parallax/depth-map scalar fields without a depth-map resource, lightmap exposure, and any feature-gated texture channel value whose texture is absent. If implementation proves one of these is needed with no resource binding or shader-def cost, move it into table values with a dedicated test.
...
- Keep the example on the shared Fairy Dust instrumentation helpers (`diegetic_stats_panel`, `diegetic_stats_tree`, `fps_stats_panel`, `gpu_meter_panel`) instead of reintroducing copied screen-panel styling.
