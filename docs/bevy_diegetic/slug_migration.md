...
| `preload_text.rs` | Rewrite to a slug preload: pre-extract + pack a known glyph set into the slug cache, measure the cost, and display it in a custom text panel (the `typography.rs` `spawn_hud_panels` pattern). **No public slug preload API exists today** (`SlugGlyphCache` has `get_or_insert_packed` but no batch warm) — it must be designed + built. Scope of that is a pending decision (see Proposed user decisions). |
