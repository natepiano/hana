...
| Functionality | What survey showed | Proposed API surface | Files that would change |
|---|---|---|---|
...
| Cube builder typing | `PrimitiveBuilder` currently represents both cubes and ground planes, while existing `face_text` and proposed spin/face-panel helpers are cube-only. Adding more cube-only methods to a shared builder increases surprise. | Prefer `CubeBuilder<S>` / `GroundPlaneBuilder<S>`, or `PrimitiveBuilder<S, CubePrimitive>` / `PrimitiveBuilder<S, GroundPlanePrimitive>`. Put `spin`, `spin_toggle`, `face_label`, `face_panel`, and `home_target` only on the cube builder. If this split is too large for the first pass, use explicit interim names: `cube_spin`, `cube_spin_toggle`, `cube_face_label`, `camera_home_target`. | fairy_dust builder internals, then lagrange/input_custom, lagrange/input_gamepad, lagrange/input_keyboard, lagrange/input_manual, lagrange/input_preset_blender_like, lagrange/input_preset_simple, lagrange/pausing, diegetic/aa_text |
| Cube spin and tumble | Repeated cube rotation exists beyond the input examples: `aa_text`, `screen_space`, `render_to_texture`, and `pausing` also spin or rotate cubes. The usages differ: input examples need a title-toggle, `pausing` needs virtual-time semantics, `render_to_texture` rotates on more than one axis, and `aa_text` tumbles around a tilted axis. | Layered API: primitive-owned cube spin on the cube builder for Fairy Dust cubes; app-level `.with_cube_spin::<Marker>(config)` for manually spawned targets. `CubeSpinConfig { chip, key, motion, time_source, initial_state }`, with `chip`/`key` optional. `CubeSpinMotion::{Yaw(f32), AxisAngle { axis, radians_per_second }, Euler { radians_per_second: Vec3 } }`. `CubeSpinTimeSource::{Virtual, Real}`. | lagrange/input_custom, lagrange/input_gamepad, lagrange/input_keyboard, lagrange/input_manual, lagrange/input_preset_blender_like, lagrange/input_preset_simple first; lagrange/pausing, lagrange/render_to_texture, diegetic/aa_text only if the call site still names the demo-specific rotation behavior clearly |
| Spin target identity | A broad `FairyDustCube` marker is useful metadata, but it is not a safe spin target in multi-cube scenes. Spin runtime must be scoped to the actual target or target group. | Add `FairyDustCube` as identity metadata only. Primitive-owned spin inserts a private/internal spin target marker or component on that primitive. App-level spin uses caller markers. Runtime state lives on target entities as `CubeSpinRuntime { mode, motion }`; shared control/chip state may live in marker-scoped resources such as `CubeSpinControl<M>`. | lagrange/input_custom, lagrange/input_gamepad, lagrange/input_keyboard, lagrange/input_manual, lagrange/input_preset_blender_like, lagrange/input_preset_simple |
...
8. Evaluate `pausing`, `render_to_texture`, `aa_text`, and `screen_space`
   individually. Use shared helpers only when they keep the demo-specific
   timing, rotation, or rendering lesson explicit.
...
- `basic`, `follow_target`, and `focus_bounds` get constants only. Their raw
  `App`/`commands.spawn` style is part of their lesson.
