...
- Deleted `crates/hana_diegetic/src/render/oit_guard.rs` (the shader-patching + activation-gate module).
...
### `oit_trace` (hana_diegetic, `render/oit_trace.rs`)
...
### `OIT_DISARM` / `OIT_CLASSIFY` (hana_diegetic, `render/oit_guard.rs`)
...
`git worktree add /Users/natemccoy/rust/bevy_hana_preguard b3948cc` — the commit before the guard (`00c7095`): raw OIT, no guard module, crates.io bwm rc.2. Build with `cargo build -p hana_diegetic --example typography --features typography_overlay` (needs unsandboxed because the rustc wrapper is sccache).
