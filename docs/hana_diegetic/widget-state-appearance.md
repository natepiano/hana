...
- **Key files** (line refs re-verified after Phase 2; files neither Phase 1 nor Phase 2 touched still carry `HEAD` = `64f8bdc0` refs):
  - `src/layout/builder.rs` (**2288** lines) — **every line ref in this bullet is Phase-2 vintage and superseded. Current values re-verified after Phase 12, 2026-07-30**; Phase 12 shifted this file **non-uniformly**, roughly `+1` above line 1170 and `-11` below 1350, so do not apply an offset to anything not listed here: `WidgetPart` **`:108`**, `PressedPart` **`:112`**, `EditorStateColors` **`:121`**, `PressedEditorStateColors` **`:134`**, `into_editor_part` **`:175`**, `EditorPartColorRole` **`:221`**, sealed `ElementRole` **`:236`**; `EditorPart::into_text` **`:645`** (`common.id = None` **`:646`**); `impl<L> El<L, LayoutOnly>` **`:1066`**; `El::editable_field` **`:1107`**, `El::button` **`:1123`**; `El<L, WidgetElement<W>>::disabled` **`:1203`**; `impl<L> El<L, WidgetElement<Slider>>` **`:1353`** (Phase 14 DELETED `El::disabled_color`; `:1381` was `focused_thumb_border_color`, which Phase 18 has since deleted); `next_auto_id` field **`:1617`** with `take_auto_id` **`:1888`** and the three `next_auto_id: 0` inits **`:1745`/`:1770`/`:1824`**; `AcceptsElement` **`:1678`** (which has **no `with` method**) and its five impls **`:1969`/`:1982`/`:1996`/`:2013`/`:2030`**; **`LayoutContentBuilder::with` is a trait method** declared **`:1700`** with two impls (**`:2048`** `LayoutBuilder`, **`:2124`** `WidgetBuilder`) — the inherent ones are `LayoutBuilder::with` **`:1844`** and `WidgetBuilder::with` **`:2072`**; `tooltip_add_text` **`:2163`** (counter param **`:2167`**, minting **`:2174`**/**`:2176`**). **RESTAMPED AFTER PHASE 17, 2026-07-31 — every ref above supersedes as follows and the older stamps are void.** The file is now **2335** lines. Phase 17's shift is **non-uniform**: `+6` above old `:1067`, `−27` between `:1067` and `:1168`, `+8` from `:1168` to `:1529`, `+11` to `:1702`, `+17` thereafter — do not apply a single offset. Current values: `impl<L> El<L, LayoutOnly>` **`:1072`**, which now holds **no state verb** — the four moved to the new `impl<L> El<L, WidgetChild>` (`hovered` **`:1146`**, `focused` **`:1154`**, `disabled` **`:1162`**, `pressed` **`:1171`**). `WidgetChild` is a new sealed role marker at **`:102`**, minted **only** by `WidgetBuilder::child(&self, El<L, LayoutOnly>) -> El<L, WidgetChild>` (**`:2120`**). `AcceptsElement` **`:1689`** now has **six** impls (**`:1986`/`:1999`/`:2013`/`:2030`/`:2047`/`:2064`**) — the sixth is `AcceptsElement<WidgetChild> for WidgetBuilder`. `LayoutContentBuilder::with` **`:1711`**, re-signed to `fn with<L, Role>(&mut self, el: El<L, Role>, children: impl FnOnce(&mut <Self as AcceptsElement<Role>>::ChildBuilder<'_>)) where Role: ElementRole, Self: AcceptsElement<Role>`; **both** impls now route through `AcceptsElement::with_child_builder`. The 16 `IntoAppearance` verbs are at **`:1146`/`:1154`/`:1162`/`:1171`/`:1191`/`:1201`/`:1211`/`:1272`/`:1282`/`:1290`/`:1298`/`:1307`/`:1317`/`:1325`/`:1333`/`:1342`**. **`focused_thumb_border_color` is GONE** — Phase 18 deleted the builder forward that stood at `:1389` (impl block `:1386-1391`) along with the property itself; `rg focused_thumb_border_color` over the crate returns nothing. Everything below `:1386` in this file shifted `−7`. `CommonEl::default_state_surfaces` is **`:725`**. (An earlier revision listed the 16 `IntoAppearance` verbs at `:1070`/`:1078`/`:1086`/`:1095`/`:1183`/`:1193`/`:1203`/`:1264`/`:1274`/`:1282`/`:1290`/`:1299`/`:1309`/`:1317`/`:1325`/`:1334`; those are pre-Phase-17 and void — use the restamped list above. The first four are no longer `LayoutOnly` at all; they are `WidgetChild`.) The historical inventory follows — `El<L, Role>`; roles `WidgetPart` (`:105`), `PressedPart` (`:109`), sealed `ElementRole` (`:112`); owner kinds `WidgetOwner` (`:127`), `Widget` (`:143`), `Pressable` (`:187`); `El::editable_field` (`:814`); the four state verbs in four blocks — `El<L, LayoutOnly>` (`:740`-`:765`, `pressed` yields `PressedPart`), `El<L, WidgetElement<W>>` (`:853`-`:873` plus `pressed` at `:888` under `Pressable`), `El<L, WidgetPart>` (`:898`-`:923`, `pressed` upgrades the role), `El<L, PressedPart>` (`:933`-`:958`, all role-preserving); `El::disabled_color` (`:1049`); `WidgetBuilder<'a, W>` (`:1240`); `AcceptsElement` (`:1295`) and its five impls (`:1582`-`:1658`); `LayoutContentBuilder` (`:1314`); `LayoutBuilder::with_root` (`:1455`), `with_widget_root` (`:1467`), `with` (`:1460`); `WidgetBuilder::with` (`:1685`); `Text::layout` (`:265`).
  - `src/layout/element.rs` — `CommonEl`/`Element`, `appearance` field (`:148`); `LayoutTree::validate_widgets` (`:782`, walk body `:793-846`), the **only** appearance-reachable walk that returns `Result<_, PanelBuildError>`, calling `validated_element_widget_owner`. **Phase 18 reverted it to `&self`** — its one and only tree write was the zero-width transparent border it synthesized for `focused_thumb_border_color`, and with that property deleted the walk is purely a read. `widgets::validate_tree` (`widgets/id.rs:259`) is `&LayoutTree` again, and so are its three call sites (`panel/builder.rs:842`/`:961`, `panel/diegetic_panel.rs:940`). The `DiegeticPanelCommands::set_tree` **trait declaration** still takes the tree by value and was not touched. Do not reintroduce a `&mut` validator: nothing in the appearance path needs to mutate the tree during validation. `computed_widget_records` (`:849`, returns `Vec<ComputedWidgetRecord>` — **no `Result`**) and its owning-record walk (`:914`) calling `record_owned_widget_element` (`:1380`) and `element_visual_capabilities` (`:1319`); `set_field_editing_content` (`:1033`); `validated_element_widget_owner` (`:1289`); `classify_element_change`'s exhaustive `Element` destructure (`:1398`); `set_element_state_appearance` (`:475`, `#[cfg(test)]`). **`PanelBuildError::WidgetContainsInteractiveDescendant` is gone** — Phase 4 removed the variant, its producer, and its two tests; nesting is now a compile error. **The four `PanelBuildError::State*` variants and `validated_element_appearance` are gone** — Phase 5 replaced them with `CommonEl::default_state_surfaces` (`layout/builder.rs`), which emits a transparent fill or border at element construction so a state property always has a record to replace.
  ...
  - `src/widgets/appearance.rs` (**1309** lines) — **every line ref in this bullet was re-verified after Phase 12, 2026-07-30.** `VisualChange<T>` (`:26`); the rustdoc property table (`:52-80`, with `:75` prose naming the `text_color`/`path_color` part-local-error pair — a stale row here is invisible to `missing_docs`); `Appearance` (`:113`, derives `PartialEq`, **six** `VisualChange` fields since Phase 7: `background`, `border_color`, `border_width`, `text_color`, `path_color`, `material`) with its impl block (`:125`); `merge_over` (`:146`). **`impl From<Color> for Appearance` was DELETED in Phase 12** along with its doc paragraph — a bare `Color` no longer converts to an `Appearance` at all. In its place: the four property wrappers `BackgroundColor` / `BorderColor` / `TextColor` / `PathColor` and the crate-owned `IntoAppearance` trait with its five impls (`:236-308`), the trait carrying `#[diagnostic::on_unimplemented]` whose label lists every wrapper (`:281-283`). **`resolve_active_layers` (`:629`, body `:629-675`) is the sole `Appearance` → `VisualSlotOverride` conversion** — six accumulators, six `VisualChange::To` arms, one six-field struct literal; a new `Appearance` property that skips this function is authored, validated, cascaded, and silently dropped. The four `Widget*Appearance` wrappers — each its own `CascadeRootResource<Self>` root resource with a **public** `new` taking `impl IntoAppearance` (`:329`/`:374`/`:419`/`:464`), a **module-private** `appearance()` accessor, and size assertions at `:354`/`:399`/`:444`/`:489`; `StateAppearance` (`:499`, **not a `Component`**), `cascades()` (`:508`); `WidgetStateCascades<'a>` (`:514`) with `any_overridden` (`:538`), `layer` (`:546`), `any` (`:568`), `resolve` (`:576`); `WidgetState` (`:680`), `LAYER_ORDER` (`:694`). **`layer`/`any`/`resolve` live on `WidgetStateCascades`, not on `StateAppearance`.** **Phase 3 DELETED both `layer_onto` methods** — per-property layering is inlined in `resolve`, which matches `VisualChange::To` per property inside the `LAYER_ORDER` loop and constructs the `VisualSlotOverride` directly.
  - `src/widgets/visual.rs` — **re-verified against the post-Phase-14 tree, 2026-07-31.** `VisualSlotOverride` (`:199`) is now **seven `Option` color properties with no generic fallback** — Phase 14 deleted the `color` field, its `with_color` helper, and `WidgetVisualOverrides::subtree_color` / `set_subtree_color` / its getter along with the subtree branch of `dispatch_visual_overrides`. Do not reintroduce any of them; earlier revisions of this bullet described all of them as live. The size assertion is now exact: `const _: () = assert!(size_of::<VisualSlotOverride>() == 184)` (`:226`) — an **equality**, so it does catch a dropped field, and any phase adding or removing one must print the real `size_of` and restamp it. **`apply` (`:229`) is the only method that names color fields**; `apply_element` (`:245`) saves `offset`, delegates to `apply`, and restores `offset`, so a new field is added to `apply` alone. Constructors: `with_tint` (`:293`), `with_fill_color` (`:299`). `WidgetVisualSlots` (`:93`) with `with_elements` / `with_part_appearances` / `elements()` / `part_appearances()`. `WidgetVisualOverrides` (`:337`) with **`set_element` (`:382`)** and **`element_overrides` (`:402`)** — the Phase 3 element-index-keyed channel, which survives Phase 14 and is now the only override channel. `resolve_part_overrides` (`:431`). `write_widget_overrides` replaces the whole component and **compares immutably first, returning without writing when the resolved value is unchanged**. `dispatch_visual_overrides` (`:614`) builds the `by_element` map from the two remaining producers. **`write_slot_override` was DELETED in Phase 3** — all writes go through `write_widget_overrides`.
  - **The three presenters (rewritten in Phase 3).** `presentation_inputs_changed` was **DELETED** in all three; each presenter now builds its own kind-filtered dirty-entity set from `Changed`/`RemovedComponents` terms it owns directly, and writes the whole component via `write_widget_overrides`. `src/widgets/button.rs` — `present_button_state` (`:139`), `Changed<WidgetVisualSlots>` dirty term (`:149`), write (`:232`). `src/widgets/editable.rs` — `present_editable_state` (`:30`), `Changed<WidgetVisualSlots>` (`:40`), write (`:121`). `src/widgets/slider.rs` — `present_slider_state` (`:1141`), `Changed<WidgetVisualSlots>` (`:1152`), subtree seeding (`:1178`), write (`:1202`). **`Slider::disabled_color` is gone** — Phase 14 deleted the field, its default, its builder, and its crate-internal setter; `rg disabled_color` over the crate returns nothing. **`Slider::focused_thumb_border_color` is also gone** — Phase 18 deleted the field, its default, both builder forms, the `has_focused_thumb_border_color` accessor, and the `:1201` slot pre-layer; `rg focused_thumb_border_color` over the crate returns nothing. **`Slider` now carries no color property at all** — it is behavior only (`range`, `default_value`, `step`, `direction`, `reset_behavior`). Every slider color, focus ring included, is authored on the element through the ordinary state builders. **Restamped after Phase 18, 2026-07-31:** `present_slider_state` is **`:1091`** (was `:1141`); `slider.rs` is **6610** lines. The thumb slot override now carries **only** the value-derived offset — the presenter writes nothing else into it, and a regression that drops that offset is caught by the explicit `idle.offset.is_some()` assertion in `focused_thumb_border_layers_with_the_value_offset`. **`ButtonPress` is an `Or<>` term in the button presenter but not the slider's** — inserting/removing it on a slider wakes exactly one presenter, the cross-kind isolation discriminator.
  ...
  - `src/panel/builder.rs` — `PanelBuildError` (`:50`). **`SliderFocusedThumbBorderColorRequiresThumbBorder` no longer exists** — `rg` over `crates/` finds it nowhere; an earlier revision of this bullet placed it at `:73` with a `Display` row at `:1055`. `StateTextColorRequiresText` (`:76`), `StatePathColorRequiresDraw` (`:79`), `BuilderData` (`:194`), the four panel-level state verbs (`:422`/`:430`/`:438`/`:446`). `src/panel/diegetic_panel.rs` — `replace_from_precompose_helper` (`:481`), `seed_panel_overrides` (`:1637`). `src/panel/lifecycle.rs` — `PanelCascadeOwnership` (`:122`), `teardown_owned_shared_state` (`:782`). `src/panel/mod.rs` — `add_cascade_ownership_observers!` defined (`:175`) and invoked (`:185`) inside the private helper Phase 9 extracted to keep `build` under clippy's 100-line limit; `HeadlessLayoutPlugin` (`:219`, `impl Plugin` `:221`), which registers the five **panel** attribute cascades explicitly because `RenderPlugin` is absent. **It does not register the four `Widget*Appearance` channels** — those are `WidgetsPlugin`-only, and a second registration is a `DuplicatePlugin` panic.
  - `src/render/fill_batch.rs` — **re-verified 2026-07-31.** `apply_sdf_visual_override` (`:1356`) reads `slot_override.fill_color` and `slot_override.border_color` **directly** — Phase 14 dropped the `.or(color)` fallbacks that used to sit at `:1364`/`:1369`, so there is no generic color read left anywhere. An earlier revision of this bullet named "the only four readers of `VisualSlotOverride::color`"; that field no longer exists and every consumer reads its own property (`render/image_batch.rs:628` reads `tint`). `src/render/panel_text/batching.rs` — cascade-resolution block (`:288`), `apply_routed_text_run_update` (`:430`). `src/render/batch_store.rs` — `BatchStore::upsert` (`:201`). `src/render/analytic_paths/batching.rs` — `TextRunBatch::rebuild` (`:314`).
  - `src/lib.rs` — crate-root `pub use widgets::*` block (**`:346-419`** after Phase 12's five new exports). Phase 1 added `Appearance` and the four `Widget{Hovered,Pressed,Focused,Disabled}Appearance` wrappers; Phase 12 added `BackgroundColor`, `BorderColor`, `TextColor`, `PathColor`, and `IntoAppearance`. **The re-export is two-hop:** a symbol must be `pub use appearance::…` in `src/widgets/mod.rs` (`:21-24` for the wrappers, alphabetical) before `src/lib.rs` can re-export it from `widgets::`. A later phase adding a public **widget** symbol extends both. A public **error** type goes with `PanelBuildError` in the `panel::` block at `:238` instead. **Phase 17 added `WidgetChild`** as a public export through the parallel `layout::` path — `src/layout/mod.rs` then `src/lib.rs` — alongside `El`, `LayoutBuilder`, `AcceptsElement`, and `LayoutContentBuilder`.
  - `examples/widgets.rs` (**1712** lines; restamped at the Phase 18 review, 2026-07-31) — **the `.disabled_color` use formerly listed at `:1165` is GONE**; Phase 14 removed it and `rg disabled_color crates/hana_diegetic/examples/widgets.rs` returns nothing. `add_slider` (**`:1205`**), `apply_state_appearance` (**`:1459`**). It imports `bevy::prelude::*` **and** `use hana_diegetic::BackgroundColor;` — an explicit single-name `use` shadows a glob import, so the deliberate `bevy::prelude` name collision resolves cleanly and needs no alias.
  - `tests/headless_widgets.rs` (131 lines) — external-client integration test; no state-appearance coverage today.
  - `tests/trybuild.rs` — the driver, and the **only** place a fixture becomes reachable. It declares **one** test, `widget_state_and_tooltip_typestate_signatures_compile`, carrying four `compile_fail` globs — the `overlay_*` glob moved into it in Phase 4 — plus all three `pass/` fixtures. **That test is `#[ignore]`** (commit `aeb8ac55`: 89-second compile, CI runs ignored tests as a separate job), so `verify.sh test hana_diegetic trybuild` reports `0 run / 0 passed / 1 skipped` and proves nothing. Run `cargo nextest run -p hana_diegetic --test trybuild --run-ignored all` and require `1 passed`. **Do not remove the `#[ignore]`.** (An earlier revision of this line claimed the test carried no `#[ignore]` — that was wrong.) **A fixture whose filename matches no existing glob is never compiled and its acceptance-gate line is vacuous** — any phase adding fixtures must list `tests/trybuild.rs` in its **Files** and add or widen a glob. `tests/trybuild/pass/` — `tooltip_typestate.rs`, `typestate_helpers.rs`, `widget_state_appearance.rs`. `tests/trybuild/fail/` — **26** fixtures as of Phase 17, and the driver now carries a **fifth** `compile_fail` glob, `tests/trybuild/fail/widget_state_verbs_*.rs`, added because Phase 17's three per-verb fixtures matched none of the existing four. **29 fixtures compile in total.** (Phase 11 added `editable_widget_editor_part_has_no_property` and `editable_widget_editor_part_rejects_pressed_colors`; Phase 12 added `widget_state_appearance_bare_color`, matching the existing `widget_state_appearance_*` glob). **The `widget_state_appearance_*.stderr` snapshots are coupled to the `IntoAppearance` impl set**: `bare_color.stderr` embeds both the `on_unimplemented` label and an auto-generated `the following other types implement trait IntoAppearance:` list, so any phase adding a wrapper must re-record it with `TRYBUILD=overwrite` and inspect the result — rustc path-shortens some type names and prefixes others, unpredictably. `editable_widget_has_no_pressed_state.{rs,stderr}` now proves an editable field's *part* rejects a pressed layer: `.rs:15` is the `with` insertion and `.stderr:1` reports `error[E0277]: the trait bound `EditableField: Pressable` is not satisfied`.
...
- **Phase 4** and **Phase 14** gained the replacement-not-accumulation constraint: a state verb replaces the whole bundle, so chained single-property calls silently drop all but the last.
...
- **Phase 7** gained a `size_of` assertion for `VisualSlotOverride` at the size it grows the type to, so the later "back to 144 bytes" claim (now split across Phases 13 and 14) is a verified delta rather than a first measurement.
...
`dispatch_visual_overrides` already builds a `HashMap<usize, VisualSlotOverride>` (`visual.rs:491`) — the one Phase 14 deletes along with `subtree_color`. The element channel **merges into that existing map**; do not introduce a second one.
...
- Phase 14 is unaffected, but its instructions were not: the element channel merges into the existing `HashMap<usize, VisualSlotOverride>` built at `visual.rs:512`, which now serves three producers — subtree seeding (`:513-523`), slot overlays (`:524-532`), and the element channel (`:533-538`). Phase 14 therefore **keeps** the map and deletes only the subtree branch, not the map itself.
...
- The element channel's per-property composition (`apply_element`) is strong enough that Phase 14's focus-border rework may reduce to a deletion — but only if Phase 10 routes the widget level through that channel rather than the root slot, which Phase 10 did not specify. Deferred there as a pending decision.
...
**Phase 14.** Every `slider.rs` and `visual.rs` line ref was wrong and is corrected. The map deletion is now definite — keep the map, delete only the subtree branch. The `rg` gate was split, because `with_color` reaches ~29 sites across seven files, three of which were missing from **Files**. The focus-border rework largely collapses into Phase 3's per-property composition, conditional on Phase 10's channel decision.

**Deferred to Phase 10 (pending decision):** which override channel carries the widget level — the per-element channel or the root slot. Under the element channel Phase 14's focus-border work becomes a deletion; under the root slot it must be written by hand. Recommended the element channel.
...
- **Phase 14** gained two Phase 4 constraints its example migration would otherwise hit blind: widget-declaring verbs are `LayoutOnly`-only (declare the widget before any state verb), and a part-authoring helper cannot be generic over the builder.
- **Phase 15** was missing a second auto-id minting path, `LayoutTree::tooltip_add_text` (`builder.rs:1790-1807`); without it tooltip content keeps positional ids.
- **Stale references corrected across the plan:** `validate_tree` does not exist (it is `LayoutTree::validate_widgets`); every `widgets/visual.rs` reference in Phases 8, 10, and 14 was ~22 lines low; `layout/builder.rs` references below ~1240 drifted 10-14 lines, and Phase 13's `disabled_color` forward pointed at the wrong impl block entirely; the Delegation Context still named the deleted `WidgetContainsInteractiveDescendant` variant and described a trybuild driver with two tests, an `#[ignore]`, 14 fixtures, and an `E0599` diagnostic that Phase 4 replaced with one test, no ignore, 18 fixtures, and an `E0277`.
...
- **Phase 14 gained a `**Pending decision:**`** — `SliderFocusedThumbBorderColorRequiresThumbBorder` survived Phase 5 and is the same error class on the same record. Deleting it is recommended. A gate line was added because Phase 14's `subtree_color|disabled_color` grep does not reach it.
...
- **Phase 14 Files** cited `dispatch_visual_overrides` line numbers contradicting its own Spec; dropped.
...
- **Phase 14** — `layout/builder.rs` refs corrected ~+200, `widgets/visual.rs` refs +2, and `render/image_batch.rs:628` added to the `color`-removal list (the phase's `rg` gate cannot see `slot_override.color`).
- **Phase 15** — `layout/builder.rs` refs corrected ~+206, and the editor content tree added as the proving case for structural ids: it is the crate's highest-churn auto-id generator and its elements cannot be named.
...
**Images are out of scope for this phase.** They get their own `.tint()` property in Phase 13; Phase 14 then deletes the generic `color` field images currently read. Until then `image_batch.rs:628` keeps reading `slot_override.color` unchanged; do not edit it here.
...
- **`src/render/analytic_paths/batching.rs` is NOT an edit site** — the file contains zero `VisualSlotOverride` references. It consumes a color already resolved and stamped by `panel_text/batching.rs`. Dropped from this phase (and re-examine before trusting Phase 14's list).
...
- A `const _: () = assert!(size_of::<VisualSlotOverride>() <= …)` records the type's new size, following the per-attribute precedent at `widgets/appearance.rs:219`. Two fields land here; the measured value is **184**. Phase 13 adds `tint` and raises the assertion to its measured value; Phase 14 removes `color` and returns it to **184**.
...
- Predicted `size_of::<VisualSlotOverride>()` was 176; measured **184**.
  `Option<Color>` is 20 bytes here, not 16. Phase 7's assertion and Phase 14's
...
**Implications for remaining phases:**
- Phase 13 adds `tint` and Phase 14 removes the generic `color`; the pair is
  net-zero, so the assertion returns to 184, not the "back to 144" the plan carried.
- Phase 14 **deletes** the subtree channel rather than re-pointing it. The
...
- **Phase 15**: **Files** gains `validate_part_state_colors`, which mints
...
- **Line-reference drift** corrected across every remaining phase and the Delegation
  ...
  `widgets/appearance.rs`, and the three presenters. Phases 14 and 17 cited
...
- **Ref drift corrected in Phases 10 and 13–16** — `widgets/appearance.rs` moved +2 to +110, and the `widgets/visual.rs` and `layout/builder.rs` refs drifted again. Each affected Work Order's correction block now says it overrides every line number elsewhere in that Work Order, including its Spec, which the Phase 6 blocks did not.
...
- **Line-reference drift**, corrected throughout: `bevy_kana/src/cascade.rs` shifted ~+50 by the `main` merge (which also gave `CascadePlugin` a second type parameter), `widgets/appearance.rs` by ~+130, `widgets/mod.rs` +4, `panel/builder.rs` +5, `panel/mod.rs` by the clippy helper extraction, plus `layout/element.rs` and `layout/builder.rs` refs in Phases 15 and 17. Phase 15's own "these refs are unaffected" note was wrong about two of them and now says so.
...
Consequence recorded in Phase 14's **Constraints from prior phases**: its focus-border rework collapses to deleting the `!(disabled && slider.disabled_color.is_some())` guard at `slider.rs:1221-1222`.
...
- **The capability mask is read, not discarded.** `resolve_part_overrides` no longer binds `&(element_index, _)` (check that function specifically — the identical binding in the `dispatch_visual_overrides` subtree-seeding loop at `visual.rs:557` is **not** this phase's to remove; Phase 14 deletes it), and a recipient whose `VisualElementCapabilities` can present no property the reduced bundle names produces **no** `VisualOverrideIndex` entry — asserted by the three enumerated dormancy rows below, sharing their fixtures.
...
- Phase 14 still owns the `dispatch_visual_overrides` subtree-seeding destructure —
...
- Phase 10's test insertions moved line numbers in four files by **non-uniform**
  amounts, including within a single file. Every line ref in Phases 11–17 was
...
- **No remaining phase is redundant, mis-scoped, or mis-sequenced.** Phases 11–17 were re-read against the implemented tree; their goals, boundaries, and ordering all hold. Phase 14's focus-border section was already pre-trimmed to a deletion by Phase 10's own embedded resolution, and Phase 15 remains correctly independent of the appearance work.
...
- **Phase 14 and Phase 15 each gained a constraint naming the vacuity trap specific to its own gate** — Phase 14's focused × disabled × dragging matrix (both branches must produce different colors, and every absence row needs a positive control), Phase 15's zero-drop assertions (each zero-drop frame also asserts the live row count; insertion stability asserts on the key, not a row count).
- **Every line reference in Phases 11–17 was re-verified against the working tree and corrected in place.** Phase 10's ~1000 new test lines shifted four files by **non-uniform** amounts, including within a single file: `layout/builder.rs` has three bands (+1, +20, +21), `widgets/visual.rs` three (+27, +47, +93), `layout/element.rs` a flat +14 from `EditorElementOrigin` on, and `widgets/slider.rs` drifted −1 to −4. Phase 14 carried the most stale refs, since it edits the two files Phase 10 changed most.
- **Four pre-existing wrong references were found and fixed in the same pass** — they predate Phase 10 and would have sent a delegate to the wrong code: `SliderFocusedThumbBorderColorRequiresThumbBorder`'s two raise sites and its producer (Phase 14), the `El<L, LayoutOnly>` `disabled` line (Phase 17), the second `with<L, Role>` method (Phase 17), and `RoleSealed` (Phase 11).
- **`AcceptsElement::with` is not a trait default method.** Phases 14 and 17 both described it that way; there are two **inherent** `with<L, Role>` methods, `LayoutBuilder::with` (`layout/builder.rs:1741`) and `WidgetBuilder::with` (`:1969`). Corrected wherever cited, since Phase 17's central difficulty is exactly this signature.
...
- **Phase 10's own Work Order was left byte-for-byte as dispatched**, including the `visual.rs:557` subtree-seeding ref that was correct at dispatch time. The Retrospective carries the current location (`:650`, under the guard at `:649`) for Phase 14 to use.
...
**Implications for remaining phases:**
- `EditorStateColors` and `PressedEditorStateColors` were inserted near the top of `layout/builder.rs` (roughly lines 126–219), so every reference below that point in Phases 12–16 has shifted downward. Phase 10 shifted one file by three different amounts in three bands — do not assume a uniform offset here either.
...
- **New Phase 12 — one named wrapper per color property.** Phase 13's `tint` would have collided with Phase 11's `From<Color> for Appearance`: a bare color always means *background*, so `.hovered(RED)` on an image part would compile, cascade, and paint a fill behind an opaque texture. The plan owner rejected documenting, refusing, or redirecting the shorthand and chose to remove the guess — `BackgroundColor`/`BorderColor`/`TextColor`/`PathColor` newtypes reaching the verbs through a crate-owned `IntoAppearance` trait with `#[diagnostic::on_unimplemented]`, and no impl for `Color`. A blanket `impl<T: ColorProperty> From<T> for Appearance` was ruled out: it is E0119 against core's reflexive `impl<T> From<T> for T`.
- **Phase 17 must name a crate-internal promotion that survives it.** `EditorStateColors::into_editor_part` (`layout/builder.rs:174`) performs the `LayoutOnly → WidgetPart` promotion Phase 17 abolishes, privately, with no enclosing widget in the type. It is correct — the `editor_*` arms are the widget — but a delegate reading only "abolish the promotion" either breaks `editor_*` or widens the public relation to accommodate it.
- **Phase 17's acceptance was entirely vacuous and now is not.** Its whole proof is trybuild fixtures, and `verify.sh test hana_diegetic trybuild` cannot run them: the runner's sole test is `#[ignore]` by deliberate repo policy, so the command reports `0 run / 0 passed / 1 skipped`. Phases 12 and 16 now require `cargo nextest run -p hana_diegetic --test trybuild --run-ignored all` with **1 passed**. Delegation Context line 46 claimed the test carried **no** `#[ignore]` — the opposite of the truth — and was corrected.
- **Phase 14's `color`-consumer list was wrong by five files.** `VisualSlotOverride::color` has exactly four readers (`fill_batch.rs:1364`/`:1369`, `image_batch.rs:628`, `visual.rs:230`). The five other files the Work Order named have no `VisualSlotOverride` color read at all — their `with_color` hits are the unrelated `TextStyle::with_color`. The list was replaced with the verified set.
- **Phase 14's "delete the guard at `slider.rs:1217-1222`" would have deleted the thumb-offset slot write.** `:1221-1223` is the `SLIDER_THUMB` write the same Work Order says must survive. Corrected to line `:1217` only, in all three places the range appeared.
- **Phase 15 named one auto-id producer; there are five.** `PanelElementId::auto` is also minted at `layout/element.rs:838`, `:1161` (inside `clone_reminting_auto_ids_into`, the per-keystroke editor path the Work Order called its highest-churn generator without ever locating), and `:1314`. Change 2 is unimplementable unless all five move together.
- **Phase 15's "Ref corrections" block declared a stale set authoritative and contradicted itself four sentences later.** The Phase-6-vintage overrides were deleted rather than corrected, and the block now carries the re-verified values.
- **`AcceptsElement::with` does not exist** — corrected in Phases 14 and 17 for the third time. The trait (`builder.rs:1696`) declares only `type ChildBuilder` and `with_child_builder`; the method taking `El<L, LayoutOnly>` is `LayoutContentBuilder::with` at `:1718`. Phase 17's central difficulty is that signature, so the wrong name pointed at the wrong sealing boundary.
...
- **Two constraints added to Phases 14 and 17:** `.hovered(Default::default())` no longer infers (write `Appearance::default()`), and editor parts get no `tint` role — `EditorPartColorRole` has exactly `Fill` and `Text`, and Phase 13 must not extend it.
- **Phase 14's size gate must be measured, not asserted from the plan.** Tightening `size_of::<VisualSlotOverride>()` from `<= 184` to `== 184` is right in intent, but the number has to come from a printed measurement, or the assertion is the same vacuous shape it exists to close.
...
### Phase 12 — One named wrapper per color property; a bare `Color` no longer authors a state · status: done

#### Work Order

**Goal:** A state verb never guesses which property a color was meant for. `.hovered(RED)` stops compiling; `.hovered(BackgroundColor(RED))` and `.hovered(TextColor(RED))` say what they set, and the compiler names the fix when an author writes the bare form.
...
Each wrapper may carry `impl From<Color> for BackgroundColor` and so on for its own ergonomics. **That does not make `.hovered(RED)` compile** — Rust never chains two user conversions, and a blanket impl that would is not writable (see below). No call site may depend on it.
...
#[diagnostic::on_unimplemented(
    ...
    label = "wrap it: `BackgroundColor({Self})`, `TextColor({Self})`, \
...
**The names are settled: `BackgroundColor`, `BorderColor`, `TextColor`, `PathColor`** — plus `Tint` when Phase 13 adds the property. Every one of them is `pub` at the crate root alongside the rest of the widget surface (`lib.rs:346-410`); none is prefixed, and none is hidden in a non-glob module.

**All four collide with `bevy::prelude`** — `BackgroundColor` and `BorderColor` are Bevy UI components, `TextColor` is Bevy's text color component, and a name-collision with `PathColor` must be checked the same way. The plan owner accepted the collision: these widgets are not authored alongside `bevy_ui`, and an author who does import both preludes resolves it with an explicit `use hana_diegetic::TextColor;`. **Do not rename, prefix, or module-scope any wrapper to avoid a Bevy collision** — that trade was made deliberately.

Nothing in this phase's code or tests may `use bevy::prelude::*` and a wrapper name in the same scope; import `bevy::prelude::Color` (and any other Bevy items) by name where a wrapper is also in scope.
...
**What worked:**
- The wrapper set and the crate-owned `IntoAppearance` landed as specified. The blind reviewer returned APPROVE with no findings.
- The two vacuity risks the Work Order named were both real and both closed: the fixture name matches the existing `widget_state_appearance_*` glob (so it is genuinely compiled), and each of the four wrapper tests uses a distinct color against a literal six-field `Appearance` (so a wrapper writing the wrong field fails).
- `#[diagnostic::on_unimplemented]` does what the phase needed. The recorded `.stderr` opens with `error[E0277]: a bare `Color` does not say which property it sets` and carries the wrap-it label — the bare-color author is told exactly what to write.

**What deviated from the plan:**
- One fix pass. The example first spelled the wrapper `hana_diegetic::BackgroundColor(...)` inline, against the file's one-import-per-line convention; corrected to `use hana_diegetic::BackgroundColor;` plus the bare call.
- The Spec line forbidding `use bevy::prelude::*` in the same scope as a wrapper name caused collateral damage: the delegate converted several files to named imports and dropped items they still needed (`PreUpdate`, `ReflectResource`, `IntoScheduleConfigs`, `SystemCondition`). It detected and repaired this itself before reporting.
- The `Appearance` doc example's second arm was rewritten from bundle form to wrapper form, so both arms now demonstrate the wrapper. Left as-is — reverting costs a doc rebuild to restore a stylistic preference.

**Surprises:**
- **The Bevy collision is a non-event in practice.** An explicit single-name `use` shadows a glob import, so `use hana_diegetic::BackgroundColor;` alongside `use bevy::prelude::*;` compiles with no ambiguity error. `examples/widgets.rs` now proves it. The accepted-collision decision carries no ergonomic cost that was not already known.
- **The `.stderr` snapshot path-shortens inconsistently.** rustc renders `Appearance` and `PathColor` bare while prefixing the other three with `hana_diegetic::`. That is rustc's own formatting, not something the fixture controls — any change to the wrapper set, their module path, or the trait's impl order churns the snapshot.
- `cascade/attributes.rs` held four more `impl Into<Appearance>` bounds (the "runtime state-appearance commands" the Work Order named without locating). The `rg` census the Spec required found them.

**Implications for remaining phases:**
- **Phase 13's `Tint` has four working siblings to copy**: tuple struct over `Color`, `Copy + Clone + Debug + PartialEq`, `From<Color>`, an `IntoAppearance` impl, a crate-root re-export, and one literal-six-field test with a color distinct from the other four. Phase 13 must also add `Tint` to the `on_unimplemented` label, or the error will name four of five wrappers — and adding it **re-records the trybuild `.stderr` snapshot**, which is a required part of that phase, not a surprise failure.
- **The bound is now `impl IntoAppearance`, not `impl Into<Appearance>`.** Phase 17 rewrites those same 16 state-verb signatures; it must carry the new bound through.
- **Line refs moved.** `layout/builder.rs` grew to 2288 lines and its 16 state verbs shifted **non-uniformly**, from `+1` at the top to `-10` at the bottom — the current lines are `1070`, `1078`, `1086`, `1095`, `1183`, `1193`, `1203`, `1264`, `1274`, `1282`, `1290`, `1299`, `1309`, `1317`, `1325`, `1334`. `widgets/appearance.rs` grew to 1309 lines; the four `Widget*Appearance::new` constructors are now `324`/`369`/`414`/`459` (was `251`/`294`/`337`/`380`), and the wrappers plus trait occupy `237`–`308`.

### Phase 12 Review

- **The old Phase 13 was split in two, and the spine resequenced.** It had grown a twelve-check acceptance gate against the plan's own ~8–10 rule, and its two halves fail differently: the additive half is proved by a compile-fail fixture and a field-level test, the removal half by the example still looking right with no material rows dropped. **Phase 13** is now additive (`tint`, `Tint`, capability arm, build error, runtime delivery) and **Phase 14** is the removal (`disabled_color`, the subtree channel, `VisualSlotOverride::color`, the focus border, the example migration). The ordering was already forced: Phase 14's image-batch migration target does not exist until Phase 13 creates it. Old Phases 14 and 15 became **15** and **16**; no `done` phase was renumbered, so every checkpoint commit message still matches its phase number. (Combined with the Phase 11 review's earlier insert, the original Phases 12/13/14 are now 13/14/15/16 — 13 split into two.) Every cross-reference in the document was swept highest-first in the same pass.
- **Two runtime-delivery gaps were named that no automated gate would have caught.** Adding a property to `Appearance` and an arm to `merge_over` leaves it authored, validated, cascaded — and dropped, because `resolve_active_layers` (`widgets/appearance.rs:629-675`) is the only function that turns a resolved `Appearance` into a `VisualSlotOverride`, and `for_capabilities` (`widgets/visual.rs:253-271`) is the only place a property is filtered per part. Both are now named explicitly in Phase 13's Spec and Files. Without them the whole gate stays green while `tint` does nothing.
- **The `image_batch.rs:628` handoff between the two new phases is written out.** Phase 13 leaves a deliberate `.or(slot_override.color)` fallback so the still-live subtree path keeps tinting images mid-plan; Phase 14 deletes it. The same split applies to the size assertion: Phase 13 raises it to the measured value, Phase 14 tightens it back to `== 184`, both from printed measurements rather than from the plan's arithmetic.
- **Phase 13 gained the trybuild gate the plan was missing.** Adding a fifth `IntoAppearance` impl invalidates `widget_state_appearance_bare_color.stderr` regardless of the `on_unimplemented` label, and `verify.sh test hana_diegetic trybuild` cannot see it — the runner's sole test is `#[ignore]` by deliberate repo policy, so that command reports `0 run / 0 passed / 1 skipped`. The gate line is `cargo nextest run -p hana_diegetic --test trybuild --run-ignored all` requiring **1 passed**.
- **`disabled_color` dimmed three properties, not one.** `widgets/visual.rs:649-660` writes `color`, `text_color`, **and** `path_color` for every element in the subtree, so Phase 14's example migration is an `Appearance::new().background(…).text_color(…)` bundle, not a single `BackgroundColor`. Written into Phase 14's Spec together with the per-field exactness gate that catches a silently dropped route.
- **`fill_batch.rs:1364`/`:1369` are fallbacks, not routes.** The Work Order said to "move" them to `fill_color`/`border_color`, which are already the primary reads; the correct edit is to drop the `.or(…)`. Corrected in Phase 14.
- **Mechanical corrections applied in place across Phases 13–17 and the Delegation Context** so every remaining Work Order stays dispatch-ready: the `Tint` wrapper expanded from two pieces to the full seven (including the two-hop `lib.rs`/`mod.rs` re-export and the rustdoc property table), Phase 17's trait-vs-inherent `with` claim corrected for the third time, its five-fixture inventory and sixteen-verb list re-verified against the post-Phase-12 tree, its vacuous `rg` gate row replaced, and every line reference in the remaining phases restamped to post-Phase-12 values.
...
**Goal:** Images get a state-authorable `tint` property — a seventh `Appearance` property with its own wrapper, capability check, build error, and runtime delivery — while the generic `color` field it eventually replaces is still in place.

**Spec:**

**`tint` is a seventh `Appearance` property, capability `IMAGE`** (the bit Phase 7 created for exactly this). `Appearance` (`widgets/appearance.rs:113`) carries six `VisualChange` fields and `merge_over` (`:146`) six arms; this adds a seventh to both, and to any merge matrix written as "for each of the six properties".

**It is a separate property from `text_color` and `path_color` because it does something different.** `image_batch.rs:136` documents "Linear RGBA tint multiplied after texture sRGB decode" — the image route *multiplies*, where the text and shape routes *replace*. Naming one property for both operations was the reason `content_color` was cancelled; do not undo that here by routing images through `text_color`.

**Copy Phase 12's wrapper recipe in full — it is seven pieces, not two,** all visible as four working siblings at `widgets/appearance.rs:236-308`:
1. `pub struct Tint(pub Color)` — a tuple struct with a doc comment on the field, deriving `Clone, Copy, Debug, PartialEq`.
2. `impl From<Color> for Tint`.
3. `impl IntoAppearance for Tint`, returning `Appearance::new().tint(self.0)`.
4. `pub use appearance::Tint;` in `src/widgets/mod.rs`, alphabetically between `PathColor` (`:21`) and `TextColor` (`:24`) — the crate root re-exports from there, not from the module directly.
5. `pub use widgets::Tint;` in `src/lib.rs`, alphabetically within the `:346-419` block.
6. **Add `Tint` to the `#[diagnostic::on_unimplemented]` label** at `widgets/appearance.rs:281-283`. Leaving it out makes the bare-color error name four of the five wrappers — the omitted one being the property this phase exists to add.
7. **Re-record `tests/trybuild/fail/widget_state_appearance_bare_color.stderr`.** That snapshot embeds both the label text *and* an auto-generated `the following other types implement trait IntoAppearance:` list of every impl, so adding a fifth impl invalidates it whether or not the label changes. Re-record with `TRYBUILD=overwrite`, then read the result — rustc path-shortens inconsistently (some types render bare, others with a `hana_diegetic::` prefix), so the new file must be inspected rather than assumed.

Its equivalence test follows the same shape as the other four: a literal seven-field `Appearance` with a color distinct from `BackgroundColor`/`BorderColor`/`TextColor`/`PathColor`'s test colors, so a wrapper writing the wrong field cannot pass.

**`tint` must reach the runtime, and this is where an additive phase silently fails.** Adding the field to `Appearance` and an arm to `merge_over` leaves the property authored, validated, cascaded — and dropped. Three further edits are required, each named here because no gate above them can see the omission:
- **`VisualSlotOverride`** (`widgets/visual.rs:201` is the generic `color` field; the struct's other color fields sit beside it) gains `tint: Option<Color>`, and `apply` (`:229`) gains its per-property overlay line. `apply_element` (`:245`) saves `offset`, delegates to `apply`, and restores `offset` — it names no color field and needs no edit.
- **`resolve_active_layers`** (`widgets/appearance.rs:629`) is the one function that turns a resolved `Appearance` into a `VisualSlotOverride`. It holds six `let mut <prop> = None;` accumulators, six `if let VisualChange::To(value) = &layer.<prop>` arms, and a six-field `VisualSlotOverride { … }` literal (`:629-675`). All three need a seventh entry. Without it every gate in this Work Order except the runtime ones still passes.
- **`VisualSlotOverride::for_capabilities`** (`widgets/visual.rs:253-271`, called at `:492` in `resolve_part_overrides`) zeroes each property on an element lacking its capability bit. It needs an `IMAGE` arm for `tint`. The deleted-in-Phase-14 generic `color` was never capability-filtered, so this is a real behavior change, not a rename.

**Two different mechanisms reject a tint the recipient cannot render, and both are required.** A tint authored **directly on one part** that has no image is an author error with no useful reading, so it is a build error (below). A tint arriving from the **widget level or a cascade** lands on every part in the widget at once — label, track, thumb, image — so erroring there would make widget-level authoring impossible; `for_capabilities` instead drops it on the parts that cannot render it and keeps it on the ones that can. The two never collide: `validate_part_state_colors` returns early on `element.widget.is_some()`, so a widget root never reaches the error path.

**The build error.** `validate_part_state_colors` (`layout/element.rs:1362`, the Phase 7 part-local check) gains a third arm — `IMAGE` capability, `PanelBuildError::StateTintRequiresImage` — alongside the `text_color` arm (`:1384`) and the `path_color` arm (`:1389`), plus `tint` in `Appearance`'s per-property walk. Copy `StateTextColorRequiresText` (`panel/builder.rs:76`) and `StatePathColorRequiresDraw` (`:79`) for the variant and its `Display` row.

**`image_batch.rs:628` reads `tint` first, keeping the generic `color` as a fallback.** It currently reads `slot_override.color.map_or(tint, linear_tint)`; this phase makes it `slot_override.tint.or(slot_override.color).map_or(tint, linear_tint)`. The `.or(…)` is deliberate and temporary — Phase 14 deletes it along with the field. Without it, the slider's `disabled_color` subtree path (still live until Phase 14) stops tinting images mid-plan.

**Size grows in this phase — measure it.** Phase 7 shipped `const _: () = assert!(size_of::<VisualSlotOverride>() <= 184)` at `widgets/visual.rs:226`. Adding a field pushes the type past 184, so the assertion fails as written. **Print the real `size_of::<VisualSlotOverride>()` and raise the bound to the measured value**; do not compute it from this Work Order. Phase 14 removes the generic `color` and tightens the assertion back to `== 184`.

**Update the `Appearance` doc property table.** `widgets/appearance.rs:52-80` is a rustdoc table of property → target record, plus prose at `:75` naming `text_color`/`path_color` as the part-local-error pair. Both become wrong when the seventh property lands, and `missing_docs` does not catch a stale row — the docs gate below only proves the doc *builds*.

**Files:**
- `src/widgets/appearance.rs` — the seventh `Appearance` field and its `merge_over` arm; the `Tint` wrapper and its `IntoAppearance` impl beside the other four (`:236-308`); the `on_unimplemented` label (`:281-283`); **`resolve_active_layers` (`:629-675`) — the accumulator, the `VisualChange::To` arm, and the struct literal, all three**; the rustdoc property table (`:52-80`) with its `:75` prose; and the equivalence test.
- `src/widgets/visual.rs` — `VisualSlotOverride` gains `tint` beside `color` (`:201`), the overlay line in `apply` (`:229`), the `IMAGE` arm in `for_capabilities` (`:253-271`, called `:492`), and the raised size bound at `:226`. `apply_element` (`:245`) is not edited.
- `src/layout/element.rs:1362` — `validate_part_state_colors` gains the `IMAGE` / `StateTintRequiresImage` arm and `tint` in the per-property walk.
- `src/panel/builder.rs:73-79` — the new `PanelBuildError` variant and its `Display` row, copying `:76` and `:79`.
- `src/render/image_batch.rs:628` — read `tint` with the `color` fallback described above.
- `src/widgets/mod.rs:21-24` and `src/lib.rs:346-419` — the two-hop `Tint` re-export, alphabetical in both.
- `crates/hana_diegetic/tests/trybuild/fail/widget_state_appearance_bare_color.stderr` — the snapshot re-record forced by the fifth `IntoAppearance` impl.

**Constraints from prior phases:**
- **Phase 7 — the `IMAGE` bit already exists and nothing uses it yet.** Phase 7 split `CONTENT` into `TEXT` / `IMAGE` / `DRAW` (`widgets/id.rs:115`, derived in `element_visual_capabilities`, `element.rs:1319`) and created `IMAGE` specifically for this phase. Do not re-derive or widen it.
- **Phase 7 — images were deliberately left reading the generic `color`.** That is why this phase adds `tint` beside it rather than replacing it: the removal is Phase 14's, and the two must not be merged back together.
- **Phase 7 — `VisualSlotOverride` measured 184 bytes** with an upper-bound assertion. This phase grows it; see the Spec.
- **Phase 12 — the wrapper set and the trait.** Every state verb takes `impl IntoAppearance`, a crate-owned conversion trait; there is deliberately **no impl for `Color`** and no `From<Color> for Appearance`. The four existing wrappers are `BackgroundColor`, `BorderColor`, `TextColor`, `PathColor` — unprefixed, exported from the crate root, and deliberately colliding with `bevy::prelude`. `Tint` is the fifth and follows them exactly; do not prefix, alias, or module-scope it to avoid a collision.
...
- **Phase 11 — editor parts get no `tint` role, and `EditorPartColorRole` must not grow one.** `EditorPartColorRole` (`layout/builder.rs:221`) has exactly two variants, `Fill` and `Text`: `editor_caret` / `editor_selection` paint fill, `editor_text` / `editor_validation` paint glyphs. An editor part can express no border, no material, and no `tint`. The seventh property stops at the `Appearance` / `VisualSlotOverride` layer.
- **Phase 7 — the part-local check and its exemptions.** `validate_part_state_colors` (`layout/element.rs:1362`) runs from `LayoutTree::validate_widgets` (`:796`) at panel build and returns early for anything that is not an owned widget part: `owning_widget.is_none() || element.widget.is_some() || element.editable.is_some()` (`:1367`) — so widget roots and editable-field elements are exempt, and the editor-generated subtree never reaches it at all (those elements are minted at runtime by `set_field_editing_content`, `:1047`, long after build validation). The `tint` arm inherits every one of those exemptions; do not widen the early return to compensate.
- **Phase 11/12 — `Default::default()` does not infer at a state verb.** `.disabled(Default::default())` fails with E0283 under `impl IntoAppearance`; write `Appearance::default()`.

**Acceptance gate:**
- `bash ~/.claude/scripts/delegate/verify.sh check hana_diegetic`
- `bash ~/.claude/scripts/delegate/verify.sh test hana_diegetic`
- `bash ~/.claude/scripts/delegate/verify.sh lint hana_diegetic`
- **`cargo nextest run -p hana_diegetic --test trybuild --run-ignored all` reports `1 passed`.** This phase adds a fifth `IntoAppearance` impl, which invalidates `tests/trybuild/fail/widget_state_appearance_bare_color.stderr` whether or not the `on_unimplemented` label is edited. Without this line the staleness escapes the phase entirely and surfaces in Phase 17 or CI: the ordinary `verify.sh test hana_diegetic trybuild` reports `0 run / 0 passed / 1 skipped`, because the runner's sole test is `#[ignore]` by deliberate repo policy — 89-second compile, CI runs ignored tests separately. **Do not remove the `#[ignore]`.** Re-record with `TRYBUILD=overwrite` and inspect the result before accepting it.
- **Docs (orchestrator-run — see Delegation Context → Docs):** this phase adds public API (`Tint`, `Appearance::tint`, a new `PanelBuildError` variant) and rewrites a rustdoc table, so both doc commands must pass before checkpoint.
- The `Tint` equivalence test asserts a literal seven-field `Appearance` whose colors are all distinct, so a wrapper writing the wrong field fails.
- **`tint` reaches the image renderer.** A state `tint` resolved onto an image recipient produces `Some(color)` in the `VisualSlotOverride` that `image_batch.rs` reads — asserted on the resolved override, not on pixels, since the headless harness produces none.
- **A `tint` authored directly on a part with no image is a `PanelBuildError::StateTintRequiresImage`**, and the same tint authored on the widget root is *not* an error: it resolves, and `for_capabilities` leaves `tint: None` on the non-image parts while keeping it on the image part. Both rows are required — the error row alone would pass with `for_capabilities` unedited.
- `size_of::<VisualSlotOverride>()` is printed and the assertion at `widgets/visual.rs:226` carries that measured number.

**Ref corrections (verified at the Phase 12 review, 2026-07-30, against the post-Phase-12 tree) — these override every line number elsewhere in this Work Order. `widgets/appearance.rs` is 1309 lines and `layout/builder.rs` 2288; both drifted non-uniformly from earlier revisions, so do not apply an offset to any number this block does not list.**
- `widgets/appearance.rs`: `Appearance` **`:113`**, `merge_over` **`:146`**, the four wrappers and `IntoAppearance` **`:236-308`**, the `on_unimplemented` label **`:281-283`**, `resolve_active_layers` **`:629`** (its body **`:629-675`**), the four `Widget*Appearance::new` **`:329`/`:374`/`:419`/`:464`**, the four size assertions **`:354`/`:399`/`:444`/`:489`**, `StateAppearance` **`:499`**, `cascades()` **`:508`**, `WidgetStateCascades` **`:514`**, `any_overridden` **`:538`**, `layer` **`:546`**, `any` **`:568`**, `resolve` **`:576`**, `WidgetState` **`:680`**, `LAYER_ORDER` **`:694`**, the rustdoc property table **`:52-80`**
- `widgets/visual.rs` (unmoved by Phase 12): `VisualSlotOverride::color` **`:201`**, size assert **`:226`**, `apply` **`:229`**, `apply_element` **`:245`**, `for_capabilities` **`:253-271`** (called at **`:492`**), `with_color` **`:278`**
...
### Phase 14 — Remove `Slider::disabled_color` and the subtree channel · status: done

#### Work Order

**Goal:** The blunt subtree-recolor path is gone, its consumers read the per-property fields, and the slider's focus border composes correctly against a cascaded disabled bundle.
...
With its only production producer gone, delete `VisualSlotOverride::color` (`visual.rs:201`), its overlay logic, and the `with_color` test helper (`:278`); move the text consumer to `text_color` and the draw-primitive consumer to `path_color`, both added in Phase 7; the image consumer already reads Phase 13's `tint`. **The overlay logic lives in `apply` alone** (`:229`) — `apply_element` (`:245`) saves `offset`, delegates to `apply`, and restores `offset`, so it names no color field and needs no edit.
...
Convert `examples/widgets.rs`'s slider (`add_slider` `:1204`, `.disabled_color` use `:1164`) to author its parts explicitly. **Write it in Phase 12's wrapper form** — a single-property state is a named wrapper (`.hovered(BackgroundColor(SLIDER_TRACK_HOVERED))`), and only a state that sets two or more properties uses `Appearance::new()`. A bare `Color` does not compile.

**`disabled_color` dimmed three properties, not one — the migration is not a single `BackgroundColor`.** `widgets/visual.rs:649-660` writes `color: Some(color)`, `text_color: Some(color)`, **and** `path_color: Some(color)` for every element in the subtree. So `focused_thumb_border_color` maps to `BorderColor` on the thumb's focused state (one property, one wrapper), but the widget's disabled state maps to an `Appearance::new().background(…).text_color(…)` bundle — plus `.path_color(…)` on any part carrying a `PanelDraw`. Authoring it as `.disabled(BackgroundColor(SLIDER_DISABLED_COLOR))` silently loses the label's text dimming, which is exactly what the live-smoke row and the per-field exactness row below exist to catch.
...
- **The `color`-consumer list is exactly four read sites in three files** — re-verified 2026-07-30: `src/render/fill_batch.rs:1364` and `:1369`, `src/render/image_batch.rs:628`, `src/widgets/visual.rs:230`. Earlier revisions of this list also named `panel_text/batching.rs`, `panel_text/reify.rs`, `panel_shapes/batching.rs`, `analytic_paths/batching.rs`, and `widgets/tooltip.rs`; **none of those reads `VisualSlotOverride::color`** — their `with_color` hits are the unrelated `TextStyle::with_color`. Do not migrate them. `VisualSlotOverride::with_color` has 15 call sites in three files: `fill_batch.rs` ×7, `visual.rs` ×7, `widgets/reify.rs` ×1. **All three render-side reads are fallbacks, not routes:** `fill_batch.rs:1364` is `slot_override.fill_color.or(slot_override.color)`, `:1369` is `slot_override.border_color.or(slot_override.color)`, and after Phase 13 `image_batch.rs:628` is `slot_override.tint.or(slot_override.color)`. The correct edit in all three is to **drop the `.or(…)` and keep the primary read** — do not "move" them to `fill_color`/`border_color`/`tint`, which are already there.
- `src/layout/builder.rs:1399` — remove the `El<L, WidgetElement<Slider>>::disabled_color` forward.
- `src/panel/builder.rs:73` and `:1061`, `src/layout/element.rs:827` and `:858`, `src/widgets/slider.rs:5481` — delete `SliderFocusedThumbBorderColorRequiresThumbBorder`, its `Display` row and test-table row, both raise sites, its producer, and the test asserting it (resolved decision above).
- `src/render/image_batch.rs:628` — drop the `.or(slot_override.color)` fallback Phase 13 added, leaving `slot_override.tint.map_or(tint, linear_tint)`.
- `examples/widgets.rs:1165`, `:1205` — author slider parts explicitly. (Re-verified 2026-07-30: the file is 1703 lines and these are `+1` from the numbers earlier revisions carried; `apply_state_appearance` is `:1459`.)

**Constraints from prior phases:**
- **Phase 7:** `text_color` and `path_color` exist on `Appearance` and `VisualSlotOverride`, consumed by the text route and the `PanelDraw` route respectively. Phase 7 asserted `VisualSlotOverride` at **184** bytes.
- **Phase 13 — `tint` and `Tint` are already in the tree.** The seventh `Appearance` property, its wrapper and crate-root re-export, its `on_unimplemented` label entry, its `resolve_active_layers` accumulator/arm/literal, its `for_capabilities` `IMAGE` arm, its `validate_part_state_colors` arm and `PanelBuildError::StateTintRequiresImage`, and the re-recorded trybuild snapshot all shipped there. This phase adds none of them. What it inherits as work is the `.or(slot_override.color)` fallback Phase 13 left in `image_batch.rs:628` and the raised size assertion at `widgets/visual.rs:226`, both of which come back down when `color` goes.
...
- **Phase 12 — a bare `Color` no longer authors a state, and this phase's example migration must be written in wrapper form.** Every state verb takes `impl IntoAppearance`; there is no `From<Color> for Appearance`. A single-property state is written `.hovered(BackgroundColor(SLIDER_TRACK_HOVERED))` — the wrapper names the property — and a state setting two or more properties still builds one `Appearance`. The wrapper set is `BackgroundColor`, `BorderColor`, `TextColor`, `PathColor`, and Phase 13's `Tint` — names settled, unprefixed, exported from the crate root, and deliberately colliding with `bevy::prelude`. The slider's `focused_thumb_border_color` maps to `BorderColor` on the thumb's focused state; `disabled_color` does **not** map to a single wrapper, because it dimmed background, text, and path together — it becomes an `Appearance::new().background(…).text_color(…)` bundle.
- **Phase 12 — `EditorStateColors` was deliberately left alone.** Its setters still take a bare `Color`, because the method name already says which property. Do not wrap them.
...
**Resolved (user decision, 2026-07-31): `SliderFocusedThumbBorderColorRequiresThumbBorder` is deleted in this phase.** Remove the variant (`panel/builder.rs:73`), its `Display` row (`:1061`) and its row in the `Display` test table, both raise sites (`layout/element.rs:827` and `:858`), and its producer (`widgets/slider.rs:5481`) along with the test that asserts it. A thumb with no authored border then gets the same synthesized transparent `Border::all(Px(0.0), Color::NONE)` every other element gets from `CommonEl::default_state_surfaces`, and `focused_thumb_border_color` replaces it like any other state border color. **Acceptance:** a slider authoring `focused_thumb_border_color` on a thumb with **no** `El::border` builds without error and its focused thumb presents that border color — the row must assert the presented color, not merely that the build succeeded, or it passes on a synthesized transparent border. The rationale below is retained as the record of why.

**The premise changed in Phase 7.** Phase 5 abolished the "a state property needs its ordinary declaration" error class, and this decision was originally written as "except here". Phase 7 reinstated the class: `StateTextColorRequiresText` (`panel/builder.rs:76`) and `StatePathColorRequiresDraw` (`:79`) are exactly that shape, raised from `validate_part_state_colors` (`layout/element.rs:1384` / `:1389`). "It is the only one left" is therefore no longer the argument.
...
The option not taken was keeping it as a deliberate exception, on the argument that a focused thumb border color with no thumb border is more likely a typo than a state-only role, and that a widened transparent border on a slider thumb is invisible in a way an author would not expect. It was rejected because under the synthesizability test this is the one rejection in the codebase whose missing record the builder can already mint — a special case, not a category — and because the recovery it demands, declaring the thumb border at its resting color, is already what `Appearance::border_width`'s doc tells authors to do.
...
- **The size assertion at `visual.rs:226` is tightened from Phase 13's raised upper bound to `== <measured>`, expected 184.** Phase 7 shipped an upper bound, which cannot detect a shrink — exactly the failure this gate exists to catch, since this phase removes a field. **Print the real `size_of::<VisualSlotOverride>()` before tightening** and use the measured value; a number written from this Work Order's expectation rather than from a measurement is the same vacuous shape the assertion is meant to close. If the measured number is not 184, a field was dropped or added that this Work Order does not describe.
- **The image route reads `tint` alone.** `rg -n 'slot_override\.color' crates/hana_diegetic/src/render` returns nothing, and a state `tint` on an image recipient still multiplies its texture color and restores on state exit — the Phase 13 delivery test still passes with the fallback gone.
...
**Ref corrections (re-verified at the Phase 12 review, 2026-07-30, against the post-Phase-12 tree) — these override every line number elsewhere in this Work Order, including its Spec. `layout/builder.rs` is now 2288 lines and `widgets/appearance.rs` 1309; both drifted non-uniformly, so do not apply an offset to any number this block does not list.**
- `layout/builder.rs` — `El<L, WidgetElement<Slider>>::disabled_color` → **`:1388`**, its `impl` block at **`:1353`**
- `layout/builder.rs` — `impl<L> El<L, LayoutOnly>` → **`:1066`** (`editable_field` **`:1107`**, `button` **`:1123`**)
- `layout/builder.rs` — `El<L, WidgetElement<W>>::disabled`, the verb **not** to delete → **`:1203`**
- `layout/builder.rs` — `LayoutContentBuilder::with`, which takes `El<L, LayoutOnly>`, is a **trait method** declared at **`:1707`** with two impls (`LayoutBuilder` **`:2055`**, `WidgetBuilder` **`:2131`**), not an inherent method. The inherent `with` methods are `LayoutBuilder::with` **`:1851`** and `WidgetBuilder::with` **`:2079`**. The `AcceptsElement<Role>` trait is at **`:1685`** and **has no `with` method** — it declares `type ChildBuilder` and `with_child_builder` only.
- `layout/builder.rs` — `EditorStateColors` **`:121`**, `PressedEditorStateColors` **`:134`**, `into_editor_part` **`:175`**, `EditorPartColorRole` **`:221`**
- `widgets/visual.rs` (unmoved by Phase 12): `VisualSlotOverride::color` **`:201`**, size assert **`:226`**, `apply` **`:229`**, `apply_element` **`:245`**, `for_capabilities` **`:253-271`** (called at **`:492`**), `with_color` **`:278`**, `subtree_color` field **`:335`**, `set_subtree_color` **`:342`**, getter **`:347`**, the `by_element` map **`:648`**, the subtree branch **`:649-661`**
...
### Retrospective (Phase 14)

**What worked:** Every production instruction landed as written — the `.or(…)` fallbacks were dropped rather than "migrated", the subtree branch went while the `by_element` map and its other two producers stayed, and `size_of::<VisualSlotOverride>()` measured **184** exactly as the Work Order predicted, so the assertion tightened from `<= 200` to `== 184` off a real measurement. Both reviews found **zero production defects**.

**What deviated from the plan:** Nothing in the production change. The first delegate pass left three acceptance-gate rows unproved and had to be corrected in a fix pass — see the Review block below.

**Surprises:**
- **A narrower replacement fixture silently drops coverage.** `complete_slider_tree` carried label + track + thumb (four elements); its replacement `focused_thumb_state_tree` kept only the thumb. Everything still compiled and passed, but the label `text_color` route — the exact route `disabled_color` used to carry, and the one the migration was most likely to lose — was then asserted nowhere. **Later phases: when a replacement fixture is narrower than the one it replaces, the deleted breadth needs its own test, not an assumption that something else covers it.**
- **The thumb-border synthesis could not reuse `CommonEl::default_state_surfaces`.** That function (`layout/builder.rs:719`) decides from *the element's own* appearance cascade, and `focused_thumb_border_color` is authored on the `Slider` spec, not on the thumb element. Only the widget-ownership walk in `LayoutTree::validate_widgets` knows the slider→thumb association, so the synthesis lives there. That made `validate_widgets` / `widgets::validate_tree` / `DiegeticPanelCommands::set_tree` take `&mut`, which is why `widgets/id.rs` and `panel/diegetic_panel.rs` appear in a diff whose Files list never named them.
- **A validator that mutates invites a whole-collection clone.** The first pass reached for `self.elements.get(index).cloned()` on **every** element of **every** panel build so one rare thumb could be mutated mid-walk. `Element` clones a `WidgetSpec`, a `PanelDraw` shape vector, a `Tooltip`, five `Box`ed editor parts, and several `Cascade`s. Fixed by collecting the indices during an immutable walk and synthesizing after the loop.
- **Focus visibility in the slider test harness is order-sensitive.** Focus must be requested after the pointer grab, and keyboard traversal is what reveals focused presentation. Three fixture iterations went into getting the drag-release frame right.

**Implications for remaining phases:**
- **`VisualSlotOverride` is now seven `Option` properties with no generic fallback**, and all four former `color` read sites read their own property. Phase 15 and 16 can treat per-property routing as settled.
- **`validate_widgets` now takes `&mut self`** and synthesizes as well as validates. Any later phase adding validation there inherits a function that is no longer read-only.
- **Open question, not a Phase 14 defect:** the synthesized thumb border is `Border::all(Px(0.0), Color::NONE)`. `focused_thumb_border_color` on a borderless thumb now reaches the renderer as a color on a **zero-thickness** ring, which draws nothing — the author gets silence where they used to get a build error. The same `Px(0.0)` synthesis has been the Phase 5 shape all along, so this is pre-existing rather than introduced here, but whether an explicitly authored focus border color should synthesize a **visible** width is undecided.

### Phase 14 Review

- **Both reviews found no production defects.** The blind reviewer and the orchestrator diff pass independently confirmed the removals are complete (`disabled_color`, `subtree_color`, `VisualSlotOverride::color`, `with_color`, `SliderFocusedThumbBorderColorRequiresThumbBorder` gone from source, examples, and tests), no `#[allow(…)]` was added, and no surviving assertion was weakened.
- **Three stated acceptance-gate rows were unproved after the first pass and were fixed in fix pass 1** — all three were missing tests, not wrong behavior: the per-field exactness row (root/track/thumb/label asserted one `assert_eq!` per field, including label `text_color`), the row requiring the matrix to include *the frame that queues `SliderDrag` removal* (the test removed the component directly from the world instead of through the production release path), and the material-churn row's label/track/thumb shape (the added test used a single-slot widget). Final suite: **1185 passed, 0 failed, 2 skipped**, up from 1182.
- **Orchestrator gates run outside the delegate:** the ignored trybuild gate passed **1 passed, 0 skipped** (the delegate could not confirm it and said so); `RUSTDOCFLAGS="-D warnings" cargo doc` clean; doctests 10 passed, 0 failed; `verify.sh fmt` clean with no reformatting.
- **Corrected 2026-07-31 — an architect pass after this block was written found otherwise.** The original bullet claimed the remaining phases needed no re-scoping. That held only for *substantive overlap*; it was wrong about refs, the Delegation Context, and the test-count floor, and it missed a design hole. What the pass changed, all recorded in the Work Orders themselves:
  - **The old Phase 15 was split.** Its own recorded probe (frame 6) shows the capacity work alone makes a wholesale re-key free, and cold start / panel respawn are unreachable by key stability at all — so the capacity half satisfies all three zero-drop rows by itself. **Phase 15** is now the capacity half, **Phase 16** the identity half. Old Phase 16 became **17**; 19 cross-references were swept highest-first. No `done` phase renumbered. **User decision, approved.**
  - **Child dividers have no element identity.** `emit_child_dividers` stamps every separator with the *parent's* index and `panel_geometry.rs:413` substitutes a shared sentinel, so `command_index` is their only distinguisher today. Phase 16's key now carries a parent-plus-ordinal discriminator, with a gate row asserting distinct keys per gap. **User decision, approved.**
  - **The editor cannot be proven by a tree-path id.** `inline_editor_content_tree` varies its element *count* per keystroke, so a sibling-index path re-keys the caret exactly as build order did, and `EditorPart::into_text` clears authored ids. Phase 16 now specifies **role-derived** ids for editor-generated elements, qualified by the owning editor so two editors in one panel do not collide and focus never participates in identity. **User decision, approved.**
  - **The zero-width focus border is resolved by deletion.** `Slider::focused_thumb_border_color` is the only authoring path with no way to supply a border width, so it can never render; Phase 14 already migrated the example off it. New **Phase 18** deletes the property and returns `validate_widgets` to `&self`. Do **not** reinstate the build error. **User decision, approved.**
  - **Record identity was split from draw order into a new Phase 19.** `SdfRecordKey` carries the same positional `command_index`, but unlike the material key it is also read as a **sort order** (`sort_records`, `fill_batch.rs:644-655` breaks same-depth ties on it), so it cannot simply adopt Phase 16's identity. Phase 19 separates the two: the key takes the identity shape, the command index becomes an explicit draw-order field on the record. Records have no capacity limit and drop nothing, so this is churn and a hidden coupling, not a visible defect. Numbered 19 rather than inserted at 17 — the dependency is "after 16", which any later slot satisfies, and renumbering twice in one pass risks a missed reference. **User decision, approved.**
  - **Mechanical, applied without a gate:** every Phase 15/16/17 `layout/builder.rs` ref below `:1388` restamped −7 and re-verified against the tree; the `layout/element.rs` auto-id producer refs restamped (`:832` / `:1143` / `:1158` / `:1310` / `:1355` / `:1371`); Phase 16 gained an explicit `SdfRecordKey` scope boundary and a row-budget re-measurement note (synthesized transparent borders are live material rows and postdate the probe); Phase 17 gained the two example call sites Phase 14 added. Phase 15's out-of-scope image-key constraint (recorded at the Phase 13 review) still holds and was carried onto both halves.

### Phase 15 — Material buffer growth: no dropped material rows · status: done

> **Split at review, 2026-07-31.** This was one phase covering four changes. The
> capacity work (changes 3 and 4) and the identity work (changes 1 and 2) fix two
> unrelated causes, and the recorded probe's frame 6 shows the capacity work alone
> makes a wholesale re-key free — so it satisfies all three zero-drop rows by itself
> and also covers the two cases identity work cannot touch (cold start, panel
> respawn). Phase 15 is now the capacity half; **Phase 16** is the identity half.

#### Work Order

**Goal:** No frame ever renders a surface whose material row was dropped under re-key, cold start, or panel respawn. **Corrected at implementation:** the drop path is not unreachable in general — `clear_frame_material_table` sizes the frame's append window from the *previous* frame's capacity, so a single-frame content increase beyond it still drops. 100% headroom raises that threshold from a ~1.13x jump to roughly 4x. `warn_material_table_drops` therefore remains a strong signal but is not proof of a defect on its own.

**Spec:**

**The defect.** `SdfMaterialSourceKey` (`render/fill_batch.rs:167-175`) identifies a material row by `command_index: CommandIndex` — a slot number in the panel's `LayoutResult::commands` vector (`render/draw_order.rs:30-33`). It is the key into `source_slots: HashMap<MaterialSourceKey, MaterialSlotId>` (`render/material_table.rs:548`), which maps a source to its row in the GPU material table. Because the field is positional, inserting or removing one command shifts every later index, so every later surface presents a key the map has never seen and claims a **fresh** row while its old row is still pinned by `retire_unseen_sources` (`:711-728`, `MATERIAL_SLOT_RETIREMENT_FRAMES = 2` at `:118`). That frame needs 2× the live row count.
...
**Two changes, both required.** Identity stability is Phase 16 and is deliberately not relied on here: these two changes must close every drop path on their own, with the positional key still in place.

1. **Remove the growth lag.** A cold start with more surfaces than the initial 128-row capacity drops on frame 1 regardless of key stability, and a panel respawn changes `panel: Entity` and re-keys wholesale no matter what Phase 16 does. Either promote a grown buffer in the same frame it is staged, or stop clamping the CPU append window to the active capacity and truncate the *upload* instead at `encode_material_table_upload` (`:1390-1399`) / `padded_rows` (`:509-517`). Both remove the drop; the second costs one frame of stale rows for the overflow.

2. **Widen the growth headroom.** `CAPACITY_HEADROOM_DIVISOR = 8` (`:114`, applied at `:826`) reserves 12.5%; a wholesale re-key needs ~100%. Raise it so a re-key of the current live set fits without growing.

   **Re-measure the row budget before choosing the divisor.** The 100-source figure in the probe above predates Phases 5 and 14. `CommonEl::default_state_surfaces` (`layout/builder.rs:719`) and Phase 14's thumb-border synthesis both insert `Border::all(Px(0.0), Color::NONE)`, `positioning.rs:193` emits a real `RenderCommandKind::Border` for it with **no zero-width skip**, and `fill_batch.rs:1062-1080` treats any present border as authored — so every synthesized transparent border is a live material row. State-appearance authoring has raised the live row count since the probe was recorded; take a fresh count from the `widgets` example before picking the divisor.
...
**Files:**
- `src/render/material_table.rs` — `:114` `CAPACITY_HEADROOM_DIVISOR`; `:509-517` `padded_rows`; `:617-632` the drop guard; `:785` `clear_with_active_capacity` and `:1213-1222` its caller; `:1205-1211` / `:1265-1268` the stage-then-promote lag; `:1390-1399` upload encoding; `:1417-1425` `warn_material_table_drops`; `:2502` `probe_rekey_drop`, the test the zero-drop gate promotes.
- `src/render/fill_batch.rs:988`-≈`:1030` — `append_sdf_record_materials`, the paired Fill/Border append and its `rollback_assignments_after` path (`:1009`, `:1025`). Read-only unless the amplification note above proves real; the **key struct at `:167-175` is Phase 16's, not this phase's** — do not touch `command_index` here.

**Constraints from prior phases:**
- **Independent of phases 1-14.** This is a render-layer capacity defect; no widget appearance behavior depends on it and it gates none of the earlier phases.
- **Identity stability is Phase 16 — do not anticipate it.** Leave `SdfMaterialSourceKey.command_index` (`render/fill_batch.rs:171`) exactly as it is. This phase must close every drop path with the positional key still in place; a fix that only works once keys are stable belongs to Phase 16 and would make both phases' gates unprovable in isolation.
- **Phase 10 — every zero-drop assertion needs a positive control.** `assert_eq!(dropped_record_count(), 0)` passes identically when the frame did no work at all. Delegation Context → *A test must be able to fail* is the rule; here it means each zero-drop frame also asserts the expected live row count, so a test that silently stopped exercising the re-key cannot read as a pass.
...
- **The image batch key is deliberately out of scope — examined at the Phase 13 review, 2026-07-30, and left alone.** `ImageRecordKey` embeds `command_index` (`render/image_batch.rs:76`), the same positional shape Phase 16 removes from `SdfMaterialSourceKey`, so a future review will find it again. It does not share the failure: material rows live in a table with a hard `row_limit` that **rejects** new identities and counts the drops (`render/material_table.rs:548-558`), while image batches grow capacity and rebuild a larger mesh instead (`image_batch.rs:778-811`) — nothing is rejected and no image disappears. Do not widen either phase to `image_batch.rs`.
...
**What worked:** The headroom change alone closes every drop path the phase gated on. `CAPACITY_HEADROOM_DIVISOR = 1` makes capacity `next_pow2(2 x required)`; a live measurement of the `widgets` example (131 live material rows, taken through BRP) sizes the buffer at 512, so a wholesale re-key needing 262 fits without growing. Tests 1185 -> 1188.

**What deviated from the plan:** The Work Order offered two designs for change 1 and required one. The delegate chose same-frame promotion; the dual review found it defective and it was **reverted in full**. The phase therefore shipped change 2 only, and the pending-buffer lifecycle is byte-identical to its pre-phase state.

**Surprises:**
- **Change 1 was unnecessary and unsafe, and the plan did not see it.** Unsafe: `MaterialTableUpdatedToCurrent` has no ordering against `AssetEventSystems` (`material_table.rs:1122`, cf. the sibling `BatchAssetWrites.before(AssetEventSystems)` at `:1119`), so a `ShaderBuffer` allocated there may not be prepared this frame; `upload_extracted_material_table` (`:1429`) then silently uploads nothing while every batch material has already been rebound to the new handle. The deleted module doc line stated exactly this invariant. Unnecessary: `clear_frame_material_table` (`:1198`) clamps the append window at the top of the frame, so rows are already rejected before `ensure_...` runs — same-frame promotion cannot un-drop them and only benefits the next frame, which the staged path already did. **No headless test can catch either half.**
- **Cold start never needed change 1.** The no-handle branch of `ensure_material_table_buffer_handle` already activates immediately, and with no active handle `clear_frame_material_table` uses the device row limit rather than a capacity clamp.
- **The drop-count amplification is real, and now has a test.** At the row limit, a surface whose `Border` fails after `Fill` succeeded rolls back and frees its slot for immediate reuse, so `dropped_records` counts once per *surface*, not once per missing row (`fill_batch.rs`, `border_limit_drop_counts_each_rolled_back_surface`).

**Implications for remaining phases:**
- **The Goal's warn claim overreaches and is corrected below.** A single-frame content increase beyond the current capacity still drops, because the append window is sized from the previous frame. 100% headroom raises the trigger from a ~1.13x jump to roughly 4x, but does not remove it.
- **Phase 16 inherits a capacity margin, not an unconditional guarantee.** Its constraint "do not weaken Phase 15's headroom" stands and is now the load-bearing half of this phase.
- **`probe_rekey_drop` no longer registers `activate_prepared_material_table_buffer` in its local schedule** (production registration at `:1137` is untouched). It passes because capacity reaches 512 on frame 1 so promotion is never needed. If a later phase makes growth necessary mid-probe, restore the system to keep the probe faithful to production.

### Phase 15 Review

- **Phase 16 re-scoped to key on `ElementIndex` (approved).** The `PanelElementId` key in the old Work Order could not be built: the batching layer requires `MemberKey: Copy + Eq + Hash` (`batch_store.rs:68`) and `PanelElementId` is `Clone`-only because `Named` holds a `String`, and the render path has no route to an element id at all — `gather_surfaces` takes no layout tree and `RenderCommand` carries only `element_idx`. `ElementSurface` already stores `index: ElementIndex` beside `command_index`, and `ElementIndex` already derives `Copy + Eq + Hash`, so the key is a field swap with no plumbing. This deleted the whole of Phase 16's change 2 — interning, the `RenderCommand` handle field, five auto-id producers, structural auto ids, editor role ids, two-editor qualification — and the `Named(String)` hashing risk with it. The divider ordinal survives; element-insertion churn (the editor's per-keystroke rebuild) is now covered by Phase 15's capacity margin rather than by identity.
- **Phase 19 gains a gated sort hoist (approved).** Its stated benefit — removing rebatching work on insertion — was not delivered by the key change: `record_upload` is a batch-level `Dirty` flag (`fill_batch.rs:595`) and an insertion lengthens the record vec, so the buffer re-uploads regardless. The removable work is `sort_records` running *inside* `upsert_record` (`:673`), once per changed record. Hoisted to one call after the append pass, gated on a dirty flag so an idle panel sorts zero times. The Spec now also records why dodging the equality check instead would be wrong (stale order values colliding with a fresh one) and that the `transform` carry-forward at `:665` is not a precedent.
- **Phase 16 gained two acceptance rows:** structural-id injectivity is now free by construction (`gather_surfaces` keys surfaces by `ElementIndex` in a `HashMap`), so that row narrowed to dividers; and a new row requires guarding the `MaterialTableUpdatedToCurrent` allocation hazard in both the schedule-ordering test (`material_table.rs:1943-1968`) and the set's doc comment, since Phase 15 shipped that defect, reverted it, and no headless test catches it.
- **Corrected drifted refs across the plan.** `material_table.rs` +3 (`source_slots` `:548`, `retire_unseen_sources` `:711-728`, `MATERIAL_SLOT_RETIREMENT_FRAMES` `:118`); `default_state_surfaces` `:719` not `:740`; the border emission `positioning.rs:193` not `:189`; Phase 19's `SdfRecordKey` sites `:3454` / helpers `:4821`,`:4831`; Phase 17's and the Delegation Context's `layout/builder.rs` refs above `:1388` were 7 high (the Phase 14 review restamped only below that line).
- **Delegation Context corrected:** it still described `El::disabled_color` and the four `Slider::disabled_color` sites, all deleted in Phase 14; test-count floor raised 1185 → 1188.
- **Constraints propagated into Phase 16:** Phase 15 bought a capacity margin, not an unconditional guarantee (the append window is sized from the previous frame, so a jump past ~4x still drops); `probe_rekey_drop` no longer registers `activate_prepared_material_table_buffer` in its local schedule; do not allocate a `ShaderBuffer` inside `MaterialTableUpdatedToCurrent`.
- **Phase 18 corrected:** it named two of the three tests referencing `focused_thumb_border_color` (`focused_thumb_border_layers_with_the_value_offset` at `slider.rs:5286` was missing), and its test-count waiver now requires restating the new floor so it cannot silently lower the bar for Phases 16/17/19.
- **`validate_widgets` shared-surface warning between Phases 16 and 18 was added, then removed** — the Phase 16 re-scope means it no longer touches that function at all.

### Phase 16 — Stable material identity: keys survive command insertion · status: done

> **Split out of the old Phase 15 at review, 2026-07-31, and re-scoped at the Phase 15
> review the same day.** Phase 15 closed the visible drop path with capacity work alone.
> This phase removes the *cause* of the churn: a surface's material identity is its slot
> in the command stream, so inserting any command renumbers every surface after it.
> Nothing here is required to make Phase 15's zero-drop gate hold.

#### Work Order

**Goal:** A surface's material identity is the element it belongs to, not its position in the command stream. Inserting or removing a render command — which is exactly what authoring a state color does — leaves every other surface's `SdfMaterialSourceKey` unchanged.

**Spec:**

**The defect.** `SdfMaterialSourceKey` (`render/fill_batch.rs:167-175`) identifies a material row by `command_index: CommandIndex` — a slot number in the panel's `LayoutResult::commands` vector (`render/draw_order.rs:30-33`). It is the key into `source_slots: HashMap<MaterialSourceKey, MaterialSlotId>` (`render/material_table.rs:548`). Because the field is positional, inserting or removing one command shifts every later index, so every later surface presents a key the map has never seen and claims a **fresh** row while its old row is still pinned by `retire_unseen_sources` (`:711-728`, `MATERIAL_SLOT_RETIREMENT_FRAMES = 2` at `:118`). Phase 15 made that affordable; this phase makes it stop happening.

**State-appearance authoring is itself the churn generator.** `CommonEl::default_state_surfaces` (`layout/builder.rs:719`) and Phase 14's thumb-border synthesis insert `Border::all(Px(0.0), Color::NONE)`, and `positioning.rs:193` emits a real `RenderCommandKind::Border` for it with **no zero-width skip**. So adding `.focused(BorderColor(...))` to any element inserts a command and shifts every later `command_index` — the feature this whole plan builds is a generator of the churn this phase removes.

**The change: key on the element, not the command slot (decided at review, 2026-07-31).**

```rust
pub(crate) struct SdfMaterialSourceKey {
    pub panel:   Entity,
    pub element: ElementIndex,   // was: command_index: CommandIndex
    pub role:    SdfMaterialRole,
}
```

`ElementIndex` (`render/draw_order.rs:37`) already derives `Clone, Copy, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd`, so it satisfies the `Batch::MemberKey: Copy + Eq + Hash` bound (`render/batch_store.rs:68`) with no new machinery. **The value is already in the render path** — `gather_surfaces` (`render/panel_geometry.rs:390`) builds `surfaces: HashMap<ElementIndex, ElementSurface>` keyed by `ElementIndex::from(cmd.element_idx)` and stores it as `ElementSurface.index` (`:132`), directly beside the `command_index` field whose own doc comment already calls it *"the reconcile reuse key"*. `fill_batch.rs:1298` reads `stored_surface.element_index()` today. This is a field swap, not a plumbing change.

**Why it works.** Adding a state border to element *n* inserts a Border **command**, which shifts every later `cmd_index` — but not a single `cmd.element_idx`, because no element was added. Every later surface keeps its key. **Injectivity is free**: `gather_surfaces` already collapses all of an element's commands into one `ElementSurface` via `surfaces.entry(element_index)`, with fill and border as two `Option` slots on that one struct, so `(panel, element, role)` is unique by construction for everything except dividers.

**Dividers need a discriminator — they are the one collision (decided at review, 2026-07-31).** `emit_child_dividers` (`layout/engine/positioning.rs:807`) emits one `RectangleSource::ChildDivider` command per adjacent child pair (`:831`) and stamps every one with the **parent's** index (`:851` / `:870`); `panel_geometry.rs:411-413` then discards that and substitutes the shared sentinel `ElementIndex::CHILD_DIVIDER` (`render/draw_order.rs:125`, `= usize::MAX`), leaving `command_index` as the only distinguisher (`:410`, `:417`, `:422`). Keying on `element` alone would collapse every divider in a panel onto one row. Keep the parent's real index instead of the sentinel and add the **pair ordinal** — the `windows(2)` position, already stable and already generated in order, and already implicit in the push order of the `dividers: Vec<ElementSurface>`. The parent's index alone is not sufficient: it would also collide with the parent's own fill surface, so the ordinal (or an enum-shaped identity field distinguishing "element fill" from "divider *n* of element") is required.

**Scope boundary — what this deliberately does not fix.** Inserting or removing an *element* still renumbers later elements, so the editor's per-keystroke rebuild of `inline_editor_content_tree` (`ime/editor.rs:1146`) still re-keys its subtree. That is covered by Phase 15's capacity margin rather than by identity: a wholesale positional re-key is free at the measured 131 live rows against 512 capacity. Identity stability and capacity headroom pay for the same thing; this phase spends identity on the constant case (command insertion, which this plan generates on every state-color authoring) and headroom on the rare one (element insertion, which means real content change). **Do not reintroduce `PanelElementId` into the render key** — an earlier revision of this Work Order did, which forced interning it to a `Copy` handle (its `Named` variant holds a `String`), plumbing that handle through `RenderCommand`, and migrating all five `PanelElementId::auto` minting sites to structural ids. All of that is cut; none of it is reachable from a key built out of `ElementSurface.index`.

**Files:**
- `src/render/fill_batch.rs:167-175` — `SdfMaterialSourceKey`: `command_index` (`:171`) → `element: ElementIndex` plus the divider discriminator. The three production construction sites move with the field: `:388` and `:393` (fill and border sources for one surface) and `:1114` (per-role append).
- `src/render/panel_geometry.rs:400-426` — the divider branch: `:411` tests `RectangleSource::ChildDivider`, `:413` substitutes the sentinel, `:417`/`:422` carry `command_index`. The divider discriminator is constructed here; this is also where the parent index must stop being discarded.
- `src/layout/engine/positioning.rs:807-875` — `emit_child_dividers`: the `windows(2)` loop at `:831` supplies the pair ordinal; both `push_command` sites (`:851`, `:870`) pass `parent_idx` as the element index.
- `src/render/material_table.rs` — **a `SdfMaterialSourceKey` construction site Phase 15 added.** Its test module builds the key directly in `write_panel_respawn_rows` and `probe_write_rows`, and imports `SdfMaterialRole` via `#[cfg(test)] use super::fill_batch::SdfMaterialRole;` (`:45-46`). Changing the key's shape breaks this file's tests; migrate them with the rest. The schedule-guard gate row below also lands here (`:1039-1040`, `:1943-1968`).
- `src/render/draw_order.rs:30-33` — `CommandIndex` loses this consumer; delete it only if no other consumer remains (`SdfRecordKey` still holds one until Phase 19). `:36-37` — `ElementIndex`, the replacement. `:125` — `ElementIndex::CHILD_DIVIDER`, the sentinel the divider branch stops using.

**Constraints from prior phases:**
- **Phase 15 bought a capacity margin, not an unconditional guarantee.** Its headroom change (`CAPACITY_HEADROOM_DIVISOR = 1`, capacity `next_pow2(2 x required)`) makes a wholesale positional re-key free at the measured 131 live rows. It does **not** close the drop path in general: `clear_frame_material_table` sizes the frame's append window from the *previous* frame's capacity, so a single-frame content jump past it still drops rows — headroom raises that threshold from roughly a 1.13x jump to roughly 4x. Do not re-open capacity work here, and do not weaken the headroom to "compensate" for stabler keys. If a zero-drop test from Phase 15 fails during this phase, that is a regression in this phase, not a signal to retune the divisor. **This phase's scope boundary leans on that margin by design** — see the Spec.
- **`probe_rekey_drop` does not register `activate_prepared_material_table_buffer` in its local `Update` schedule** (the production registration at `material_table.rs:1137` is untouched). It passes because capacity reaches 512 on frame 1, so the pending buffer never needs promoting. If work in this phase makes a capacity growth happen mid-probe, restore that system to the probe's chain first — otherwise the probe silently stops mirroring production.
- **Do not allocate a `ShaderBuffer` inside `MaterialTableUpdatedToCurrent`.** That set has no ordering against Bevy's `AssetEventSystems` (`configure_sets`, `material_table.rs:1125-1132`; contrast the sibling `BatchAssetWrites.before(AssetEventSystems)` at `:1128`), so a buffer created there may not be prepared into `RenderAssets<GpuShaderBuffer>` that frame while `rebind_registered_material_table_buffers` has already repointed every batch material at the new handle — `upload_extracted_material_table` then silently uploads nothing and surfaces render stale or blank for a frame. Phase 15 shipped exactly this defect and reverted it. **No headless test catches it**, which is why this phase adds the guard named in the gate.
- **`SdfRecordKey` is out of scope here and is Phase 19's subject.** `SdfRecordKey` (`render/fill_batch.rs:212-217`) carries the same positional `command_index`, so insertion still churns retained records after this phase. It is deferred rather than left alone: it needs the identity shape this phase establishes, and it also reads `command_index` as a **sort order** (`sort_records`, `:644-655`), which this phase does not touch. Do not migrate it here — the two keys must not change shape in the same dispatch, or a draw-order regression and an identity regression become indistinguishable.
- **The image batch key is likewise out of scope** — see Phase 15's constraint for the reasoning; it applies unchanged here.
- **Phase 10 — assert on the key, never on a row count.** A stable row count with shifted keys passes by accident. Every stability row in this phase reads the resolved `SdfMaterialSourceKey` itself.
- **Phase 14:** `VisualSlotOverride::color` is gone and all consumers read `text_color` / `path_color` / `tint`. Do not reintroduce a `color` read while touching the batching files.
- **Sequencing:** after Phase 15, before or after Phases 17 and 18 — the re-scoped phase no longer touches `validate_widgets` or any auto-id producer, so it shares no surface with either.
...
- `bash ~/.claude/scripts/delegate/verify.sh lint hana_diegetic`
- `bash ~/.claude/scripts/delegate/verify.sh example hana_diegetic widgets`
- **Authoring a state color does not shift a sibling's key.** Adding `.focused(BorderColor(...))` to element *n* — which inserts a real border command via `default_state_surfaces` — leaves element *n+1*'s material key unchanged. This is the churn path the rest of this plan creates and the reason this phase exists.
- **Command insertion in general does not shift keys.** Inserting any render command above an existing surface leaves that surface's resolved material key unchanged. Assert on the key, not on the row count.
- **Dividers get distinct material rows.** A panel whose row/column carries `.child_divider(...)` over three or more children resolves a **distinct** `SdfMaterialSourceKey` for each separator, none of them equal to the parent's own fill key, and all of them unchanged when a command is inserted elsewhere in the panel. Assert the count of distinct divider keys equals the number of gaps — a row count alone passes when every divider shares one slot.
- **The `MaterialTableUpdatedToCurrent` allocation hazard is guarded.** Extend the existing schedule-ordering test (`render/material_table.rs:1943-1968`, which today asserts only the `BatchAssetWrites.before(AssetEventSystems)` edge) so the constraint is stated in code, and state it in the `MaterialTableUpdatedToCurrent` doc comment (`:1039-1040`), which is currently silent about it. Phase 15 shipped a buffer allocation in that set, found it defective, and reverted it; nothing in the tree stops the next implementer from doing it again.
- **Phase 15's zero-drop rows still pass** with the new keys in place.

### Retrospective

**What worked:** The element index was already in the render path — `gather_surfaces` builds `HashMap<ElementIndex, ElementSurface>` and `ElementSurface` already stored `index` beside `command_index`. The re-key was a field swap; no plumbing, no interning, no `RenderCommand` change.

**What deviated from the plan:** The divider discriminator landed as a flat `divider_ordinal: Option<usize>` on both `ElementSurface` and `SdfMaterialSourceKey` rather than an enum-shaped identity. `RectangleSource::ChildDivider` became a struct variant carrying `ordinal`, so the ordinal is produced in `emit_child_dividers` and read in `gather_surfaces` without a second lookup.

**Surprises:**
- **Keeping the parent's `ElementIndex` changed rendering, not just identity.** `resolve_sdf_surface` derives a surface's material and corner radius from `surface.index`, and `fill_batch.rs` keys widget visual-slot overrides on the same value. Under the old `usize::MAX` sentinel both lookups always missed; with the parent's real index a divider began inheriting the parent's `.material(...)`, corner radius, and state override. Blind review caught it. The fix adds `visual_source_element()`, which returns `None` for a divider, so identity keeps the parent index while every element-derived visual property does not.
- **The forbidden-allocation constraint was already violated by pre-existing code.** `ensure_material_table_buffer_handle` runs inside `MaterialTableUpdatedToCurrent` and calls `Assets<ShaderBuffer>::add` in both branches. The first guard checked one system by name and passed straight over it. The real invariant is narrower than "never allocate here": a growth buffer must be parked in `pending` and promoted on a later frame, which the code already does. The guard is now a behavioral test — drive the growth path, assert `handle` is unchanged and `pending` becomes `Some` — which holds regardless of how a future system is named.
- **Key-equality assertions could not see the material table.** `SdfMaterialSlotAppended` drops the key that `append_material_slot` returns, and the record layer reconstructs its own. Both stability tests now assert the surviving surface keeps the same `MaterialSlotId` across the insertion.
- `frame_material_row_count` → `frame_material_live_row_count` in `record_and_material_rows_follow_live_sdf_surfaces` is a correct re-measurement, not a weakened assertion: retained sparse rows keep the table's physical extent above the live source count. Reviewed and confirmed; do not relitigate.

**Implications for remaining phases:**
- **Phase 19 inherits the identity shape but not the sort.** `SdfRecordKey` still carries `command_index` and still reads it as draw order. The `visual_source_element()` split is the precedent for separating "who am I" from "what do I look like"; Phase 19 separates "who am I" from "when do I draw".
- Test floor is now **1193**.

### Phase 16 Review

- **Phase 19 — its record key needs a third field Phase 16 did not need.** `push_resolved_sdf_surfaces` emits two `ResolvedSdfSurface`s from one `ElementSurface` when a clipped border splits from its fill, and both copy the same `index` and `divider_ordinal` through `..self`. `SdfMaterialSourceKey` survives that because it carries `role`; a record key keyed on `(panel, element, divider_ordinal)` alone would collide and lose the border half. Spec change 1 now adds a `Whole | Fill | Border` field, the "reuse Phase 16's shape exactly" constraint is amended to sanction it, and a gate row asserts two records survive.
- **Phase 19 — Phase 16's two stability tests are re-pointed, not deleted.** `fill_batch.rs:5283`/`:5317` assert on `record_key.command_index`, the field Phase 19 removes. They are Phase 16's proof that position shifts while identity does not; the constraint now requires them to move onto the new draw-order field.
- **Phase 19 — every file/line ref restamped** against the post-Phase-16 tree, plus the public `SdfBatchStore::upsert_record` wrapper, the two sorting test helpers, and ten bare `record_key.command_index` assertions that the earlier ref list missed.
- **Phase 19 — the sort gate corrected three ways.** `remove_record` does not sort today, so the dirty flag is upsert-only; the hoist point is in `route_sdf_batch_records` after `retain_records`, not inside `SdfBatch`; and the gate reads "one sort per dirty batch per frame" because a flat per-frame count is unmeasurable on a multi-batch panel. The `#[cfg(test)]` counter goes on `SdfBatch` since `sort_records` is private.
- **Phase 19 — `CommandIndex` survives**, with its five remaining consumers named, and `ImageRecordKey` is declared explicitly out of scope with the reason.
- **Phase 18 — the "shared surface with Phase 16" constraint was false and is deleted.** Phase 16 never touched `layout/element.rs`; Phase 18 owns `validate_widgets` alone and its line refs are undisturbed.
- **Phase 18 — a fourth test references the property.** `valid_slider_retains_authored_configuration` proves authored configuration survives validation generally; strip the property from it rather than deleting the test. Also corrected: `set_tree` takes the tree by value, so reverting `&mut` is dropping one `mut` binding at `panel/builder.rs:940`.
- **Phase 17 — the affected trybuild fixture list was wrong.** Four are affected, not five: `tooltip_part` carries an `El<_, LayoutOnly>` vs `El<Row, WidgetPart>` mismatch from `Tooltip::new` rather than the recorded `E0277`, and `forged_scope` dies earlier on an `E0308` in a `with_child_builder` argument and never reaches the verb, making it conditional.
- **Rejected — the review's public-API finding.** `RectangleSource` becoming a struct variant was flagged as a breaking public change. It is not public: `lib.rs:55` declares `mod layout` private, and the only outward path is `pub mod bench_support`, which is `#[cfg(feature = "bench_support")]`, `#[doc(hidden)]`, and documented as internal benchmark exports. No approval was required and the variant stands as shipped. The as-built doc at `as-built/child-layout.md` was stale and is updated.
- **Rejected — `frame_material_row_count` → `frame_material_live_row_count`.** Traced and confirmed a correct re-measurement; recorded in the retrospective so future passes do not relitigate it.

### Phase 17 — The state verbs require a widget in the type · status: done
...
Today all four state verbs live on `impl<L> El<L, LayoutOnly>` (`layout/builder.rs:1066`, `disabled` at `:1086`), and calling one **promotes** the element to `WidgetPart`. That promotion is Phase 4's shipped builder-acceptance relation, and it means this compiles:
...
**The hard part is the child-building closure, not the verbs.** Get the dispatch right before touching anything: `LayoutContentBuilder::with` is a **trait method**, declared `fn with<L>(&mut self, el: El<L, LayoutOnly>, …)` at `layout/builder.rs:1700` with **two impls** — `LayoutBuilder` at `:2048` and `WidgetBuilder` at `:2124`. Separately, `LayoutBuilder::with` (`:1844`) and `WidgetBuilder::with` (`:2072`) are **inherent** methods on the same two types. That is four items, not three, and the `LayoutOnly` bound this phase must move is baked into the *trait* signature at `:1700` — an earlier revision of this Work Order called all three "inherent methods" and named `:1718`/`:1862`/`:2090`, which would leave a delegate looking for two impls it never found. `AcceptsElement` (`:1678`) has **no** `with` method at all — it declares `type ChildBuilder` and `with_child_builder` only, so a plan line reading `AcceptsElement::with` names a method that does not exist and points at the wrong sealing boundary. The closure accepts any role, and every child inside it starts life as `El::new()`, which is `LayoutOnly`. Moving the verbs without changing that closure makes widget parts unauthorable. The closure must hand out elements that already carry their owner, which is what makes this phase touch every widget-child call site and revisit Phase 4's acceptance relation.
...
**A crate-internal promotion survives this phase — name it or a delegate breaks it.** `EditorStateColors::into_editor_part` (`layout/builder.rs:175`, private, added in Phase 11) builds `El::new()`, writes `common.appearance`, and calls `into_role()` — exactly the `LayoutOnly → WidgetPart` promotion this phase abolishes, performed inside the crate with no enclosing widget in the type. It is correct: the four `editor_*` arms are the enclosing widget, and the caller cannot reach it. Whatever mechanism replaces the public promotion must leave a crate-internal construction path for it. Do not "fix" it by widening the public acceptance relation, and do not delete it — the `editor_*` surface Phase 11 shipped depends on it.

**Files:**
- `src/layout/builder.rs:1066` — the `impl<L> El<L, LayoutOnly>` block holding all four verbs (`hovered` `:1070`, `focused` `:1078`, `disabled` `:1086`, `pressed` `:1095`); the **trait** method `LayoutContentBuilder::with` (`:1700`) with its two impls (`:2048`, `:2124`) and the two inherent `with` methods `LayoutBuilder::with` (`:1844`) / `WidgetBuilder::with` (`:2072`); the role markers `WidgetPart` (`:108`) / `PressedPart` (`:112`) behind sealed `ElementRole` (`:236`); `AcceptsElement<Role>` (`:1678`) and its five impls (`:1969`, `:1982`, `:1996`, `:2013`, `:2030`). **Re-verified after Phase 15, 2026-07-31 — the Phase 14 review restamped −7 only below `:1388`, leaving this phase's above-`:1388` refs 7 high.**
- `src/layout/builder.rs:175` — `EditorStateColors::into_editor_part`, the crate-internal promotion described above.
- `crates/hana_diegetic/tests/trybuild/` — the compile-fail fixtures for this phase, plus every existing fixture whose recorded error moves. **Four `fail/` fixtures are affected, with a fifth conditional — re-verified at the Phase 16 review, 2026-07-31.** Phase 11's `editable_widget_editor_part_has_no_property.rs` and `editable_widget_editor_part_rejects_pressed_colors.rs` author `El::new().editable_field(…)`. Two more author a state verb on an element with no enclosing widget and are today rejected *downstream* rather than at the verb: `widget_state_appearance_outside_widget.rs`, whose snapshot records `E0277: LayoutBuilder cannot accept WidgetPart`, and `widget_state_appearance_tooltip_part.rs`, whose snapshot records **not** an `E0277` but an `El<_, LayoutOnly>` vs `El<Row, WidgetPart>` mismatch arising from `Tooltip::new` — a different error whose text will still move when the verb does. `widget_state_appearance_forged_scope.rs` is the **conditional** fifth: it dies earlier, on an `E0308` in a `with_child_builder` argument, and never reaches the state verb — its snapshot only needs re-recording if this phase also perturbs that argument's type. `widget_state_appearance_nested_widget.rs` is unrelated to the verbs and is correctly not listed. Phase 12's `widget_state_appearance_bare_color.stderr` is affected only if the verb's signature or bound wording changes.
- Every widget-child authoring site — enumerate with `rg -n '\.with\(' crates/hana_diegetic/src` and the example.

**Constraints from prior phases:**
- **Phase 4:** the acceptance relation and the five `AcceptsElement` impls are what this phase revises. `tests/trybuild/pass/typestate_helpers.rs::add_widget_content` is the worked example of a part-authoring helper; it takes `&mut WidgetBuilder<'_, W>` for a concrete owner because `LayoutContentBuilder::with` (the trait method at `layout/builder.rs:1700`) could not be made generic over the builder. That constraint is what this phase has to solve properly.
...
- **Phase 12:** every state verb takes `impl IntoAppearance` — a crate-owned conversion trait, **not** `impl Into<Appearance>` and not a bare `Appearance`. The convertible set is `Appearance` itself plus one newtype per color property (`BackgroundColor`, `BorderColor`, `TextColor`, `PathColor`, and Phase 13's `Tint`); there is deliberately **no impl for `Color`**. The `El<L, LayoutOnly>` verbs this phase removes (`layout/builder.rs:1070`/`:1078`/`:1086`/`:1095`) carry that bound when this phase starts, and whatever replaces them must keep it, or every wrapper call site in `examples/widgets.rs` breaks. **Sixteen** verbs carry the bound in `layout/builder.rs` — re-verified against the post-Phase-12 tree, 2026-07-30: `:1070`, `:1078`, `:1086`, `:1095`, `:1183`, `:1193`, `:1203`, `:1264`, `:1274`, `:1282`, `:1290`, `:1299`, `:1309`, `:1317`, `:1325`, `:1334`.
...
- **Phase 11:** the four `editor_*` arms no longer take `El<L2, WidgetPart>` — they take `EditorStateColors` (`layout/builder.rs:121`), a colors-only value with no property names, and reject the distinct pressed-role sibling `PressedEditorStateColors` (`:134`) by type. That surface is settled; this phase revises the acceptance relation around it and does not widen it back to a full `Appearance`.
- **Phase 14 added six state-verb call sites in the example — verified 2026-07-31.** `examples/widgets.rs` now authors `.hovered(BackgroundColor(SLIDER_TRACK_HOVERED))` on a bare `El::new()` track and `.slider_thumb().focused(BorderColor(SLIDER_THUMB_FOCUSED_BORDER))` on the thumb, both two closures deep inside the slider's `WidgetBuilder`. Six sites added crate-wide, none removed. `slider_thumb` lives in the role-generic `impl<L, Role> El<L, Role>` (`layout/builder.rs:888`), so it composes with whatever role replaces `LayoutOnly` — but the thumb chain is the shape this phase's `pass/` fixture should mirror, since it is the deepest real authoring site in the crate.
...
- **A trybuild compile-fail fixture per verb** — `El::new().hovered(…)`, `.focused(…)`, `.pressed(…)`, `.disabled(…)` on an element with no enclosing widget each fail to compile, with the error naming the missing widget rather than a generic trait mismatch. **`disabled` already has one**: `tests/trybuild/fail/widget_state_appearance_outside_widget.rs` authors exactly that shape today and is rejected downstream — **extend** it (and re-record its `.stderr` to show the error moving onto the verb) rather than adding a second `disabled` fixture.
...
- **The four state verbs are gone from the `impl<L> El<L, LayoutOnly>` block.** Do **not** gate this with `rg -n 'impl<L> El<L, LayoutOnly>'` — that prints only the `impl` header and says nothing about the block body, so it passes identically before and after the phase (Delegation Context → *A test must be able to fail*). Prove it by scanning the block's line range instead, e.g. `awk 'NR>=1066 && NR<=1168' crates/hana_diegetic/src/layout/builder.rs | rg 'pub fn (hovered|focused|disabled|pressed)'` returning nothing — or rely on the compile-fail fixtures, which are the real proof.
- `EditorStateColors::into_editor_part` (`layout/builder.rs:175`) still compiles and the `editor_*` tests still pass — the crate-internal promotion named in the Spec survives.
...
**What worked:**
- The typestate landed as specified: a new `WidgetChild` role (`layout/builder.rs:102`) minted only by `WidgetBuilder::child`, with the four verbs moved to `impl<L> El<L, WidgetChild>`. No `PanelBuildError`, no widened public acceptance relation, and `EditorStateColors::into_editor_part` survived untouched.
- The Work Order's warning that "the hard part is the child-building closure, not the verbs" was correct and load-bearing.

**What deviated from the plan:**
- The Work Order's line refs were stamped "7 high" after Phase 15. They were **not** high — every ref (`:1066`, `:1070`, `:1078`, `:1086`, `:1095`, `:1700`, `:2048`, `:2124`, `:1844`, `:2072`, `:175`, `:1678`) was exact. That stale warning cost verification time and should not be repeated on Phases 18–19.
- The affected-fixture count was four affected + one conditional; the conditional (`widget_state_appearance_forged_scope.rs`) did need editing, and `widget_state_appearance_tooltip_part.rs` needed re-recording as predicted.

**Surprises:**
- **The trait impl was the real hole, and every gate missed it.** `LayoutContentBuilder::with` had two impls whose closure parameter was `impl FnOnce(&mut Self)`, calling `with_element` directly instead of `AcceptsElement::with_child_builder`. Method-call syntax on a concrete `LayoutBuilder` resolves to the *inherent* `with`, so the whole first implementation passed check, tests, lint, the example, and trybuild while the trait path still handed back a plain `LayoutBuilder`. It took a fix pass to correct both impls to `impl FnOnce(&mut <Self as AcceptsElement<Role>>::ChildBuilder<'_>)`.
- **A green gate set is not evidence when the broken path is unreachable from the gates.** The fix pass had to add coverage that reaches the trait path — a compile-fail fixture using a generic helper bounded by `LayoutContentBuilder`, plus a generic `add_widget_content` in `pass/typestate_helpers.rs` — and prove both flip when the fix is reverted.
- `AcceptsElement::with_child_builder` carries `where Self: 'a`. Any helper generic over `LayoutContentBuilder` that opens a widget therefore needs `Builder: 'static` plus an HRTB `ChildBuilder<'a> = WidgetBuilder<'a, W>` equality bound, so such a helper cannot take a `WidgetBuilder<'_, W>` receiver. This predates Phase 17 — the phase only made it reachable.

**Implications for remaining phases:**
- Authoring shape changed crate-wide: a widget child is now `builder.child(El::new())…`, not a bare `El::new()`. Phases 18–19 that touch widget-child authoring sites must use it.
- Compiled trybuild fixture count is now 29 (was 28); the `fail/` count is 26.

### Phase 17 Review

- **Delegation Context restamped for `layout/builder.rs`.** The file is 2335 lines and Phase 17's shift is non-uniform in five bands, so no single offset applies. `WidgetChild`, `WidgetBuilder::child`, the relocated verbs, the sixth `AcceptsElement` impl, and the re-signed `LayoutContentBuilder::with` are all recorded with current refs; the superseded lists are marked void rather than deleted.
- **Delegation Context corrections.** `SliderFocusedThumbBorderColorRequiresThumbBorder` was listed at `panel/builder.rs:73` but exists nowhere in `crates/`. The `fail/` fixture count moved 22 → 26 (29 compiled) and the driver gained a fifth glob, `widget_state_verbs_*`. `WidgetChild` is recorded as a new public export. The test floor is restamped to Phase 17 with the reason it did not move.
- **Phase 18 — the Work Order described an authoring shape that no longer compiles.** Its Goal, Spec, and the "thumb path still presents" gate all wrote `.slider_thumb().focused(BorderColor(...))` on a bare element. Rewritten to `builder.child(El::new()).slider_thumb().focused(...)`, with a Phase 17 constraint bullet and a pointer at the live example.
- **Phase 18 — one deletion became an edit.** `focused_thumb_border_layers_with_the_value_offset` (`widgets/slider.rs:5286`) is the only coverage of the border/offset composition, and this phase changes that composition from one slot-keyed override into a two-channel merge through `apply_element`. Re-point it instead of deleting it; only `:5318`, `:5573`, and the `:5067` fixture are deletions.
- **Phase 18 — the `mut` revert pointed at the wrong file.** The binding is `panel/diegetic_panel.rs:940` feeding `validate_tree(&mut tree)` at `:941`, not `panel/builder.rs:940` (unrelated root-sizing code). That is a third `validate_tree` call site the Files list never named; `layout/element.rs:2366` carries a fourth `mut` that goes unused on the revert.
- **Phase 18 — ref drift and an unrunnable gate.** `focused_thumb_border_color` `:1381` → `:1389` (block `:1380-1382` → `:1386-1391`), `default_state_surfaces` `:719` → `:725`. The "property is gone" gate passed `rg` a repo-root `examples` path that does not exist, so `rg` would error; dropped.
- **Phase 19 — the Spec body carried pre-Phase-16 refs while its own Files list carried correct ones.** A delegate reading top-down hit the stale set first. Spec body restamped to match Files.
- **Phase 19 — the stability constraint named the wrong test and missed four sites.** `:5283`/`:5317` are exact but live in `focused_border_authoring_preserves_sibling_material_source_key` (`:5249`); the insertion test is `inserted_border_command_preserves_existing_material_source_key` (`:5324`) with assertions at `:5358`/`:5389`, and two more sit at `:5447`/`:5474` inside `divider_material_source_keys_are_distinct_and_stable` (`:5396`), where a `HashMap` keys on `(fill_source, command_index)` and a careless re-point would collapse entries. Six sites, all re-pointed, none deleted.
- **Phase 19 — a gate row that could not fail.** "Prove it by the field existing and the sort compiling" is satisfied by anything that compiles. Replaced with a line-range scan asserting `sort_records` contains no `record_key` reference.
- **Reviewed and unchanged:** neither remaining phase is redundant or subsumed by Phase 17, sequencing is unchanged, and Phase 19 touches no widget-child authoring site — its two files were untouched by Phase 17 and every ref in them verified exact.

### Phase 18 — Delete `Slider::focused_thumb_border_color` · status: done

> **Added at review, 2026-07-31**, resolving the open item Phase 14's retrospective
> recorded. Phase 14 deleted the build error that used to reject a focus color on a
> borderless thumb and replaced it with a synthesized `Border::all(Px(0.0), Color::NONE)`.
> A color on a zero-thickness ring draws nothing, so the property cannot render through
> its own authoring path — the author gets silence where they used to get an error.

#### Work Order

**Goal:** The only way to author a slider's focus ring is on the thumb element itself — `builder.child(El::new()).slider_thumb().focused(BorderColor(...))`, which carries a real border width. The widget-spec property that cannot supply a width is gone, and `LayoutTree::validate_widgets` returns to a non-mutating `&self` pass.

**Spec:**

`Slider::focused_thumb_border_color` is the same shape as the `disabled_color` Phase 14 deleted: a color on the widget spec duplicating a per-element property, reachable more directly through the element. It is worse in one respect — it is the only authoring path with **no way to supply a border width**, so it structurally cannot render. Phase 14 already migrated `examples/widgets.rs` off it (the thumb authors `.focused(BorderColor(SLIDER_THUMB_FOCUSED_BORDER))`), leaving the property exercised by nothing but its own unit tests while forcing the validator to mutate.

**The authoring shape changed under this Work Order — Phase 17, 2026-07-31.** A state verb no longer exists on a bare element. `focused` now lives on `impl<L> El<L, WidgetChild>` (`layout/builder.rs:1154`), and `WidgetChild` is minted **only** by `WidgetBuilder::child` (`:2120`), so the surviving authoring path is `builder.child(El::new()).slider_thumb().focused(BorderColor(...))` inside a slider's `WidgetBuilder` closure. Every place this Work Order says `.slider_thumb().focused(...)` on a bare element means that shape. The live example of it is `examples/widgets.rs:1270-1279`.

**Do not "fix" it by giving the synthesized border a visible width.** That synthesis path is shared with `CommonEl::default_state_surfaces` (`layout/builder.rs:725`), so a visible default would silently change what every author of a state border on a borderless element sees. Deletion is the resolved direction.

**Delete, in this order:**

1. The `focused_thumb_border_color` property on `Slider` (`widgets/slider.rs`) — field, default, and builder method — plus `has_focused_thumb_border_color`.
2. The builder forward at `layout/builder.rs:1389` (`El<L, WidgetElement<Slider>>::focused_thumb_border_color`).
3. The border synthesis inside `LayoutTree::validate_widgets` (`layout/element.rs:797`) — the thumb-index collection pass and the `Border::all(Px(0.0), Color::NONE)` writes it performs.
4. The slot pre-layer at `widgets/slider.rs:1201` that seeds the focus border color. The property's other sites are `widgets/slider.rs:171` (field), `:189` (default), `:218`/`:223` (builder), plus `layout/element.rs:842` and `layout/builder.rs:1386-1391` (**restamped after Phase 17** — was `:1380-1382`).
5. Revert `validate_widgets` to `&self`, and with it `widgets::validate_tree` (`widgets/id.rs:259`) and **three** call sites — `panel/builder.rs:842`/`:961` (`tree.as_mut()` → `tree.as_ref()`, the `validate_tree` calls themselves at `:843`/`:962`) and a **third the earlier revision missed**, `DiegeticPanelCommands::set_tree` at `panel/diegetic_panel.rs:940-941`. Phase 14 widened all of them to `&mut` solely to carry this synthesis.

**Files:**
- `src/widgets/slider.rs` — the property, its builder, `has_focused_thumb_border_color`, the slot pre-layer at `:1201`, and the tests naming it (`focused_thumb_border_composes_with_disabled_state_and_drag_offset`, `focused_thumb_border_without_authored_border_builds_and_presents`, the `focused_thumb_state_tree` fixture).
- `src/layout/builder.rs:1389` — the builder forward (**restamped after Phase 17**; was `:1381`).
- `src/layout/element.rs:797` — `validate_widgets`; the synthesis and the `&mut` receiver. Also `src/layout/element.rs:2366` — a test binds `let mut tree` solely to call `tree.validate_widgets()` at `:2368`; that `mut` goes unused on the revert. Phase 17 rewrote this test body.
- `src/widgets/id.rs:259` — `validate_tree`; back to `&self`.
- `src/panel/builder.rs:842`, `:961` — `tree.as_mut()` back to `tree.as_ref()`.
- `src/panel/diegetic_panel.rs:940` — `DiegeticPanelCommands::set_tree`'s `mut tree` binding, feeding `widgets::validate_tree(&mut tree)` at `:941`. This is the third call site; drop the `mut` and the `&mut`.

**Constraints from prior phases:**
- **Phase 14 deleted `SliderFocusedThumbBorderColorRequiresThumbBorder`** and its `Display` test-table row. Do **not** reinstate a build error — the whole point is that the thumb's own `.focused(BorderColor(...))` is the authoring path, and it needs no diagnostic.
- **Phase 17 changed the authoring shape this phase preserves.** A widget child is now `builder.child(El::new())…`, never a bare `El::new()`. Any test this phase writes or re-points must use it, or it will not compile. See the Spec's Phase 17 note.
- **Phase 5's zero-width synthesis stays.** `CommonEl::default_state_surfaces` (`layout/builder.rs:725`, **restamped after Phase 17**; was `:719`) keeps minting `Border::all(Px(0.0), Color::NONE)` for state-border authoring on ordinary elements. This phase removes only the slider-specific synthesis inside the validator.
- **Phase 14's example is already migrated.** `examples/widgets.rs` authors the focus ring on the thumb; the example must not need editing for this phase, and if it does, something else still references the deleted property.
- **Phase 16 note:** removing the synthesis removes one command-inserting path, which slightly reduces the churn Phase 16 addresses. Not a dependency in either direction.
- **`set_tree` needs only one character changed, but it is in a different file than an earlier revision said.** The `DiegeticPanelCommands::set_tree` trait method (`panel/diegetic_panel.rs:748`) takes the tree **by value**, so its signature does not change. The `mut` this phase drops is the binding in its **impl**, `fn set_tree(&mut self, entity: Entity, mut tree: LayoutTree)` at `panel/diegetic_panel.rs:940`, feeding `widgets::validate_tree(&mut tree)` at `:941`. An earlier revision pointed at `panel/builder.rs:940`, which is unrelated root-sizing code.
- **Test count may go down here, and that is expected** — this phase deletes tests for a deleted property. Name the deleted tests explicitly in the report; the "count must go up" rule does not apply to a deletion phase. **Three tests reference the property, not two:** `focused_thumb_border_layers_with_the_value_offset` (`widgets/slider.rs:5286`) alongside the two already named (`:5318`, `:5573`), plus the `focused_thumb_state_tree` fixture at `:5067`. **But `focused_thumb_border_layers_with_the_value_offset` is an EDIT, not a deletion — corrected at the Phase 17 review, 2026-07-31.** It is the only test covering the composition this phase changes the mechanism of. Today the focus border and the drag offset land in **one** slot-keyed `VisualSlotOverride` (`widgets/slider.rs:1195-1202`). After the deletion the offset stays slot-keyed while the border color arrives on the element-keyed channel and is merged by `apply_element` (`widgets/visual.rs:245`, which saves and restores `offset`) inside `dispatch_visual_overrides` (`visual.rs:650-656`). That two-channel merge is exactly where one of the two values can silently vanish, and retiring its only test would leave the new path unproven. Re-point it at `builder.child(El::new()).slider_thumb().focused(BorderColor(...))` — the same treatment `valid_slider_retains_authored_configuration` gets below. Only `:5318`, `:5573`, and the `:5067` fixture are deletions. **A fourth test is an edit, not a deletion:** `valid_slider_retains_authored_configuration` (`widgets/slider.rs:1998`) authors the property at `:2004` and asserts on it at `:2012`, but it exists to prove that *authored configuration survives validation* generally — the property is one of several it carries. Strip the property from it and leave the test in place; deleting it would retire an unrelated guarantee.
- **Restate the new floor in the Delegation Context when this phase lands.** The waiver above applies to this phase only; leaving the old number in place would silently lower the bar for whichever of Phases 16/17/19 runs afterwards.
- **No shared surface with Phase 16.** An earlier revision of this Work Order warned that Phase 16 would move a `PanelElementId::auto` producer inside `validate_widgets`. Phase 16 shipped without touching `layout/element.rs` at all — it keyed on `ElementIndex` in the render path and never entered the validator. This phase owns `validate_widgets` alone; its line refs are not disturbed by Phase 16.
...
- `bash ~/.claude/scripts/delegate/verify.sh lint hana_diegetic`
- `bash ~/.claude/scripts/delegate/verify.sh example hana_diegetic widgets`
- `cargo nextest run -p hana_diegetic --test trybuild --run-ignored all` reports **1 passed** (not `0 run / 1 skipped` — the runner's sole test is `#[ignore]` by repo policy; **do not remove the `#[ignore]`**).
- **The property is gone from the public surface.** `rg -n 'focused_thumb_border_color' crates/hana_diegetic` returns nothing. (An earlier revision appended a second path, `examples` — there is no repo-root `examples/` directory, so `rg` errors out and the gate is unrunnable. The examples live at `crates/hana_diegetic/examples`, already inside the first path.)
- **The thumb path still presents.** A slider whose thumb authors `builder.child(El::new()).slider_thumb().focused(BorderColor(...))` over a real authored border resolves that border color when focused, and `None` when not — the surviving authoring path is proven, not merely left compiling.
- **`validate_widgets` takes `&self`.** Proven by the signature compiling with `&self` and no `.clone()` on the element collection.

### Retrospective

**What worked:** The deletion was as mechanical as the Work Order predicted — every named ref was exact, and the `&self` revert reached all three call sites without a borrow fight. Landing it took one delegate pass plus one small fix pass.

**What deviated from the plan:** Nothing in scope. Three deletions the Work Order did not enumerate followed as dead code once the two authorized tests went away: the `queue_release` test helper, the `Scene.window` field, and the `STATE_DISABLED_COLOR` constant. All three had exactly one caller apiece, all inside the deleted tests, and the lint gate is what surfaced them.

**Surprises:**
- Re-pointing `focused_thumb_border_layers_with_the_value_offset` at real authoring forced it to read the thumb override from the **panel-wide** `VisualOverrideIndex` instead of the widget's own `WidgetVisualOverrides` — because a `.focused(BorderColor(…))` on the thumb element is an *element* override, not a widget slot override. That index is fed by both channels, so `assert_eq!(focused.offset, idle.offset)` no longer proved an offset existed at all; `None == None` would have passed. An explicit `idle.offset.is_some()` assertion was added and passes. **Any test that switches from `thumb_override` to `indexed_root` loses that implicit proof and must re-add it.**
- The blind reviewer could not run any build gate: its read-only sandbox cannot take `target/debug/.cargo-build-lock`. Source-reading review still worked, but a reviewer's "verification" section in this setup means nothing was run. Treat the implementation delegate's gate run as the only gate evidence.
- Deleting the property emptied `Slider` of color entirely. It is now a behavior-only spec, which is the end state the whole plan was driving at for sliders — worth stating because a future phase that adds a color back to `Slider` would be reversing a settled outcome, not extending one.

**Implications for remaining phases:** None for Phase 19, which touches `render/fill_batch.rs` and the SDF batch record — files Phase 18 never opened. Phase 19's three named stability tests were re-verified at `fill_batch.rs:5249`/`:5324`/`:5396`; those refs are unchanged. The test floor drops to 1191 and Phase 19 inherits the no-drop rule intact.

### Phase 18 Review

The architect reviewed Phase 19 — the only remaining phase — against Phase 18's implementation. Phase 19 is confirmed **not** redundant: `SdfRecordKey` is still `{ panel, command_index }` and `sort_records` still reaches into it, so nothing Phase 18 shipped touches that work. Findings applied to Phase 19's Work Order:

- **`render/panel_geometry.rs` added to Files** — the phase changes five sites there (`ResolvedSdfSurface` `:528`, the `push_resolved_sdf_surfaces` stamp `:760`, and the `StoredResolvedSdfSurface` roundtrip `:631`/`:663`/`:698`) and the file was listed nowhere. Verified against source before applying; this was the one finding that made the Work Order unbuildable from its own Files list.
- **The divider-stability constraint had its data structure backwards** — it described a `HashMap` keyed on `(fill_source, command_index)` warning about collapsed entries. The map is `HashMap<SdfMaterialSourceKey, CommandIndex>`: `fill_source` is the key, the command index is the **value**, compared `after > before` at `:5484`. Verified against source; the warning now names the real hazard (a re-point that drains that comparison of meaning while still compiling).
- **The unfailable-gate replacement was re-verified as genuinely failable** (2 matching lines today) and given a drift-proof companion, `rg 'pub command_index'` returning nothing — because `sort_records` is guaranteed to move under this phase and a self-restamped range could quietly become unfailable again.
- **Two gate rows asked for tests that already exist.** The split-clipped-border row now names `clipped_filled_border_splits_fill_behind_and_border_in_front_of_image` (`:3893`) and its material twin (`:3973`); the draw-order row now names `in_batch_upload_order_follows_draw_depth_then_command_index` (`:4434`) and asks for its fixture to be tightened to a proven same-depth pair rather than for a new test.
- **Four constraint facts added:** the new key is injective by construction (`gather_surfaces` already separates dividers), nothing reads a batch's records between upserts so the hoist is safe, `assign_contiguous_runs` sorts off the surface not the key and must be left alone, and `record_key.command_index` is also image-record spelling in the same file — `:3846` compares one of each and needs a mixed re-point that compiles either way.
...
> not a design preference — the record key is doing **two jobs in one field**, and only
> one of them wants to be positional.

#### Work Order

**Goal:** `SdfRecordKey` identifies a retained record the same way `SdfMaterialSourceKey` identifies a material row — by what the surface is, not where it sits. Draw order becomes an explicit property of the record instead of a side effect of how its key was built.

**Spec:**

**The two jobs.** `SdfRecordKey` (`render/fill_batch.rs:216-221`) is used as an identity by `position_of` (`:644`), `upsert_record` (`:663`), `remove_record` (`:685`), and `retain_records` (`:767`) — and as a **sort key** by `sort_records` (`:650-661`), which orders by draw depth and then breaks ties on `record_key.command_index`. **(Restamped at the Phase 17 review, 2026-07-31 — this paragraph carried pre-Phase-16 numbers `:212-217`/`:638`/`:657`/`:679`/`:761`/`:644-655` while the Files bullets below already carried the correct ones. The Files bullets are authoritative; every ref in this Spec body now matches them.)**

```rust
left.draw_depth.draw_order_index().cmp(&right.draw_depth.draw_order_index())
    .then(left.record_key.command_index.cmp(&right.record_key.command_index))
```

Two surfaces at the same draw depth paint in command order. That tiebreak is load-bearing and is the reason the field cannot simply adopt Phase 16's identity: an identity has no meaningful ordering, so overlapping same-depth surfaces would paint in an arbitrary, frame-to-frame-unstable order.

**The change.** Split the two jobs:

1. `SdfRecordKey` adopts the identity shape Phase 16 established — `ElementIndex` plus the divider discriminator (parent index + pair ordinal). Keep `panel`. **There are no editor role ids** — Phase 16 was re-scoped at the Phase 15 review to key on `ElementIndex` alone, which the render path already carries; `PanelElementId` never enters the render key.

   **A record key needs a THIRD discriminant that the material key got for free (found at the Phase 16 review).** `push_resolved_sdf_surfaces` (`render/panel_geometry.rs:760-779`) emits **two** `ResolvedSdfSurface`s from one `ElementSurface` when a clipped border splits from its fill: `fill_only()` (`:206`) and `border_only()` (`:221`) each copy the source through `..self`, so both carry the *same* `index` **and** the same `divider_ordinal`. `SdfMaterialSourceKey` survives this because it carries a `role` field (`SdfMaterialRole::Fill` / `Border`); `SdfRecordKey` has no such field. Adopting `(panel, element, divider_ordinal)` alone therefore makes the two halves collide — the second `upsert_record` overwrites the first and the split border record **vanishes from the batch**.

   Add a third field — a `Whole | Fill | Border` discriminant — stamped where the split happens in `push_resolved_sdf_surfaces` and carried on `ResolvedSdfSurface` alongside `divider_ordinal`. This **amends** the "reuse Phase 16's shape exactly" constraint below: reuse the element + divider discriminator verbatim, and add this one field, which the material key already has under a different name.
2. The command index moves onto `ResolvedSdfBatchRecord` (`:344`, whose `record_key` field is `:346`) as its own field — a draw-order tiebreak, read only by `sort_records`.
3. `sort_records` breaks ties on that field instead of reaching into the key.
4. **Hoist the sort out of `upsert_record`, and gate it (decided at review, 2026-07-31).** `sort_records` is called *inside* `upsert_record` (the call is at `:679`, inside the `:663` function), so it runs once per record that fails the equality check — 100 changed records means 100 full `sort_by` passes over the vec in one frame. That is the rebatching cost this phase is for. Move the sort (and `refresh_first_draw_order_index`, which reads sorted order) to a single call after the whole append pass. **It must stay gated on something actually having been upserted** — a dirty flag marked by `upsert_record`, checked once at the hoist point. Do not sort unconditionally every frame; an idle panel must do zero sorting, which is what it does today via the equality early-return.

   **`remove_record` is not part of the gate.** It does **not** sort today (`:685-692` — it calls `refresh_first_draw_order_index` at `:688` but never `sort_records`), and removing an element from a sorted vec cannot unsort it. Marking the dirty flag on removal would force a pointless full sort every time a panel sheds a surface. Upsert only.

   **The hoist point is outside `SdfBatch`** — a batch method cannot see the end of the append pass. Put it in `route_sdf_batch_records`: the upsert loop is **`:1357-1362`** (the `store.upsert_record(record)` call itself is **`:1361`**) and `retain_records` at `:1363`, so the gated sweep goes immediately after `:1363`, iterating `SdfBatchStore::batches_mut()` (`:777`) and sorting each dirty batch. `route_sdf_batch_records` **ends at `:1368`**; SDF `take_empty_batches` is at **`:1444`**, inside the separate `reconcile_sdf_batch_entities` system — an earlier revision implied the two were adjacent in one function, which they are not. The sweep only needs to precede anything that reads sorted records, and nothing between `:1363` and `:1368` does.

**Why this is not merely cosmetic.** As written, anyone changing the record key's shape silently changes draw order, and no test covers it — the coupling is invisible. After the split, identity and order fail independently and each has its own gate row.

**No drop path here.** Unlike the material table, retained records have no capacity limit and reject nothing: lookup is a linear scan (`position_of`, `:644`), retention is set membership (`retain_records`, `:767`). Positional record keys cost rebatching work on insertion, not a visible defect — which is why this never surfaced as a bug. Do not import Phase 15's zero-drop framing into this phase's gate; there is nothing to drop.

**Where the cost actually is (corrected at the Phase 15 review, 2026-07-31).** An earlier revision of this Work Order implied the key change alone removes the insertion cost. It does not. `record_upload` is a `Dirty` flag on the **batch** (`:595`), not per record, and an insertion lengthens `self.records`, so the record buffer re-uploads regardless of how the key is shaped. The removable work is the **per-upsert sort** in change 4 — which is why that change is required for this phase to deliver its stated benefit. Note also that the draw-order field must be written normally: skipping the assignment to dodge the equality check would leave stored records holding stale order values that collide with a freshly inserted record's, producing an ambiguous sort. The `transform` carry-forward at `:665` is not a precedent here — there the *stored* value is authoritative and the incoming one is a placeholder; for draw order it is the other way round.

**Files** — **all line refs re-verified against the post-Phase-16 tree, 2026-07-31, and re-audited at the Phase 18 review the same day** (Phase 18 touched no render file, so the SDF-side numbers held; the four corrections that follow were pre-existing drift, not Phase 18 shift).

- **`src/render/panel_geometry.rs` is in scope and was missing from this list entirely.** Spec change 1 carries the `Whole | Fill | Border` field on `ResolvedSdfSurface`, so five sites here change: the `ResolvedSdfSurface` struct (**`:528`**, its `command_index` at `:532`, and note the element field is spelled **`element_index`** here, not `index` as on `ElementSurface`), the stamp in `push_resolved_sdf_surfaces` (**`:760`**, the two split pushes at `:778-779`), and the `StoredResolvedSdfSurface` roundtrip — struct **`:631`** (**not `:637`**, which was wrong), `from_resolved` **`:663`**, `as_resolved` **`:698`**. Both roundtrip constructors are exhaustive field lists with no `..`, so a missed field is a compile error rather than a silent drop.
- `src/render/fill_batch.rs:214-221` — `SdfRecordKey`; `command_index` → the Phase 16 identity shape plus the `Whole | Fill | Border` field.
- `src/render/fill_batch.rs:344` — `ResolvedSdfBatchRecord` (the `record_key` field is at `:346`); gains the draw-order field.
- `src/render/fill_batch.rs:416`, `:1334`, `:3508` — the three `SdfRecordKey` construction sites (the last is a test).
- `src/render/fill_batch.rs:644` `position_of`, `:663` `upsert_record`, `:685` `remove_record`, `:724` `type MemberKey`, `:767` `retain_records` — identity consumers; shape change only. Note `upsert_record` also has a **public** wrapper, `SdfBatchStore::upsert_record` (`:760`) — change both.
- `src/render/fill_batch.rs:650-661` — `sort_records`; the tiebreak moves off the key, and the call moves out of `upsert_record` (`:679`) to one gated call after the append pass. `refresh_first_draw_order_index` (`:631-642`, called from `:680` and `:688`) moves with it — but **not** because it reads sorted order. It computes a `.min()` over `self.records` and is order-independent, as is `position_of` (a linear scan). Record order is therefore **not** load-bearing between upserts: the vec may sit unsorted for the entire append loop, which is exactly what makes the hoist safe. `record_upload: Dirty` (`:601`) and `bounds_update` (`:603`) show the existing dirty-flag idiom to follow for the gate.
- `src/render/fill_batch.rs` test helpers constructing, returning, or **sorting on** `SdfRecordKey` — `sdf_records` (`:2635-2648`, which sorts on `record_key.command_index` at `:2643-2645`), `sdf_material_assignments_except` (**`:2768-2787`**, `sort_by_key` at **`:2783-2785`**), the key literal at `:3508`, `sdf_record_key_for_fill_color` (`:5018`), and `sdf_record_batch_snapshot` (**`:5026`**, not `:5024`). **Ten further tests assert on `record_key.command_index` directly** — `:3806`, `:3846`, `:4414`, `:4415`, `:4427`, `:4428`, `:4453`, `:4454`, plus the two Phase 16 rows named in the constraints below. Every one needs re-pointing at the new draw-order field; none should be deleted.

**Constraints from prior phases:**
- **Phase 16 owns the identity shape.** Reuse the element + divider discriminator exactly — `ElementIndex` plus `Option<usize>` pair ordinal — and add the one field Spec change 1 names (`Whole | Fill | Border`), which the material key already carries as `role`. That addition is the **only** sanctioned divergence; do not invent a second identity scheme beyond it. **Phase 16 does not produce editor role ids or structural auto ids** — that scope was cut when Phase 16 was re-scoped to key on `ElementIndex`. The editor's per-keystroke tree rebuild re-keys its own subtree in both phases; Phase 15's capacity margin covers it.
- **Do not touch `SdfMaterialSourceKey`.** Phase 16 settled it. This phase changes the record key only.
- **Six assertion sites read the field this phase deletes — re-point them all, delete none. Corrected and expanded at the Phase 17 review, 2026-07-31.** An earlier revision named only two and attributed them to the wrong test:
  - `fill_batch.rs:5283` (`Some(CommandIndex::from(1))`) and `:5317` (`Some(CommandIndex::from(2))`) are exact, but both live in **`focused_border_authoring_preserves_sibling_material_source_key`** (`:5249`) — an *authoring* test, not the insertion test the earlier text described.
  - The **insertion** test Phase 16 actually shipped as its position-shifts-while-identity-holds proof is **`inserted_border_command_preserves_existing_material_source_key`** (`:5324`), whose `record_key.command_index` assertions are at **`:5358`** and **`:5389`**. It was never named here.
  - Two further sites are in **`divider_material_source_keys_are_distinct_and_stable`** (`:5396`), at **`:5447`** and **`:5474`**. **The Phase 18 review corrected this description — an earlier revision had the data structure backwards.** It collects `HashMap<SdfMaterialSourceKey, CommandIndex>`: the **key** is `fill_source` alone, already unique per divider, and `command_index` is the **value**, compared `after > before` at **`:5484`**. So the hazard is not collapsed entries; it is that substituting the new *identity* key for the **value** either fails to compile or silently drains the `after > before` comparison of meaning. Re-point the value to the new draw-order field and leave the key alone.
  After the split every one of these asserts on the record's new draw-order field and still shows the same shift. Deleting any of them retires Phase 16's proof. The "ten further tests" enumeration in **Files** is short by these four.
- **`CommandIndex` survives this phase.** Removing it from `SdfRecordKey` does not retire the type: it stays in use at `render/draw_order.rs:30`, in `render/mod.rs`, in `image_batch.rs`, and on `ResolvedSdfSurface.command_index` (`panel_geometry.rs:532`) / `StoredResolvedSdfSurface` (`:637`) — and it becomes the type of the new draw-order field. Do not delete the type or its impls.
- **Scope boundary — `ImageRecordKey` stays positional.** `image_batch.rs:72-77` keys image records by command index and sorts inside its own `upsert_record` (`:318`, comparator at `:310-312`), the same two defects this phase fixes for SDF records. Leave it alone. Image records are emitted one-per-image directly from the command list with no split-surface or divider producers, so the key is injective as written, and the volume is a fraction of the SDF path — the sort hoist has no measurable payoff there. Changing it would widen this phase past one reviewable commit.
- **Phase 15's zero-drop rows must still pass**, unchanged.
- **The new key is injective by construction, and the proof is in a file this phase would otherwise not read.** `gather_surfaces` (`panel_geometry.rs:403`) accumulates non-divider surfaces into a `HashMap<ElementIndex, ElementSurface>` (`:410`) and dividers into a separate `Vec` stamped `divider_ordinal: Some(ordinal)` (`:428`). `(panel, element_index, divider_ordinal)` is therefore **already** unique over `ElementSurface`; the fill/border split is the **only** source of multiplicity, which is exactly what the third `Whole | Fill | Border` discriminant covers. Do not re-derive this and do not add a fourth discriminant.
- **Nothing reads a batch's records between upserts, so the sort hoist is safe.** `SdfBatch::upsert_record` is private with exactly one direct caller outside the routing system — `fill_batch.rs:3544` in `batch_bounds_use_clipped_world_corners`, which asserts `world_bounds` and is order-independent. Every other path is `Batch::insert`/`update` (`:729`/`:734`) → `SdfBatchStore::upsert_record` (`:760`) → the routing loop (`:1361`).
- **Leave `assign_contiguous_runs` alone.** Its comparator (`fill_batch.rs:1168-1173`) sorts by `(draw_order_index, command_index)` read off **`ResolvedSdfSurface`**, not off `record_key`, so it survives the key change untouched — but a grep for "sorts by command index" will find it. Two consequences: because records therefore reach each batch already in order, each per-upsert `sort_by` is O(n) on a near-sorted vec rather than O(n log n). The hoist still removes O(n²) total work; the Spec's "100 full `sort_by` passes" framing overstates the per-pass cost and should not be read as a per-pass estimate.
- **`record_key.command_index` is also how *image* records spell it inside this same file — do not blanket-rename.** `image_records` (`:2650-2663`) sorts `ResolvedImageRecord` on `record_key.command_index` at `:2655-2660`, and the assertion at **`:3846`** compares one `ImageRecordKey` against one `SdfRecordKey` in a single expression. That site needs a **mixed** re-point: the image side keeps `record_key.command_index`, the SDF side moves to the new draw-order field. Both are `CommandIndex`, so a wrong re-point still compiles — which is what makes this the most dangerous of the ~18 sites.
- **Sequencing:** after Phase 16. Independent of Phases 17 and 18, which share no surface with the render work. **Confirmed after Phase 18 landed, 2026-07-31:** Phase 18 touched only `layout/builder.rs`, `layout/element.rs`, `panel/builder.rs`, `panel/diegetic_panel.rs`, `widgets/id.rs`, and `widgets/slider.rs` — it never opened `render/fill_batch.rs`, so **every line ref in this Work Order is unmoved and was re-verified exact**: the three test definitions at `:5249`, `:5324`, `:5396` and all six assertion sites above. This is the last phase in the plan; the widget-appearance surface is closed and nothing in it is Phase 19's concern.
- **Test floor is 1191**, not 1193 — Phase 18 deleted two tests whose subject it deleted, and that waiver is spent. Phase 19 may not land below 1191.
...
- `bash ~/.claude/scripts/delegate/verify.sh lint hana_diegetic`
- `bash ~/.claude/scripts/delegate/verify.sh example hana_diegetic widgets`
- **Record identity survives insertion.** Inserting a surface above an existing unnamed surface leaves that surface's `SdfRecordKey` unchanged. Assert on the key.
- **Draw order is unchanged by the split.** Two overlapping surfaces at the **same** draw depth paint in the same relative order before and after this phase, and that order still follows command order. This is the row that would catch the regression the current coupling hides — it must be able to fail. **A test already covers this and was not named (corrected at the Phase 18 review) — strengthen it rather than build a new one.** `in_batch_upload_order_follows_draw_depth_then_command_index` (`fill_batch.rs:4434`, assertions `:4453-4457`) reads `single_sdf_batch_records` — raw batch order, not the helper-sorted `sdf_records` — and asserts records `[0]`/`[1]` in command order. Its one shortfall is that it asserts `draw_order_index()[0] <= [1]`, so its fixture is **not proven to be a same-depth pair**, which is what this row demands. Tighten the fixture to a known same-depth pair and make the depth equality an assertion rather than an inequality.
- **Draw order survives insertion too.** Inserting a surface between two same-depth surfaces leaves their relative order intact.
- **A split clipped border keeps both records.** A surface whose border clips separately from its fill produces two records with distinct keys, and both survive the upsert pass. This is the row that catches the collision Spec change 1 describes. **Two tests already drive exactly this path — use them, do not build a fixture (corrected at the Phase 18 review).** `clipped_filled_border_splits_fill_behind_and_border_in_front_of_image` (`fill_batch.rs:3893`) and `clipped_material_filled_border_splits_fill_behind_and_border_in_front_of_image` (`:3973`), with fixtures `filled_clipped_image_border_tree` (`:2548`) and `material_filled_clipped_image_border_tree` (`:2565`), exercise `should_split_clipped_border` and **already fail** if the new key is adopted as `(panel, element, divider_ordinal)` without the `Whole | Fill | Border` field. The gate is that these two keep passing.
