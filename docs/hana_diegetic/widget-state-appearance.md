...
- **Phase 12 — `tint` is the seventh property**, not the "sixth-and-final"; `Appearance` already carries six.
- **Line-reference drift**, corrected throughout: `bevy_kana/src/cascade.rs` shifted ~+50 by the `main` merge (which also gave `CascadePlugin` a second type parameter), `widgets/appearance.rs` by ~+130, `widgets/mod.rs` +4, `panel/builder.rs` +5, `panel/mod.rs` by the clippy helper extraction, plus `layout/element.rs` and `layout/builder.rs` refs in Phases 13 and 14. Phase 13's own "these refs are unaffected" note was wrong about two of them and now says so.
...
- `src/widgets/appearance.rs:243`/`:285`/`:327`/`:369` — the four public `Widget*Appearance::new` constructors, the global authoring entry point, likewise.
...
1. **Key on identity, not position.** Replace the `command_index` field with the element's `PanelElementId` (`ime/ids.rs:85`), already stored on every `Element` as `id: Option<PanelElementId>` (`layout/element.rs:109`) and readable via `element_id` (`:696`). Keep `panel` and `role`.
...
- `layout/builder.rs` drifted again after Phase 6. **Re-verified at Phase 8 — these override every line number elsewhere in this Work Order:** `next_auto_id`
  ...
  **`:1700`**; `take_auto_id` **`:1764`**; `tooltip_add_text` **`:2039`** with its
  ...
  and the `render/material_table.rs` refs are unaffected. **The two `layout/element.rs` refs in this phase's Files were NOT unaffected and are now corrected:** the `Element::id` field is `:109` and the `element_id` accessor `:696` (this block previously asserted `:108` / `:679` still held). Everything else this phase cites was re-verified intact on 2026-07-30: `layout/builder.rs:1493`, `:1621`/`:1646`/`:1700`, `:1764`, `:2039`, and `layout/element.rs:1348`.
...
**Files:**
- `src/layout/builder.rs:924` — the `impl<L> El<L, LayoutOnly>` block holding all four verbs (`disabled` at `:937`); `AcceptsElement::with<L, Role>` (default method, `builder.rs:1720`) at `:1713`; the role markers `WidgetPart` (`:106`) / `PressedPart` (`:110`) behind sealed `ElementRole` (`:113`); `AcceptsElement<Role>` (`:1554`) and its five impls (`:1845`, `:1858`, `:1872`, `:1889`, `:1906`).
