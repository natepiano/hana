...
- **Line-reference drift** corrected across every remaining phase and the Delegation
  ...
  `widgets/appearance.rs`, and the three presenters. Phases 14 and 16 cited
...
- **Line-reference drift**, corrected throughout: `bevy_kana/src/cascade.rs` shifted ~+50 by the `main` merge (which also gave `CascadePlugin` a second type parameter), `widgets/appearance.rs` by ~+130, `widgets/mod.rs` +4, `panel/builder.rs` +5, `panel/mod.rs` by the clippy helper extraction, plus `layout/element.rs` and `layout/builder.rs` refs in Phases 15 and 16. Phase 15's own "these refs are unaffected" note was wrong about two of them and now says so.
...
- Phase 10's test insertions moved line numbers in four files by **non-uniform**
  amounts, including within a single file. Every line ref in Phases 11–16 was
...
- **No remaining phase is redundant, mis-scoped, or mis-sequenced.** Phases 11–16 were re-read against the implemented tree; their goals, boundaries, and ordering all hold. Phase 14's focus-border section was already pre-trimmed to a deletion by Phase 10's own embedded resolution, and Phase 15 remains correctly independent of the appearance work.
...
- **Every line reference in Phases 11–16 was re-verified against the working tree and corrected in place.** Phase 10's ~1000 new test lines shifted four files by **non-uniform** amounts, including within a single file: `layout/builder.rs` has three bands (+1, +20, +21), `widgets/visual.rs` three (+27, +47, +93), `layout/element.rs` a flat +14 from `EditorElementOrigin` on, and `widgets/slider.rs` drifted −1 to −4. Phase 14 carried the most stale refs, since it edits the two files Phase 10 changed most.
- **Four pre-existing wrong references were found and fixed in the same pass** — they predate Phase 10 and would have sent a delegate to the wrong code: `SliderFocusedThumbBorderColorRequiresThumbBorder`'s two raise sites and its producer (Phase 14), the `El<L, LayoutOnly>` `disabled` line (Phase 16), the second `with<L, Role>` method (Phase 16), and `RoleSealed` (Phase 11).
- **`AcceptsElement::with` is not a trait default method.** Phases 14 and 16 both described it that way; there are two **inherent** `with<L, Role>` methods, `LayoutBuilder::with` (`layout/builder.rs:1741`) and `WidgetBuilder::with` (`:1969`). Corrected wherever cited, since Phase 16's central difficulty is exactly this signature.
...
- **Phase 16 must name a crate-internal promotion that survives it.** `EditorStateColors::into_editor_part` (`layout/builder.rs:174`) performs the `LayoutOnly → WidgetPart` promotion Phase 16 abolishes, privately, with no enclosing widget in the type. It is correct — the `editor_*` arms are the widget — but a delegate reading only "abolish the promotion" either breaks `editor_*` or widens the public relation to accommodate it.
...
- **`AcceptsElement::with` does not exist** — corrected in Phases 14 and 16 for the third time. The trait (`builder.rs:1696`) declares only `type ChildBuilder` and `with_child_builder`; the method taking `El<L, LayoutOnly>` is `LayoutContentBuilder::with` at `:1718`. Phase 16's central difficulty is that signature, so the wrong name pointed at the wrong sealing boundary.
...
- **Two constraints added to Phases 14 and 16:** `.hovered(Default::default())` no longer infers (write `Appearance::default()`), and editor parts get no `tint` role — `EditorPartColorRole` has exactly `Fill` and `Text`, and Phase 13 must not extend it.
...
- **The bound is now `impl IntoAppearance`, not `impl Into<Appearance>`.** Phase 16 rewrites those same 16 state-verb signatures; it must carry the new bound through.
...
- `bash ~/.claude/scripts/delegate/verify.sh test hana_diegetic`
- `bash ~/.claude/scripts/delegate/verify.sh lint hana_diegetic`
- **`cargo nextest run -p hana_diegetic --test trybuild --run-ignored all` reports `1 passed`.** This phase adds a fifth `IntoAppearance` impl, which invalidates `tests/trybuild/fail/widget_state_appearance_bare_color.stderr` whether or not the `on_unimplemented` label is edited. Without this line the staleness escapes the phase entirely and surfaces in Phase 16 or CI: the ordinary `verify.sh test hana_diegetic trybuild` reports `0 run / 0 passed / 1 skipped`, because the runner's sole test is `#[ignore]` by deliberate repo policy — 89-second compile, CI runs ignored tests separately. **Do not remove the `#[ignore]`.** Re-record with `TRYBUILD=overwrite` and inspect the result before accepting it.
...
- **Phases 15 and 16 need no re-scoping from this phase.** Phase 15's out-of-scope image-key constraint (recorded at the Phase 13 review) still holds, and nothing Phase 14 changed touches the material-row `row_limit` that Phase 15 addresses.
...
- `src/layout/builder.rs:1624` (declaration), `:1752`/`:1777`/`:1831` (the three constructors), `:1895` (`take_auto_id`) — auto-id minting becomes structural.
- `src/layout/builder.rs:2170-2187` — **a second minting path the original Work Order missed.** `LayoutTree::tooltip_add_text` mints `PanelElementId::auto` from a caller-held counter (parameter at `:2174`, minting at `:2181`/`:2183`); structural auto ids must cover it or tooltip content keeps positional ids.
...
**Ref corrections and added constraints (re-verified at the Phase 12 review, 2026-07-30, against the post-Phase-12 tree):**
- `layout/builder.rs` is now **2288** lines and drifted **non-uniformly** in Phase 12
  — roughly `+1` above line 1170 and `-11` below 1350 — so no single offset applies.
  **These override every line number elsewhere in this Work Order:** `next_auto_id`
  declaration **`:1624`**; the three `next_auto_id: 0` inits **`:1752`** / **`:1777`** /
  **`:1831`**; `take_auto_id` **`:1895`**; `tooltip_add_text` **`:2170`** with its
  counter parameter at **`:2174`** and its two minting sites at **`:2181`** / **`:2183`**;
  `AcceptsElement<Role>` trait **`:1685`**; `impl<L> El<L, LayoutOnly>` **`:1066`**;
  `EditorPart::into_text` **`:645`** with its `common.id = None` at **`:646`**.
  Two earlier revisions of this block are superseded: the Phase-6-vintage set
  (`:1493` / `:1621` / `:1646` / `:1700` / `:1764` / `:2039`) and the Phase-11 set
  (`:1635` / `:1763` / `:1788` / `:1842` / `:1906` / `:2181-2198` / `:2185` / `:1696` /
  `:1065` / `:644` / `:645`). Neither is correct now.
  **Nothing else in this phase moved.** Phase 12 touched only `examples/widgets.rs`,
  `cascade/attributes.rs`, `layout/builder.rs`, `lib.rs`, `panel/builder.rs`,
  `widgets/appearance.rs`, and `widgets/mod.rs` — so `layout/element.rs:109` / `:710` /
  `:838` / `:1146` / `:1161` / `:1314` / `:1362` / `:1378`, every
  `render/material_table.rs`, `render/fill_batch.rs`, `render/draw_order.rs`, and
  `ime/ids.rs` ref is unaffected, and this phase's substantive content is untouched
  by Phase 12.
- **Add the editor content tree as the proving case.** `inline_editor_content_tree`
  ...
  `EditorPart::into_text` sets `common.id = None` (`layout/builder.rs:646`), so an
  author cannot stabilize them with a `Named` id. Use this path as the case that
  proves structural ids actually work.

### Phase 16 — The state verbs require a widget in the type · status: todo
...
**Files:**
- `src/layout/builder.rs:1066` — the `impl<L> El<L, LayoutOnly>` block holding all four verbs (`hovered` `:1070`, `focused` `:1078`, `disabled` `:1086`, `pressed` `:1095`); the **trait** method `LayoutContentBuilder::with` (`:1707`) with its two impls (`:2055`, `:2131`) and the two inherent `with` methods `LayoutBuilder::with` (`:1851`) / `WidgetBuilder::with` (`:2079`); the role markers `WidgetPart` (`:108`) / `PressedPart` (`:112`) behind sealed `ElementRole` (`:236`); `AcceptsElement<Role>` (`:1685`) and its five impls (`:1976`, `:1989`, `:2003`, `:2020`, `:2037`).
...
**Constraints from prior phases:**
- **Phase 4:** the acceptance relation and the five `AcceptsElement` impls are what this phase revises. `tests/trybuild/pass/typestate_helpers.rs::add_widget_content` is the worked example of a part-authoring helper; it takes `&mut WidgetBuilder<'_, W>` for a concrete owner because `LayoutContentBuilder::with` (the trait method at `layout/builder.rs:1707`) could not be made generic over the builder. That constraint is what this phase has to solve properly.
