...
- **Key files** (line refs re-verified after Phase 2; files neither Phase 1 nor Phase 2 touched still carry `HEAD` = `64f8bdc0` refs):
  ...
  - `examples/widgets.rs` (**1703** lines after Phase 12) — `.disabled_color` use (**`:1165`**), `add_slider` (**`:1205`**), `apply_state_appearance` (**`:1459`**). It imports `bevy::prelude::*` **and** `use hana_diegetic::BackgroundColor;` — an explicit single-name `use` shadows a glob import, so the deliberate `bevy::prelude` name collision resolves cleanly and needs no alias.
...
### Phase 18 — Delete `Slider::focused_thumb_border_color` · status: todo
...
4. **Hoist the sort out of `upsert_record`, and gate it (decided at review, 2026-07-31).** `sort_records` is called *inside* `upsert_record` (the call is at `:679`, inside the `:663` function), so it runs once per record that fails the equality check — 100 changed records means 100 full `sort_by` passes over the vec in one frame. That is the rebatching cost this phase is for. Move the sort (and `refresh_first_draw_order_index`, which reads sorted order) to a single call after the whole append pass. **It must stay gated on something actually having been upserted** — a dirty flag marked by `upsert_record`, checked once at the hoist point. Do not sort unconditionally every frame; an idle panel must do zero sorting, which is what it does today via the equality early-return.
   ...
   **The hoist point is outside `SdfBatch`** — a batch method cannot see the end of the append pass. Put it in `route_sdf_batch_records`: the upsert loop is at `:1360-1362` and `retain_records` at `:1363`, so the gated sweep goes immediately after `:1363`, iterating `SdfBatchStore::batches_mut()` (`:777`) and sorting each dirty batch, before `take_empty_batches` runs.
...
**Files** — **all line refs re-verified against the post-Phase-16 tree, 2026-07-31.** Phase 16 restamped this file; every number below is the current one.
...
- `src/render/fill_batch.rs:650-661` — `sort_records`; the tiebreak moves off the key, and the call moves out of `upsert_record` (`:679`) to one gated call after the append pass. `refresh_first_draw_order_index` (`:631-642`, called from `:680` and `:688`) reads sorted order and moves with it. `record_upload: Dirty` (`:601`) and `bounds_update` (`:603`) show the existing dirty-flag idiom to follow for the gate.
- `src/render/fill_batch.rs` test helpers constructing, returning, or **sorting on** `SdfRecordKey` — `sdf_records` (`:2635-2648`, which sorts on `record_key.command_index` at `:2643-2645`), `sdf_material_assignments_except` (`:2770-2787`, `sort_by_key` at `:2783-2784`), the key literal at `:3508`, `sdf_record_key_for_fill_color` (`:5018`), and `sdf_record_batch_snapshot` (`:5024`). **Ten further tests assert on `record_key.command_index` directly** — `:3806`, `:3846`, `:4414`, `:4415`, `:4427`, `:4428`, `:4453`, `:4454`, plus the two Phase 16 rows named in the constraints below. Every one needs re-pointing at the new draw-order field; none should be deleted.
...
- **Six assertion sites read the field this phase deletes — re-point them all, delete none. Corrected and expanded at the Phase 17 review, 2026-07-31.** An earlier revision named only two and attributed them to the wrong test:
  ...
  - Two further sites are in **`divider_material_source_keys_are_distinct_and_stable`** (`:5396`), at **`:5447`** and **`:5474`**, which key a `HashMap` on `(fill_source, record_key.command_index)` — a re-point must keep that pair unique or the test silently collapses entries.
...
- **Sequencing:** after Phase 16. Independent of Phases 17 and 18, which share no surface with the render work.
