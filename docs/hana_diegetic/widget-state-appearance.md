...
### Phase 17 — The state verbs require a widget in the type · status: todo
...
**Goal:** The only way to author a slider's focus ring is on the thumb element itself — `.slider_thumb().focused(BorderColor(...))`, which carries a real border width. The widget-spec property that cannot supply a width is gone, and `LayoutTree::validate_widgets` returns to a non-mutating `&self` pass.
...
**Do not "fix" it by giving the synthesized border a visible width.** That synthesis path is shared with `CommonEl::default_state_surfaces` (`layout/builder.rs:719`), so a visible default would silently change what every author of a state border on a borderless element sees. Deletion is the resolved direction.
...
2. The builder forward at `layout/builder.rs:1381` (`El<L, WidgetElement<Slider>>::focused_thumb_border_color`).
...
4. The slot pre-layer at `widgets/slider.rs:1201` that seeds the focus border color. The property's other sites are `widgets/slider.rs:171` (field), `:189` (default), `:218`/`:223` (builder), plus `layout/element.rs:842` and `layout/builder.rs:1380-1382`.
5. Revert `validate_widgets` to `&self`, and with it `widgets::validate_tree` (`widgets/id.rs:259`) and the two `DiegeticPanelBuilder::build` call sites (`panel/builder.rs:842`, `:961`) — Phase 14 widened all three to `&mut` solely to carry this synthesis.
...
- `src/layout/builder.rs:1381` — the builder forward.
- `src/layout/element.rs:797` — `validate_widgets`; the synthesis and the `&mut` receiver.
- `src/widgets/id.rs:259` — `validate_tree`; back to `&self`.
- `src/panel/builder.rs:842`, `:961` — `tree.as_mut()` back to `tree.as_ref()`.
- `src/panel/diegetic_panel.rs` — `set_tree` signature, if it still carries `&mut` for this reason alone.
...
- **Phase 5's zero-width synthesis stays.** `CommonEl::default_state_surfaces` (`layout/builder.rs:719`) keeps minting `Border::all(Px(0.0), Color::NONE)` for state-border authoring on ordinary elements. This phase removes only the slider-specific synthesis inside the validator.
...
- **The property is gone from the public surface.** `rg -n 'focused_thumb_border_color' crates/hana_diegetic examples` returns nothing.
- **The thumb path still presents.** A slider whose thumb authors `.focused(BorderColor(...))` over a real authored border resolves that border color when focused, and `None` when not — the surviving authoring path is proven, not merely left compiling.
...
**The two jobs.** `SdfRecordKey` (`render/fill_batch.rs:212-217`) is used as an identity by `position_of` (`:638`), `upsert_record` (`:657`), `remove_record` (`:679`), and `retain_records` (`:761`) — and as a **sort key** by `sort_records` (`:644-655`), which orders by draw depth and then breaks ties on `record_key.command_index`:
...
**No drop path here.** Unlike the material table, retained records have no capacity limit and reject nothing: lookup is a linear scan (`position_of`, `:638`), retention is set membership (`retain_records`, `:761`). Positional record keys cost rebatching work on insertion, not a visible defect — which is why this never surfaced as a bug. Do not import Phase 15's zero-drop framing into this phase's gate; there is nothing to drop.
...
- **Phase 16's two stability tests read the field this phase deletes — re-point them, do not delete them.** `fill_batch.rs:5283` asserts `record_key.command_index == Some(CommandIndex::from(1))` and `:5317` asserts `Some(CommandIndex::from(2))`. They are Phase 16's evidence that a record's *position* shifts while its *material key* does not, which is precisely the coupling this phase splits. After the split they must assert on the record's new draw-order field and still show the same shift. Deleting them would silently retire Phase 16's proof.
...
- **`sort_records` no longer reads `record_key`.** The tiebreak reads the record's own draw-order field; prove it by the field existing and the sort compiling without a `record_key` reference in the comparator.
