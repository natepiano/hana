...
### Phase 16 — Stable material identity: keys survive command insertion · status: todo
...
- `crates/hana_diegetic/tests/trybuild/` — the compile-fail fixtures for this phase, plus every existing fixture whose recorded error moves. **Five `fail/` fixtures are affected, not two.** Phase 11's `editable_widget_editor_part_has_no_property.rs` and `editable_widget_editor_part_rejects_pressed_colors.rs` author `El::new().editable_field(…)`. **Three more already author a state verb on a bare `El::new()`** and are today rejected *downstream* rather than at the verb — `widget_state_appearance_outside_widget.rs`, `_forged_scope.rs`, and `_tooltip_part.rs`, whose snapshots currently record `E0277: LayoutBuilder cannot accept WidgetPart` and `E0308 mismatched types`. Once the verb leaves `El<L, LayoutOnly>` the error moves onto the verb itself and all five `.stderr` snapshots need re-recording. Phase 12's `widget_state_appearance_bare_color.stderr` is a sixth if the verb's signature or bound wording changes.
...
- **`validate_widgets` is shared surface with Phase 16**, which is otherwise independent: Phase 16 moves the `PanelElementId::auto` producer at `layout/element.rs:832` (the `SliderThumbOutsideSlider` path, which survives this phase) inside the same function body this phase rewrites. Whichever runs second inherits shifted line refs — re-locate by name, not by number.
...
**Files:**
- `src/render/fill_batch.rs:212-217` — `SdfRecordKey`; `command_index` → the Phase 16 identity shape.
- `src/render/fill_batch.rs:342` — `ResolvedSdfBatchRecord.record_key`; gains the draw-order field.
- `src/render/fill_batch.rs:410`, `:1308`, `:3454` — the three `SdfRecordKey` construction sites (the last is a test; re-verified 2026-07-31, was `:3409`).
- `src/render/fill_batch.rs:638` `position_of`, `:657` `upsert_record`, `:679` `remove_record`, `:718` `type MemberKey`, `:761` `retain_records` — identity consumers; shape change only.
- `src/render/fill_batch.rs:644-655` — `sort_records`; the tiebreak moves off the key, and the call moves out of `upsert_record` (`:673`) / `remove_record` (`:682`) to one gated call after the append pass. `refresh_first_draw_order_index` (`:628-636`) reads sorted order and moves with it. `record_upload: Dirty` (`:595`) and `bounds_update` (`:597`) show the existing dirty-flag idiom to follow for the gate.
- `src/render/fill_batch.rs:2719`, `:4821`, `:4831` — test helpers constructing or returning `SdfRecordKey` (re-verified 2026-07-31, were `:4776`/`:4786`).

**Constraints from prior phases:**
- **Phase 16 owns the identity shape.** Reuse it exactly — `ElementIndex` plus the divider discriminator (parent index + pair ordinal). Do not invent a second identity scheme; if Phase 16's shape does not fit a record, that is a Phase 16 defect to report, not a local workaround. **Phase 16 does not produce editor role ids or structural auto ids** — that scope was cut when Phase 16 was re-scoped to key on `ElementIndex`. The editor's per-keystroke tree rebuild re-keys its own subtree in both phases; Phase 15's capacity margin covers it.
...
- **One sort per frame at most, and zero when nothing changed.** A frame that upserts *n* changed records sorts once, not *n* times — assert by counting sort invocations (a test-visible counter or an instrumented comparator) across a frame that changes several records. A frame in which no record is upserted or removed must sort **zero** times; an idle panel does no sorting today and must not start.
