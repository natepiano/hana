...
### Phase 10 — Tooltip authoring, relationship, and controller reify  · status: todo
...
- **Easy mesh-face target:** add one checked mesh-anchor command that accepts any live `Mesh3d` entity plus `MeshFace::{PositiveX, NegativeX, PositiveY, NegativeY, PositiveZ, NegativeZ}` and returns `TooltipTargetEntity<World>`. The selected face is persistent general anchor authoring, not tooltip state. After Bevy's `VisibilitySystems::CalculateBounds` supplies the entity's local `Aabb`, Hana lazily derives the usual nine rectangular `Anchor` points and their face frame into `ResolvedAnchorGeometry` while attachment demand exists. Authors never construct those points by hand. A missing-yet-expected `Aabb` leaves the target pending; an entity that never obtains usable mesh bounds remains hidden with a bounded warning. Mesh-handle or bounds changes refresh the derived local geometry, while ordinary transform changes require no rebuild because Valence composes the local points with the current transform. The public call is intentionally one step before `spawn_tooltip`, approximately `let cube = commands.mesh_anchor_target(cube, MeshFace::PositiveZ);` followed by `commands.spawn_tooltip(cube, tooltip);`; settle only mechanical extension-trait naming during implementation.
...
- **Coordinate-space inheritance:** resolve a widget target through `WidgetOf`, read a panel target directly, or consume the settled general anchor-provider context. A panel/widget world target supplies its layout unit; a panel/widget screen target supplies its window, camera order, render layers, and pixel layout unit. General targets provide the equivalent facts through the Phase 10 contract without acquiring an invented panel owner. World tooltips lower to `hana_valence::AnchoredTo`; screen tooltips retain private panel authoring and use the generalized screen target adapter. The materialized placement target must always match `TooltipFor`.
...
- **Persistent hidden state and cleanup:** materialization starts world or screen anchor demand; later hiding retains the same panel, layout, and attachment without inactivity eviction. Normal renderer routes may retire hidden GPU batch rows and rebuild them on show; that is not tooltip or panel respawn. Hidden controller, target, component-only panel-role, and full panel despawn paths detach and clean the materialized state through existing lifecycle ordering without visibility events; Phase 10.75 extends those paths only for a tooltip that was actually visible.
...
**Constraints from prior phases:** Phase 10 supplies stable controller identity, `TooltipFor`/`Tooltips`, placement builders/defaults, the checked standalone lifecycle, general target providers, and `TooltipSystems::ControllerCommandsApplied`. Phase 3 supplies the owner-panel teardown contract; Phases 4 and 4.5 supply world/screen widget geometry, typed same-space placement, demand, and `finalize_widget_anchor_state`. Panel/widget branches mint current handles through `PanelEntityReader`/`PanelWidgetReader`; a general target uses Phase 10's provider path. A materialized tooltip is an active attachment even while hidden, so converting its source or target panel is rejected until it detaches; pre-materialized `TooltipFor` membership alone does not block conversion. Phase 9 showed that component-level state does not prove final coordinate conversion, so readiness tests inspect the propagated world/screen result.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: a private request materializes the same controller as a hidden `Fit × Fit` panel through panel fence → target acquisition → attachment fence; no request leaves the controller lightweight and creates no anchor demand; repeated requests reuse one panel and attachment; world and screen panel/widget targets plus one mesh-face general target become ready only after current layout, placement, and propagated transform; a missing provider or current resolver diagnostic remains hidden and not ready; a secondary-window target transfers window, camera order, and render layers; a world widget honors nonzero offset; the settled keep-visible policy proves preferred/opposite/remaining-side order, bounded along-edge shifting, eight-logical-pixel margin, reflow-then-hide, off-viewport hiding, and interaction-camera projection; the settled Hana-owned tooltip `PanelPicking` behavior is installed during materialization; hidden teardown paths detach and clean exactly once without visibility events. No test in this phase drives the operating-system pointer or implements show/hide timing.
