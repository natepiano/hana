...
- The layout unit for an arbitrary world target remains a product decision in Phase 10.5. Panel and widget targets already provide their panel unit, but a general world target currently provides geometry only.
...
### Phase 10.5 — Hidden tooltip materialization and readiness  · status: todo
...
**Pending decision: Layout unit for arbitrary world tooltip targets**

Actual problem:
A panel or widget target supplies its panel's layout unit, but a general world target such as the mesh-face cube supplies `ResolvedAnchorGeometry` without a layout unit. Materialization needs a unit before it can create the tooltip panel.

What exists now:
- `TooltipTargetEntity<World>` identifies the target's coordinate space, and the mesh-face helper supplies geometry on demand from the current `Aabb`.
- `Unit::Meters` is the crate's default world-space unit and Bevy's ordinary world-space convention.

What should change:
- General world targets use `Unit::Meters`; panel and widget targets continue to inherit their owning panel's unit. A later typed general-world context can expose another unit if real use requires it.

Recommendation:
Use `Unit::Meters` for arbitrary world targets in v1. It makes the one-call mesh-face helper complete without adding another public component or builder before there is a demonstrated need.
...
- **Dual-space readiness:** layout is ready only after the materialized controller has a current `ComputedDiegeticPanel`. Screen placement is ready after `PanelSystems::PositionScreenSpace` when `ResolvedScreenPanelPosition::anchor_position` is `Some`; world placement is ready after `AnchorSystems::Resolve` when the controller retains its Valence attachment and no current-frame `ResolveDiagnostics` entry names it as skipped. The private ready state is finalized after `TransformSystems::Propagate`, so its `GlobalTransform` is current. Phase 10.75 may reveal only a ready controller.
- **Persistent hidden state and cleanup:** materialization starts world or screen anchor demand; later hiding retains the same panel, layout, and attachment without inactivity eviction. A mesh-face target derives geometry from the current application-maintained `Mesh3d` and `Aabb` only after that demand begins; this phase adds no asset-event, asset-revision, bounds-generation, or delayed-bounds machinery. Normal renderer routes may retire hidden GPU batch rows and rebuild them on show; that is not tooltip or panel respawn. Hidden controller, target, component-only panel-role, and full panel despawn paths detach and clean the materialized state through existing lifecycle ordering without visibility events; Phase 10.75 extends those paths only for a tooltip that was actually visible.
