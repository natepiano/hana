...
- Register screen dependency proxies and reverse-demand state with Phase 3's central panel-role ownership lifecycle. Teardown removes world, screen, and combined demand exactly once before the owned widget disappears, without despawning application-owned dependent panels.
...
- **Panel-role teardown:** Phase 3's central lifecycle calls the focus transition before despawning panel-owned widgets. It removes `WidgetFocused`, clears that window's active-panel/widget entry, and emits exactly one `WidgetFocusChanged` with `WidgetRemoved` while the target and `WidgetOf` remain queryable.
...
- **Lifecycle and demand:** pre-materialized `TooltipFor` membership is not anchor demand. Materialization inserts the world/screen placement relationship and starts demand; hiding retains it. Retargeting updates `TooltipFor`, `PanelOwned`, and the materialized placement target through the same transition. World `AnchoredHere` and Phase 4.5's screen reverse relationship keep geometry resident only while placement needs it. There is no inactivity eviction. Before Phase 3's central teardown despawns a controller because its target panel role ended, run the same visibility finalizer used by other removal paths so a visible controller emits one `TooltipHidden` while its template, transform, and relation remain queryable.
...
**Spec:**
- Stop after Phase 10.5 and use `crates/hana_diegetic/examples/widgets.rs` as the existing world-space Fairy Dust baseline. Design together how that lab is extended or supplemented; do not reopen which example owns the cumulative widget path.
