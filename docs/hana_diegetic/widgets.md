...
- **Public contract ledger (fixed before delegation except where a phase-local pending-decision block names the unresolved surface):**
  ...
  - The six exported semantic messages are `FocusNextWidget`, `FocusPreviousWidget`, `FocusFirstWidget`, `FocusLastWidget`, `ActivateFocusedWidget`, and `CancelFocusedWidget`; each carries `{ window: Entity }`. Phase 5 resolves the named window through private focus authority, applies IME and disabled gating, and emits one private entity-targeted `SemanticWidgetIntent`. The adapter exports `WidgetInputPlugin`, `WidgetInputBindings`, and `WidgetControlSummary`; Phase 5.5's pending decisions fix its runtime settings/rebind/disable surface, focused-window activation rule, defaults, and neutral summary. Initial installation remains `app.add_plugins(WidgetInputPlugin::new(WidgetInputBindings::default()))`, after which the plugin owns one context entity per window.
...
- Phase 5 carries a deferred owner decision for pointer focus originating from cameras whose render target is not a window; its existing semantic-routing decision and the Phase 5.5/10 decisions remain intentionally deferred to their phase gates.
...
- Phase 5.5 now targets the six shipped window-scoped messages exactly; its runtime settings, focused-window routing, default bindings, controller ambiguity, and neutral summary remain explicit owner decisions at that phase gate.
...
**Pending decision: focused-window routing, default controls, and neutral summary**

Actual problem:
Enhanced-input key edges are application-global, while Phase 5 focus is per-window. If every per-window context remains active, one key press can emit the same semantic request for every window; the plan also promises default bindings and `WidgetControlSummary` without fixing their values or shape.

What exists now:
- Phase 5's six messages each require one exact window and keep that window's focus independent.
- Bevy keyboard events identify the operating-system window that received input, but enhanced-input action state alone is not a window selector.
- The canonical example currently writes the six messages directly and therefore does not exercise adapter scoping or summaries.

What should change:
- Activate keyboard bindings only for the operating-system-focused window; window focus transfer changes the active context without changing either window's remembered widget focus.
- Freeze default next/previous/first/last/activate/cancel bindings and define a binding-library-neutral `WidgetControlSummary` that help UI can read without exposing enhanced-input action entities.
- Define controller input behavior when more than one window exists, because a gamepad edge carries no operating-system window.

Recommendation:
Use Tab, Shift+Tab, Home, End, Enter/Space, and Escape as the keyboard defaults. Route keyboard input only to the focused window; route controller input only when exactly one live window is focused, otherwise emit nothing until the application supplies an explicit window policy. Make `WidgetControlSummary` an immutable snapshot of the six semantic controls and their display labels, derived from `WidgetInputBindings`.
...
**Constraints from prior phases:** Phase 5 ships private `WidgetFocusAuthority`, `WidgetSystems::SemanticInput`, six public `{ window }` messages, and private entity-targeted `SemanticWidgetIntent`. The adapter writes only those public messages after enhanced-input resolution and before `WidgetSystems::SemanticInput`; it never reads or mutates focus authority, performs widget lookup, emits the private intent, or exposes enhanced-input types through core behavior APIs. The canonical example's direct shortcut writers remain the binding-library-independent baseline.

**Acceptance gate:** `cargo nextest run` green with new tests: default install creates one context per live window; one action edge emits the corresponding Phase 5 message for exactly the selected window; IME blocks routed behavior only in its leased window; adding/removing/focusing windows reconciles context activation; runtime rebind updates contexts without duplicates; repeated equal configuration is a no-op; disable removes plugin-owned context/action entities without clearing focus; re-enable restores exactly one installation per window; controller ambiguity follows the settled window policy; `WidgetControlSummary` exactly reflects defaults and runtime rebinds without exposing enhanced-input entities; removing the adapter-owned configuration follows the settled public contract. Extend and smoke-test the public path in `examples/widgets.rs`.
...
- **Complete reset:** blueprint replacement reuses the settled `DiegeticPanel` role-removal teardown instead of hand-maintaining a second cleanup list. It removes every materialized layout, render, widget, text, placement, retained-sibling, and index component, including `PanelWidgetIndex`, before installing the new blueprint. Reveal resumes only after fresh layout and placement readiness.
...
- **Default presentation:** `widgets/presets/tooltip.rs` supplies `TooltipPreset` / `TooltipStyle` through the Phase 7.5 retained visual-slot boundary. Rich content remains ordinary panel content; the preset does not add behavior authority or mutate layout in response to hover/focus state.
...
**Constraints from prior phases:** Phase 10 supplies stable controller identity, template diff inputs, semantic relationships, panel-role ownership, and the settled placement builders/defaults. Phase 10.5 supplies the sole visibility state machine, hidden materialization path, readiness gates, duplicate-safe finalizer, and resident hidden-panel contract. Phase 3 supplies the complete panel-role teardown contract; Phases 4/4.5 supply typed same-space attachment, demand, and detach-before-convert behavior; Phase 7.5 supplies retained visual slots. Replacement and retargeting must call those existing paths rather than creating parallel lifecycle, attachment, or presentation authorities.

**Acceptance gate:** `cargo nextest run` green with new tests: a new blueprint resets every old panel/widget/text/render/placement component and rebuilds hidden on the same controller; an identical clone does nothing; policy-only and placement-only changes do not rebuild; placement-only replacement updates the checked attachment after its fence; same-space/same-unit retargeting updates ownership, inherited context, and attachment in place; cross-space or different-unit retargeting emits one hidden event if visible, detaches, and rematerializes the same controller without attempting an attached conversion; reveal waits for fresh layout and placement; repeated equal replacement or retargeting is a no-op; `TooltipPreset` patches only its retained visual slots and passes the behavior/preset boundary test. Extend and smoke-test the public path in `examples/widgets.rs`.
