...
- Phase 4.5 now rejects cross-space widget placement, derives screen proxy bounds from current owner-panel state, and treats overlapping world/screen membership only as an internal lifetime case.
...
- Phase 4.5's file list now covers screen scheduling, placement, rects, world target metrics, widget registration, lifecycle, and anchoring documentation.
...
**Spec:**
- The screen placer builds candidates from `PanelAttachmentAuthored` but accepts panel targets only today. Teach it to recognize a widget, resolve the owning screen panel/window through `WidgetOf`, and derive the target rectangle from current widget-local geometry plus the owning panel's screen rect/transform instead of `ScreenPanelRect` on the target. Accept a widget only when its owner and source are compatible screen panels in the same window. A widget has one owning `PanelSpace`: a world source targeting a screen widget or a screen source targeting a world widget diagnoses and falls back without placement. Reconcile Hana-owned world lowering when a target is not world-space, while preserving an application-replaced `hana_valence::AnchoredTo`; a screen source must never carry Hana's world relation.
...
- Generalize Phase 4 retirement without coupling independently owned components. Losing final world demand always retires only Hana's private world bridge. Shared geometry retires only when both world `AnchoredHere` and screen reverse membership are empty. Losing final screen demand retires only the screen proxy/reverse state unless geometry also has no remaining demand. Application replacement of either the world bridge or geometry is preserved independently.
- Register screen dependency proxies and reverse-demand state with Phase 3's central panel-role ownership lifecycle. Teardown first unions and deduplicates world and screen dependents, removes each relationship exactly once while targets are queryable, then retires Hana-owned bridge/geometry/proxy state before owned widget despawn. It never marks or despawns an application-owned dependent panel.
...
- `src/panel/anchoring.rs` — coordinate-neutral owner-unit conversion and cross-space world-lowering reconciliation
...
**Constraints from prior phases:** Phase 1 reifies widget identity early in `Update`; Phase 2 names the post-reify command fence `WidgetSystems::ReifyCommandsApplied`; Phase 3 adds the widget rects and panel-local `Transform`; Phase 4 owns centered `WidgetAnchorRect` geometry, a private world bridge, owner-based world offset conversion, and independent ownership records for bridge and geometry. Phase 4 seeds a new widget's initial global transform once, but screen placement must still use the owning panel's current resolved screen state rather than the child global. Screen sources retain private `PanelAttachmentAuthored` and do not enter the world resolver. The existing canonical lab's world readout follows the bottom slider with a positive pixel gap and remains part of the cumulative example.

**Acceptance gate:** `cargo nextest run` green with new tests: a command-queued attachment to a new or relaid-out widget target resolves in the same frame after both required fences; an attached, non-unit-scaled, rotated, off-origin owner panel → widget → dependent chain follows graph order using owner-derived bounds and never the child global; placement uses the widget viewport rect and nonzero offset; two screen attachments maintain demand until both detach; an internal overlapping world/screen membership case keeps geometry until both demand sets are empty, while public cross-space attachments diagnose and fall back without placement; retargeting moves reverse membership; world-demand loss removes only the Hana-owned bridge, screen-demand loss removes only screen state, final no-demand removes geometry, and application replacement of bridge or geometry is preserved independently; removing the owning `DiegeticPanel` under active screen-only or overlapping internal demand removes every proxy, Hana-owned geometry/bridge component, reverse membership, and deduplicated dependent relationship while preserving application-owned dependent panels; panel targets remain unchanged; missing owner/geometry and cross-space warnings deduplicate per source, target, and reason. Extend `examples/widgets.rs` with a distinct screen-widget target while preserving and smoke-testing the existing world readout below the slider.
...
- **Panel-role teardown:** Phase 3's central lifecycle calls the focus transition before Phase 4 anchor cleanup and before despawning panel-owned widgets. It removes `WidgetFocused`, clears that window's active-panel/widget entry, and emits exactly one `WidgetFocusChanged` with `WidgetRemoved` while the target, `WidgetOf`, and attachment relations remain queryable.
...
**Constraints from prior phases:** Phase 2 supplies `WidgetDisabled`; Phase 3 supplies pick targets and `PickingInteraction`; Phase 1 supplies current traversal order. Phase 4's central panel-role path now finalizes anchor dependents before owned widget despawn, so focus finalization must be registered ahead of anchor cleanup. Activation of a focused button lands in Phase 6; this phase routes the action to that later behavior hook.
...
- **Panel-role teardown order:** register button capture finalization in the central lifecycle before Phase 4 anchor cleanup and owned-widget despawn. Emit exactly one terminal event while `WidgetOf` and any attachment relation remain queryable; anchor cleanup and recursive owned-entity removal happen only afterward.
...
- `src/widgets/mod.rs` — visual override and preset-system registration
...
**Acceptance gate:** `cargo nextest run` green with new tests: hover/press/focus and panel/global disabled edges patch only expected render rows and do not fire `Changed<DiegeticPanel>` or `Changed<ComputedDiegeticPanel>`; a widget-local disabled edit performs exactly its Phase 2 visual-only computed refresh and no layout solve, then preset presentation causes no second panel/computed change; preset z/transform patches leave the owning widget's `Transform`, computed hit rects, interaction rank, and cross-widget picking order unchanged; repeated identical state is a no-op; unrelated visual slots stay untouched; the behavior-module boundary test passes. Extend and smoke-test the public path in `examples/widgets.rs`.
...
- **Panel-role teardown order:** register slider capture finalization in the central lifecycle before Phase 4 anchor cleanup and owned-widget despawn. Emit one terminal event while `WidgetOf` and attachment state remain queryable, then allow anchor and owned-entity cleanup to proceed.
...
**Acceptance gate:** `cargo nextest run` green with new tests: thumb/fill records track the applied value in all four directions; a value change dirties only the expected visual slots and causes no relayout; preset builds under the behavior/preset boundary test. Extend and smoke-test the public path in `examples/widgets.rs`.
...
- Phase 4 shipped `AnchoredToPanel`, widget targets, and unit-aware `PanelAnchorOffset`; Phase 10.5 is supposed to consume that one placement path.
...
- Define placement-only replacement as an `AnchoredToPanel` update on an already materialized controller, without rebuilding its panel blueprint or respawning it.
...
**Constraints from prior phases:** Phase 1 preserves widget/text indexes when an identical `set_tree` advances the source revision without changing computed output; tooltip controller indexes must follow the same rule because no computed change is guaranteed to retrigger reify. Phase 2 provides `PanelWidgetReader` and `WidgetSystems::ReifyCommandsApplied`. Phase 3 provides `PanelOwned` and the one panel-role teardown choke point; this phase extends it rather than relying on target-entity despawn. Controller existence alone creates no world or screen demand from Phases 4/4.5.

**Acceptance gate:** `cargo nextest run` green with new tests: associated tooltip authoring does not alter `Button`/`Slider` equality; one lightweight controller and exact reverse relationship exist after `TooltipSystems::ControllerCommandsApplied` with no `DiegeticPanel`, placement relation, render data, or geometry demand; unrelated and policy-only tree replacements reuse that controller; an exact identical `set_tree` replacement preserves the same associated controller and every controller lookup/index without relying on a computed-panel change; a blueprint replacement updates the same controller; a standalone tooltip can target an entity resolved from `(panel, id)` and the same call accepts an already-known event target without lookup; retargeting transfers panel-role ownership; omitted builders produce 500 ms show and zero hide delays plus `Suppress`; associated declaration removal and target despawn each clean up exactly once; removing only the target panel's `DiegeticPanel` cleans associated and standalone controllers plus indexes exactly once while the panel entity survives. Extend and smoke-test the public path in `examples/widgets.rs`.
...
- **Lifecycle and demand:** pre-materialized `TooltipFor` membership is not anchor demand. Materialization inserts the world/screen placement relationship and starts demand; hiding retains it. Retargeting updates `TooltipFor`, `PanelOwned`, and the materialized placement target through the same transition. World `AnchoredHere` and Phase 4.5's screen reverse relationship keep geometry resident only while placement needs it. There is no inactivity eviction. Before Phase 4 anchor cleanup or Phase 3 owned-entity despawn runs because a target panel role ended, invoke the same visibility finalizer used by other removal paths so a visible controller emits one `TooltipHidden` while its template, transform, `TooltipFor`, and placement relation remain queryable.
...
- **Defaults and presentation:** `show_after` defaults to 500 ms, `hide_after` to zero, and disabled policy to `Suppress`; anchors and offset use the placement contract settled in Phase 10's pending decision. `widgets/presets/tooltip.rs` supplies the default panel presentation. Rich content is ordinary panel content; overflow avoidance is out of scope.
...
- `src/panel/diegetic_panel.rs`, `src/panel/lifecycle.rs` — reuse and extend the settled panel-role teardown/reset path
- `src/lib.rs` — curated tooltip visibility-event and preset exports
...
**Constraints from prior phases:** Phase 10 supplies stable controller identity, `TooltipFor`/`Tooltips`, the settled placement builders/defaults, template diff classification, and `TooltipSystems::ControllerCommandsApplied`. Phase 5 supplies `WidgetFocused` and `WidgetSystems::InteractivityCommandsApplied`; Phase 3 supplies `PickingInteraction` and the owner-panel teardown contract; Phases 4 and 4.5 supply world/screen widget target geometry, demand, and anchor cleanup. Visible-tooltip finalization must run before that anchor cleanup. Phase 7.5 supplies retained visual infrastructure for the default preset.

**Acceptance gate:** `cargo nextest run` green with new tests: show wait cancels; hide grace cancels on renewed eligibility and `Duration::ZERO` hides immediately; first show materializes hidden and reveals only after layout+placement, including zero delay and `Fit`; every later hide/show preserves the same entity and resident panel/render state; a new blueprint resets every old panel/widget/text/render/placement component and rebuilds hidden on that same controller, while an identical clone does nothing and a policy-only change does not rebuild; `Suppress` immediately hides an already-visible tooltip; `TooltipShown` and `TooltipHidden` target the tooltip entity exactly once per completed visibility edge, emit nothing for canceled waits or redundant states, expose ready transforms on show, and emit `TooltipHidden` before visible-controller cleanup; retargeting updates semantic ownership and materialized placement targets; visible associated removal and target despawn emit one hidden event before cleanup, while hidden cleanup emits none; removing the target panel role emits one hidden event before anchor cleanup and associated/standalone visible-controller cleanup, while `TooltipFor` and placement state remain queryable, and emits none for hidden controllers with no duplicate cleanup; standalone and associated tooltips share the path; world and screen targets inherit the correct space; a world widget tooltip honors the settled nonzero offset. Extend and smoke-test the public path in `examples/widgets.rs`.
...
**Spec:**
- Stop after Phase 10.5 and use `crates/hana_diegetic/examples/widgets.rs` as the cumulative dual-space Fairy Dust baseline: Phase 4's world readout still follows the bottom slider, and Phase 4.5 adds a distinct screen-widget attachment exercise. Design together how that lab is extended or supplemented; do not reopen which example owns the cumulative widget path.
...
**Constraints from prior phases:** All widget subsystems through Phase 10.5 complete and are tested through deterministic minimal-app coverage before demonstration work begins; the canonical example already retains both world- and screen-widget anchoring paths.
