...
- **Public contract ledger (fixed before delegation except where a phase-local pending-decision block names the unresolved surface):**
  ...
  - Slider exports are `Slider`, `SliderState`, `SliderRange`, `SliderStep`, `SliderDirection`, `SliderConfigError`, `SliderGrabbed`, `SliderChangeRequested`, `SliderReleased`, `SliderCanceled`, `SliderCancelCause`, `RequestSliderAdjustment`, `SliderAdjustment`, `slider_self_update`, `SliderPreset`, and `SliderStyle`. Phase 10 appends `TooltipTemplate`, `TooltipFor`, `Tooltips`, and `TooltipDisabledPolicy`; Phase 10.5 appends `TooltipShown` and `TooltipHidden`; Phase 11 appends `TooltipPreset` and `TooltipStyle`.
...
- **Panel-role teardown:** Phase 3's central lifecycle calls the focus transition before Phase 4.5's combined `finalize_widget_anchor_state` cleanup and before despawning panel-owned widgets. It removes `WidgetFocused`, clears that window's active-panel/widget entry, and emits exactly one `WidgetFocusChanged` with `WidgetRemoved` while the target, `WidgetOf`, and both world and screen attachment relations remain queryable.
...
- The former Phase 10.5 was split without changing behavior: Phase 10.5 owns initial timing, materialization, readiness, and teardown; Phase 11 owns replacement, retargeting, and the default preset.
...
- The Phase 7.5 public preset authoring/material surface remains a deliberate owner decision rather than being inferred by implementation.
...
- **Interaction boundary:** visual-slot overrides may change appearance or translate presentation records in the owning widget's panel-local XY plane. They preserve every record's authored `DrawCommandDepth` and never alter the widget `Transform`, `rect`, `clipped_rect`, or interaction rank from Phase 3, so they cannot lift one widget over another or expand its hit bounds. Presets author fixed part layering through ordinary tree `z_index`; cross-widget depth and hit-bound changes require authoritative tree/layout authoring and are outside runtime state presentation.
...
- The private slot path is proven in library tests; the unchanged example remained a callback regression smoke because no production preset authors slots yet.
...
- Presets author fixed part layering in their ordinary layout trees; button and slider hover, press, focus, disabled, and value state may change only retained appearance or panel-local XY translation.
- Button and slider presets reuse the same private slot/index path and existing material/texture re-keying; the tooltip preset is static template authoring because it has no independent changing visual state.

#### Phase 7.25 Review

- Updated Phase 7.5 to expose the private authoring/mutation hooks only inside the crate, carry each preset instance's authored style through computed records and reify, and order presentation writes before override dispatch with an explicit deferred-command fence.
...
- Updated Phase 9 to reuse the same presentation order and record the unresolved slider-fill geometry choice rather than assuming a translation-only override can resize a fill record.
- Updated tooltip phases so hidden tooltips retain their controller, panel, layout, and placement state while renderer rows may retire and rebuild; `TooltipPreset` is a static template-authoring helper rather than a state-driven retained-slot writer.
- Deferred two owner choices: the exact private carrier for a per-instance `ButtonStyle` in Phase 7.5, and whether Phase 9 ships a moving-thumb preset or expands retained overrides to resize a fill.

### Phase 7.5 — `ButtonPreset` on retained visual slots  · status: todo

#### Work Order

**Goal:** A default button presentation maps current button state onto the retained visual-slot path without owning button behavior.

**Pending decision: public preset authoring and material surface**

Actual problem:
The plan names `ButtonPreset`/`ButtonStyle`, `SliderPreset`/`SliderStyle`, and `TooltipPreset`/`TooltipStyle` without freezing how application code constructs or applies them. It also promises arbitrary custom or `ExtendedMaterial` handles even though current public panel authoring accepts `Handle<StandardMaterial>` and the renderer's extended materials are private. Most importantly, a style chosen for one authored button instance needs a defined path from its `El` through the computed widget record to that reified widget entity; a global preset system cannot recover which style that instance selected.

What exists now:
- `El::button(id, Button)` and `El::slider(id, Slider)` own semantic authoring; rich child content remains ordinary layout.
- Phase 7.25 supplies private retained visual slots and supports current fill/image/text/shape batching, including re-keying.
- `WidgetSpec` carries semantic button/slider configuration and should not become presentation authority merely to transport a style.

What should change:
- Keep presets as layout-authoring helpers that produce ordinary `El` subtrees and attach stable visual slots; they must not become ECS behavior components or a second widget API.
- Freeze constructors, style ownership, and fluent state-style methods before delegation, then require slider and tooltip presets to follow the same pattern where their structures permit.
- Limit the first public material surface to colors, images, and `Handle<StandardMaterial>`; defer arbitrary custom shaders until a separate public renderer contract exists.
- Carry a private cloneable presentation snapshot beside `WidgetSpec`: author it on the preset's root `El`, include it in `ComputedWidgetRecord`, and reify it onto the matching widget entity. Same-id reify updates that snapshot without replacing behavior state, callback identity, stable slots, or current overrides.

Recommendation:
Approve a `ButtonPreset` helper that owns a cloneable `ButtonStyle`, returns an `El` already carrying the supplied `(id, Button)`, and accepts ordinary child content through existing layout nesting. Give `ButtonStyle` private fields, `Default`, and fluent setters for normal, hovered, pressed, focused, and disabled colors/images/`StandardMaterial` values. Store the selected style in a private `WidgetPresentationSpec` (or equivalently narrow private carrier) beside `WidgetSpec`, carry it through `ComputedWidgetRecord`, and reify it as private widget presentation state. Use the same style-value convention for the later slider preset; make `TooltipPreset` a static template-authoring helper because tooltips have no changing presentation state. Do not expose private `ExtendedMaterial` types.

Approve this direction, or modify it?

**Spec:**
- **Button presentation:** `widgets/presets/button.rs` and shared helpers in `presets/style.rs` generate ordinary layout fragments with Phase 7.25 stable slots. The preset passes its caller-supplied `Button` unchanged into `El::button`; it never reconstructs the button or inspects or replaces its callback handle. Presets read `PickingInteraction`, `Has<WidgetDisabled>`, `Has<WidgetFocused>`, and the private `ButtonPress` marker, then write only widget visual overrides. They never read or mutate `ButtonCaptures`, which remains lifecycle and ordering authority. Rich content stays ordinary layout content.
...
- **Focus command fence:** add `WidgetSystems::FocusCommandsApplied`, an explicit `ApplyDeferred` after Phase 5's `WidgetSystems::SemanticInput`. Preset systems that read `WidgetFocused` run after this fence, so pointer/app focus, cleanup, and semantic traversal marker commands are visible in the same frame. Phase 10.5 reuses this scheduling point for tooltip focus eligibility.
- **Presentation ordering:** run the button presentation writer after `WidgetSystems::FocusCommandsApplied`. Follow it with `WidgetSystems::PresentationCommandsApplied`, an explicit `ApplyDeferred` needed when the writer first inserts its private override component. Run `dispatch_visual_overrides` after that fence, before the retained fill/image/text/shape routes in `PostUpdate`, so a state edge reaches retained data in the same frame. Phase 9 reuses this order.
...
- **Boundary guardrail:** presets depend on behavior, never the reverse; behavior modules (`button.rs`, `focus.rs`, `interactivity.rs`, and peers) reference no layout or material types.

**Files:**
- `src/widgets/presets/mod.rs`, `src/widgets/presets/button.rs`, `src/widgets/presets/style.rs` — public preset/style implementation after the pending decision is settled
- `src/layout/builder.rs`, `src/layout/element.rs`, `src/widgets/id.rs`, `src/widgets/reify.rs` — crate-private preset slots and per-instance presentation snapshot transport
- `src/widgets/mod.rs` — focus fence and preset-system registration
- `src/widgets/visual.rs` — consume the Phase 7.25 private override path
- `src/lib.rs` — curated preset/style exports
- `crates/hana_diegetic/examples/widgets.rs` — canonical retained button-presentation exercise

**Constraints from prior phases:** Phase 7.25 supplies stable slot identity, reified references, order-independent stale-key removal before current-key insertion, four retained-render routes, material/texture re-keying, and stock-picker exclusion for newly created batches. Its authoring/mutation hooks remain crate-private and test-gated until this phase installs the shared production writer above. Runtime overrides preserve authored depth and may change only appearance or panel-local XY translation; fixed part layering stays in the preset's ordinary tree. Phase 6 supplies the private `ButtonPress` presentation marker and keeps `ButtonCaptures` private lifecycle authority. Phase 6.5 supplies the final per-face hit stream. Phase 3 supplies `PickingInteraction`; Phase 5 supplies `WidgetFocused` and `WidgetSystems::SemanticInput`. Phase 5.5's built-in adapter and app-owned example path both send the same Phase 5 messages before semantic input, so one `WidgetSystems::FocusCommandsApplied` fence covers either origin; do not add an adapter-specific fence. Presentation runs focus fence → preset writer → presentation command fence → override dispatch → retained renderer routes; do not rely on unspecified system insertion order.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: hover/press/focus and panel/global disabled edges patch only expected retained slots; two buttons using different `ButtonStyle` values retain the correct per-instance styles through computed output and same-id reify; pressed presentation reads `ButtonPress` and never reads or mutates `ButtonCaptures`; current-frame pointer/app/traversal focus is visible after `WidgetSystems::FocusCommandsApplied`; first insertion is visible to dispatch after `WidgetSystems::PresentationCommandsApplied`; a widget-local disabled edit performs exactly its Phase 2 visual-only computed refresh and no layout solve, then presentation causes no second panel/computed change; repeated state leaves the `WidgetVisualOverrides` change tick unchanged and causes no retained upload; unrelated slots remain untouched; `.on_click` fires once and retains the same tracked system through presentation-state changes; the behavior-module boundary test passes. Extend and smoke-test the public path in `examples/widgets.rs` while preserving and expanding the existing diagnostic readout.
...
**Spec:**
- **Drag mapping:** map panel-local position to normalized value, then through the chosen range. Each `Pointer<Drag>` reprojects `pointer_location.position` via the **captured press camera and render target** → `Camera::viewport_to_world` → ray → flat panel intersection → panel-local map → clamp. Extend the existing `render::project_flat_panel_hit` boundary in `render/panel_geometry.rs` with the ray-intersection input slider dragging needs; do not create a second `panel_local_from_ray` authority in `widgets/picking.rs`. `Drag.delta` is invalid for perspective world panels. Cancel if the captured camera/target disappears or no longer matches. Surface-panel integration later replaces only this shared flat projection boundary.
...
**Constraints from prior phases:** Phase 8 supplies applied state and the proposal API. Phase 8.25 supplies the private cross-button/slider occupancy and raw-order authority while preserving button-specific terminal hooks and `ButtonPress` timing. Same-id/same-kind reify preserves active drag state, Phase 7.25 stable slots/current overrides, and the Phase 7.5/9 presentation snapshot. Phase 6.5 supplies the final per-face hit stream: `Interactive` and `WidgetsOnly` may target a slider, while `PanelOnly` and `PassThrough` cannot. Phase 3 supplies panel-local geometry, `render::project_flat_panel_hit`, and the press hit's camera. Phase 4.5 supplies combined world/screen attachment cleanup through `finalize_widget_anchor_state`; Phase 5 supplies the post-interactivity fence over Phase 2's `WidgetDisabled` plus paired `On<Remove>`/early `On<Despawn>` panel finalization. Slider capture finalization extends both lifecycle paths and runs while both attachment relation forms remain queryable.
...
**Pending decision: slider value geometry**

Actual problem:
Phase 7.25 can recolor or translate a retained record in panel-local XY, but it cannot resize the fill bar that this phase currently says should grow and shrink with the slider value. Pretending translation can do that would either move a fixed-size fill incorrectly or force layout regeneration on every value change.

What exists now:
- An overlay can author a fixed track and thumb with ordinary depth and layout.
- The retained override path can move the thumb to the applied value without relayout and can restyle either record.
- No retained override changes a record's width, height, pivot, or clip boundary.

What should change:
- Either ship the first preset as a fixed track plus moving thumb, with no variable-length fill, or explicitly add a private panel-local XY size/scale override with a defined leading-edge pivot and renderer coverage.
- Do not mutate `LayoutTree` or `ComputedDiegeticPanel` per slider value.

Recommendation:
Ship the simpler first preset as a fixed track plus moving thumb. Keep an optional authored fill decorative and fixed, rather than presenting it as value progress. Add retained fill resizing later only when a concrete design requires it; that avoids expanding all retained renderer routes for one preset assumption.

Approve this direction, or modify it?

**Spec:**
- `widgets/presets/slider.rs`: `SliderPreset` / `SliderStyle` (widget-specific names, material-first slots like ButtonPreset).
- Use `El::overlay()` — track, thumb, optional fixed decorative fill, and optional labels share one content rectangle and are layered, not arranged. `DrawZIndex` orders thumb above decoration above track.
- Thumb placement reads Phase 8's applied value. Restyle and thumb movement write Phase 7.25 visual-slot overrides, patching or re-keying only retained render records — no `LayoutTree` or computed-panel regeneration per value change. If the owner instead approves resizable fill, define and test that retained geometry override before using it here.
- Preset respects `SliderDirection` for thumb placement in all four directions.
- Reuse Phase 7.5's presentation order: applied state → slider preset writer → `WidgetSystems::PresentationCommandsApplied` → `dispatch_visual_overrides` → retained renderer routes.

**Files:**
- `src/widgets/presets/mod.rs` — slider preset module declaration and export
- `src/widgets/presets/slider.rs` — new
- `src/widgets/presets/style.rs` — shared helpers only where they remove real duplication
- `src/widgets/mod.rs` — slider preset registration
- `src/lib.rs` — re-exports
- `crates/hana_diegetic/examples/widgets.rs` — canonical slider-presentation exercise

**Constraints from prior phases:** Phase 8 supplies `SliderDirection` and the applied-value contract; Phase 8.5 supplies the completed pointer lifecycle. Phase 7.25 supplies stable visual slots and batch re-keying while preserving authored depth. Phase 7.5 supplies the shared private override writer that immutable-compares before mutable component access, the presentation command fence, the public style-value convention, and the behavior/preset boundary; `SliderPreset` reuses them, follows the convention where slider structure permits, and does not reopen the material surface.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: the thumb tracks the applied value in all four directions; first override insertion reaches dispatch through `WidgetSystems::PresentationCommandsApplied`; a value change dirties or re-keys only the expected visual slots and causes no relayout; repeated value/state leaves override change ticks and retained uploads untouched; preset builds under the behavior/preset boundary test. If resizable fill is approved, add equivalent fill geometry and renderer tests. Extend and smoke-test the public path in `examples/widgets.rs` while preserving all existing diagnostic and input paths and expanding the measured diagnostic readout.
...
- Carry the selected policy into Phase 11 replacement/reset and preset tests so rebuilt tooltips never inherit stale picking state.
...
- **Eligibility and ordering:** read target `PickingInteraction` plus `WidgetFocused` where applicable. Run after Phase 7.5's `WidgetSystems::FocusCommandsApplied` and Phase 10's `TooltipSystems::ControllerCommandsApplied`, so same-frame interactivity, focus-marker commands, and newly reified controllers are all visible. `TooltipDisabledPolicy::Suppress` prevents or immediately ends visibility; `Show` leaves hover-or-focus eligibility unchanged.
...
- **Default presentation:** `widgets/presets/tooltip.rs` supplies `TooltipPreset` / `TooltipStyle` as a static layout-authoring helper that builds an ordinary `LayoutTree` and `TooltipTemplate`. It follows Phase 7.5's public style-value convention for construction, but does not author changing visual slots or write `WidgetVisualOverrides`: tooltip visibility is on the whole panel, and the preset has no independent hover/press/focus state. Rich content remains ordinary panel content.
...
- `src/widgets/presets/mod.rs` — tooltip preset module declaration and export
...
- `src/lib.rs` — curated tooltip preset exports
- `crates/hana_diegetic/examples/widgets.rs` — canonical replacement, retargeting, and preset exercise

**Constraints from prior phases:** Phase 10 supplies stable controller identity, template diff inputs, semantic relationships, panel-role ownership, and the settled placement builders/defaults. Phase 10.5 supplies the sole visibility state machine, hidden materialization path, readiness gates, duplicate-safe finalizer, persistent controller/panel/layout/placement contract, and settled tooltip picking policy; hidden GPU rows may retire and rebuild through normal renderer paths. Phase 3 supplies the complete panel-role teardown contract; Phases 4/4.5 supply typed same-space attachment, demand, and detach-before-convert behavior; Phase 6.5 supplies absent-only `PanelPicking` installation and ownership-aware teardown. Phase 7.5 supplies the public preset style-value convention, but `TooltipPreset` is static layout/template authoring and does not use the widget presentation writer. Replacement and retargeting must call the existing lifecycle, attachment, picking, and renderer paths rather than creating parallel authorities.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: a new blueprint completes role-removal teardown before installing the replacement, resets every old panel/widget/text/render/placement/picking component, rebuilds hidden on the same controller, and receives a fresh Hana-owned copy of the settled tooltip `PanelPicking`; an identical clone does nothing; policy-only and placement-only changes do not rebuild; placement-only replacement updates the checked attachment after its fence; same-space/same-unit retargeting updates ownership, inherited context, and attachment in place; cross-space or different-unit retargeting emits one hidden event if visible, detaches, and rematerializes the same controller without attempting an attached conversion; reveal waits for fresh layout and placement; repeated equal replacement or retargeting is a no-op; `TooltipPreset` builds an ordinary static layout/template and passes the behavior/preset boundary test without `WidgetVisualOverrides`. Extend and smoke-test the public path in `examples/widgets.rs` while preserving all existing diagnostic and input paths and expanding the measured readout instead of clipping it.
