...
- **State composition:** start from the authored normal values, then layer focused → hovered → pressed → disabled overrides per property; a missing value leaves the previous layer intact. This lets focus change only the border while hover changes only the background. Presentation reads `PickingInteraction`, `Has<WidgetDisabled>`, `Has<WidgetFocused>`, and the private `ButtonPress` marker. It never reads or mutates `ButtonCaptures`, which remains lifecycle and ordering authority.
...
**Implications for remaining phases:**
- Phase 9 should reuse the same change gate and focus → presentation → dispatch fence chain for slider state presentation.
...
**Pending decision: slider axis and thumb-travel formulas**

Actual problem:
Phase 8.5 maps a panel-local pointer position to a value, while Phase 9 maps that value back to a thumb offset. Neither Work Order fixes the axis formulas, zero-extent behavior, usable thumb travel, or what the authored thumb position represents, so the two implementations can be individually reasonable but visibly disagree.

What exists now:
- `SliderDirection` supplies left-to-right, right-to-left, bottom-to-top, and top-to-bottom, and `SliderState` owns the normalized applied value.
- Phase 8.5 projects the captured pointer into the slider root's panel-local rect; Phase 9 has the slider-root and thumb rects plus an authored thumb position.

What should change:
- Freeze the horizontal/vertical normalization and inversion formulas, the result when the active axis has zero extent, and the exact root/thumb geometry used for visual travel.
- State whether the authored thumb baseline represents the range start or an arbitrary visual origin, and make the pointer and presentation acceptance tests share the same normalized endpoint table.

Recommendation:
Normalize pointer input over the slider root's full active-axis extent, clamp to `[0, 1]`, invert for right-to-left and top-to-bottom, and treat a zero extent as `ProjectionUnavailable` with no proposal. Treat the authored thumb position as the range-start baseline; translate by normalized value times `max(root extent - thumb extent, 0)` along the direction, leaving the cross axis unchanged. Put the pointer formula in Phase 8.5 and the matching thumb formula plus endpoint table in Phase 9.
...
- **Value presentation:** the marked thumb's authored position is the zero-offset baseline. Map Phase 8's applied value through `SliderDirection`, then write only a panel-local XY translation for its private retained slot. Respect all four directions, preserve authored z/depth and hit geometry, and perform no `LayoutTree` or `ComputedDiegeticPanel` regeneration per value change.
