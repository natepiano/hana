...
**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: same-key overrides dirty only the referenced row; z/material/texture incompatibility relocates the record to the correct existing or newly created batch; the stable slot resolves the destination afterward; the old record and empty batch are retired; every newly created batch carries `Pickable::IGNORE`; repeated identical overrides are no-ops; unrelated slots remain untouched; override-only changes do not fire `Changed<DiegeticPanel>` or `Changed<ComputedDiegeticPanel>`, solve geometry, or alter widget transforms, hit rectangles, interaction rank, or cross-widget picking order; slot updates retain an unchanged button's callback system entity. Smoke the canonical example with a test-only/internal slot exercise without adding a public preset API.
...
**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: hover/press/focus and panel/global disabled edges patch only expected retained slots; pressed presentation reads `ButtonPress` and never reads or mutates `ButtonCaptures`; current-frame pointer/app/traversal focus is visible after `WidgetSystems::FocusCommandsApplied`; a widget-local disabled edit performs exactly its Phase 2 visual-only computed refresh and no layout solve, then presentation causes no second panel/computed change; repeated state is a no-op; unrelated slots remain untouched; `.on_click` fires once and retains the same tracked system through presentation-state changes; the behavior-module boundary test passes. Extend and smoke-test the public path in `examples/widgets.rs` while preserving and expanding the existing diagnostic readout.
...
**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: thumb/fill records track the applied value in all four directions; a value change dirties or re-keys only the expected visual slots and causes no relayout; preset builds under the behavior/preset boundary test. Extend and smoke-test the public path in `examples/widgets.rs` while preserving all existing diagnostic and input paths and expanding the measured readout instead of clipping it.
