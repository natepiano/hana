...
- Phase 10 and Phase 10.5 retain their existing pre-dispatch tooltip decisions. Phase 12 remains an intentional discussion checkpoint rather than a delegate-ready implementation phase.
...
- Phase 10 now names the computed-panel storage and cached-geometry path required for associated tooltip records; its standalone-target decision no longer promises a synchronous `Commands` query, and inferred panel construction is an explicit owner decision.
- Phase 10.5's existing picking-policy decision now also requires semantic non-interactivity, not only pointer pass-through; its proposed materialization/visibility split remains deferred at that phase gate.
...
- Phase 10's stale standalone-target decision now reflects Bevy's automatic removal of invalid relationships and recommends cleaning up the unassociated controller instead of promising an inert `TooltipFor`.
- Phase 10.5 now reveals before retained batching, emits `TooltipShown` after transform propagation, and proves final world and screen retained-record transforms.
...
- All remaining phases are still required; the seven existing tooltip decisions remain deferred to their affected phases.
...
- **Computed storage and tree diff:** carry associated `ComputedTooltipRecord`s through `ComputedDiegeticPanel` beside computed widgets and populate them from both full layout and cached-geometry refreshes in `compute_panel_layouts`. A tooltip declaration add/remove/replacement is `VisualOnly`; an identical template clone remains `Identical`. Controller indexes still follow source revision when an identical `set_tree` advances the source tree without changing computed output.
...
**Pending decision: inferred tooltip panel construction**

Actual problem:
`TooltipTemplate::new(LayoutTree)` intentionally names only the content tree because the tooltip's panel nature is inferred. Materialization still needs concrete panel sizing and inherited presentation context, and the plan does not yet freeze those defaults.

What exists now:
- The template stores an immutable application-authored tree plus timing, disabled, and pending placement policy.
- Phase 4.5 can copy the target panel's coordinate space, layout unit, window, camera order, and render layers before creating a checked same-space attachment.

What should change:
- Freeze the panel builder values derived from the target and the size derived from the template without adding `Panel` to the public template name.
- Keep the picking seed governed by Phase 10.5's separate non-interactive picking decision.

Recommendation:
Materialize the template as `Fit × Fit`; inherit the target panel's layout unit and complete world/screen presentation context; use the template tree unchanged; and let Phase 10.5 install the settled tooltip picking policy. Add no size fields until real tooltip use demonstrates a need beyond ordinary tree sizing.
...
- Bevy's relationship insertion hook warns and removes `TooltipFor` when its target entity no longer exists; it cannot leave the controller carrying an inert relationship to that stale target.

What should change:
- Decide whether ordinary standalone construction accepts only typed `PanelEntity<Space>` / `WidgetEntity<Space>` handles, uses a query-backed `SystemParam` for a synchronous live-target check, or retains reader-then-spawn authoring with a deferred stale-target warning.
- Do not promise that plain `Commands` can synchronously inspect an arbitrary `Entity`; it cannot query the live world before its deferred spawn applies.
- If typed-handle insertion loses a race with target removal, choose whether the controller is despawned when Bevy removes `TooltipFor` or stores a separate pending target outside the relationship until retargeted.
- Fold the chosen behavior into controller ownership, cleanup, and exact acceptance tests.

Recommendation:
Use typed panel/widget handles in `TooltipFor::new(...)` and `retargeted(...)`, resolving authored `(panel, id)` through `PanelWidgetReader::typed_entity` first. If the target disappears before deferred insertion, let Bevy remove the invalid relationship and despawn the now-unassociated controller; do not add separate pending-target storage. This prevents unrelated raw entities at compile time and keeps stale-target cleanup aligned with the relationship lifecycle.
...
- If v1 is non-interactive, define how widget declarations inside the application-authored tooltip tree are prevented from reifying focusable/activatable widget entities; `PASS_THROUGH` alone affects only pointer hits.
- Carry the selected policy into Phase 11 replacement/reset tests so rebuilt tooltips never inherit stale picking state.

Recommendation:
Make first-version materialized tooltips `PanelPicking::PASS_THROUGH` on both faces and mark their panel role so widget reify does not create semantic widget entities from declarations inside the tooltip tree. The tree still supplies ordinary visual content, but cannot enter focus traversal, activation, or capture. Treat interactive tooltip content as a separate later design using `WidgetsOnly` plus tooltip-hover eligibility.
...
- **Two-stage reveal:** after world `AnchorSystems::Resolve` or screen placement readiness, a pre-batch system transitions the ready controller to `TooltipPhase::Visible` and writes visible `Visibility` before `MaterialTableAppendReady`, so the retained SDF, image, shape, and text routes create current records in that frame. A second system after `TransformSystems::Propagate` emits `TooltipShown`, so observers read the final `GlobalTransform`. Materialization and checked attachment remain in their earlier scheduled stages. If the pending Phase 10.5/10.75 split is approved, the visibility phase owns both reveal stages.
...
- `src/render/mod.rs`, `src/render/material_table.rs`, `src/render/fill_batch.rs` — expose the pre-batch reveal boundary and prove final retained transforms
...
**Pending decision: policy replacement during an active tooltip timer**

Actual problem:
Phase 11 says a policy-only template replacement updates current and future timers, but does not say what happens to elapsed time while a tooltip is already waiting to show or hide.

What exists now:
- Phase 10.5's single state machine owns `WaitingToShow(Timer)` and `WaitingToHide(Timer)`.
- Replacing `disabled_policy` with `Suppress` while visible already needs an immediate hidden transition.

What should change:
- Define whether a changed `show_after` or `hide_after` restarts the wait, preserves elapsed eligibility time, or completes immediately when the new duration is shorter than time already elapsed.
- Pin waiting-to-show, waiting-to-hide, and visible-to-`Suppress` cases.

Recommendation:
Preserve elapsed eligibility time. Recompute the remaining delay from the original wait start, complete the transition immediately when the new duration is already satisfied, and hide immediately when a replacement changes a visible tooltip to `Suppress`. This makes live policy editing adjust the deadline instead of silently restarting user intent.
