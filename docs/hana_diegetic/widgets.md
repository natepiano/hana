...
**Pending decision: pointer focus from non-window render targets**
...
Recommendation:
Allow automatic pointer focus only when the picked camera resolves to a window; do not invent a global or primary-window fallback for image-rendered panels.
...
- **Panel-role teardown order:** register button capture finalization in the central lifecycle before Phase 4.5's combined `finalize_widget_anchor_state` cleanup and owned-widget despawn. Emit exactly one terminal event while `WidgetOf` and both world and screen attachment relations remain queryable; anchor cleanup and recursive owned-entity removal happen only afterward.
...
- **Panel-role teardown order:** register slider capture finalization in the central lifecycle before Phase 4.5's combined `finalize_widget_anchor_state` cleanup and owned-widget despawn. Emit one terminal event while `WidgetOf` and both world and screen attachment relations remain queryable, then allow anchor and owned-entity cleanup to proceed.
...
- **Lifecycle, demand, and retargeting:** pre-materialized `TooltipFor` membership is not anchor demand. Materialization inserts the world/screen placement relationship and starts demand; hiding retains it. A same-space, same-layout-unit target change updates `TooltipFor`, `PanelOwned`, and the checked placement target on the same materialized panel. A coordinate-space or layout-unit change first hides, detaches, waits for the attachment command fence, resets the materialized panel state, and rebuilds on the same controller entity with the new target context; conversion is never attempted while attached. World `AnchoredHere` and Phase 4.5's screen reverse relationship keep geometry resident only while placement needs it. There is no inactivity eviction. Before Phase 4.5's combined `finalize_widget_anchor_state` cleanup or Phase 3 owned-entity despawn runs because a target panel role ended, invoke the same visibility finalizer used by other removal paths so a visible controller emits one `TooltipHidden` while its template, transform, `TooltipFor`, and world or screen placement relation remain queryable.
...
**Constraints from prior phases:** All widget subsystems through Phase 10.5 complete and are tested through deterministic minimal-app coverage before demonstration work begins; the canonical example already retains both world- and screen-widget anchoring paths through the same-space public API, and conversion tests demonstrate detach → convert → reattach rather than cross-space fallback.
