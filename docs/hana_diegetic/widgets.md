...
- **Test:** `cargo nextest run -p hana_diegetic` (never `cargo test`). If another package's behavior changes, add only that exact package to the test command.
- **Lint:** the `clippy` skill with `auto-proceed`, scoped to the exact changed package or packages rather than the whole workspace. Workspace lints are strict: `all`/`cargo`/`nursery`/`pedantic` denied, `unwrap_used`/`expect_used`/`panic`/`unreachable` denied, `missing_docs = "deny"`, `self_named_module_files` denied (use `module/mod.rs` directory form).
...
- Phase 7.5 now keeps presentation-only z/transform patches inside the owning widget's retained records and explicitly preserves Phase 3 hit bounds and cross-widget order.
...
**Acceptance gate:** `cargo nextest run -p hana_diegetic` green with new tests: `.on_click` receives both a pointer-originated `ButtonClicked` carrying `Some(pointer_id)` and a semantic `ButtonClicked` carrying `None`; exactly one plugin observer dispatches callbacks and no widget owns an observer; reify reuse does not re-register; callback replacement during a live press releases only the prior tracked handle, installs exactly one replacement, and leaves the press intact; widget removal releases the final tracked callback; global observation reads the event target directly, and an entity-scoped observer installed on a previously reader-resolved widget receives the click without re-resolving it. Extend and smoke-test the public path in `examples/widgets.rs` while preserving and expanding the existing diagnostic readout.

### Phase 7.25 — Retained widget visual slots and batch re-keying  · status: todo
...
- **Four renderer entry points:** route overrides through the existing record writers in `render/fill_batch.rs`, `render/image_batch.rs`, `render/panel_text/batching.rs`, and `render/panel_shapes/batching.rs`, with common retained-batch ownership/retirement in `render/batch_store.rs`. Same-key color or transform changes patch only the referenced record and dirty GPU row; unrelated records and batches remain untouched.
- **Re-keying is required:** `SdfBatchKey` and `ImageBatchKey` include z/material compatibility or texture facts. When an override changes one of those key facts, remove the old record, insert it into the compatible destination batch, create that batch through the existing renderer spawn path when absent, and retire an empty old batch. A moved slot keeps its stable identity and future overrides resolve its new record location.
...
- **Interaction boundary:** visual-slot z/transform overrides layer and move presentation records only inside their owning widget. They never alter the widget `Transform`, `rect`, `clipped_rect`, or interaction rank from Phase 3 and cannot lift one widget over another or expand its hit bounds. Cross-widget depth and hit-bound changes require authoritative tree/layout authoring and are outside preset state presentation.
...
**Constraints from prior phases:** Phase 6.5 made every initial fill/image/text/shape batch entity `Pickable::IGNORE`; destination batches created during re-keying preserve that invariant. Phase 3 supplies deterministic widget interaction rank and rectangles, which visual overrides never mutate. Phase 2 guarantees interactivity-only edits skip geometry solving. This phase builds only private retained-render infrastructure; it does not define public preset APIs or behavior-state mappings.

**Acceptance gate:** `cargo nextest run -p hana_diegetic` green with new tests: same-key overrides dirty only the referenced row; z/material/texture incompatibility relocates the record to the correct existing or newly created batch; the stable slot resolves the destination afterward; the old record and empty batch are retired; every newly created batch carries `Pickable::IGNORE`; repeated identical overrides are no-ops; unrelated slots remain untouched; override-only changes do not fire `Changed<DiegeticPanel>` or `Changed<ComputedDiegeticPanel>`, solve geometry, or alter widget transforms, hit rectangles, interaction rank, or cross-widget picking order. Smoke the canonical example with a test-only/internal slot exercise without adding a public preset API.
...
**Constraints from prior phases:** Phase 7.25 supplies stable slots, changed-only override writes, batch re-keying, and stock-picker exclusion for newly created batches. Phase 6 supplies the private `ButtonPress` presentation marker and keeps `ButtonCaptures` private lifecycle authority. Phase 6.5 supplies the final per-face hit stream. Phase 3 supplies `PickingInteraction`; Phase 5 supplies `WidgetFocused` and `WidgetSystems::SemanticInput`. Phase 5.5's built-in adapter and app-owned example path both send the same Phase 5 messages before semantic input, so one `WidgetSystems::FocusCommandsApplied` fence covers either origin; do not add an adapter-specific fence.

**Acceptance gate:** `cargo nextest run -p hana_diegetic` green with new tests: hover/press/focus and panel/global disabled edges patch only expected retained slots; pressed presentation reads `ButtonPress` and never reads or mutates `ButtonCaptures`; current-frame pointer/app/traversal focus is visible after `WidgetSystems::FocusCommandsApplied`; a widget-local disabled edit performs exactly its Phase 2 visual-only computed refresh and no layout solve, then presentation causes no second panel/computed change; repeated state is a no-op; unrelated slots remain untouched; the behavior-module boundary test passes. Extend and smoke-test the public path in `examples/widgets.rs` while preserving and expanding the existing diagnostic readout.
...
- `src/widgets/mod.rs` — slider request and optional self-update registration
...
**Constraints from prior phases:** Phase 6 is the behavioral baseline and its public API must remain unchanged. Phase 6.5 supplies the final hit stream but does not participate in capture. Phase 5 supplies the paired component-removal/full-despawn finalization timing. This phase adds no slider lifecycle yet and no public type.

**Acceptance gate:** `cargo nextest run -p hana_diegetic` green with every Phase 6 button regression retained plus focused cross-authority tests: one pointer/one widget occupancy, two pointers competing for one button, release or cancel freeing both indexes before a same-batch new press, stale entities retiring without leaks, and checked sequence exhaustion preserving the prior owner. The public button event sequence and payloads remain byte-for-byte equivalent, and the canonical widget example's pointer/focus/button readout behaves unchanged in a live smoke.
...
- Thumb/fill placement reads Phase 8's applied value. Restyle and thumb movement write Phase 7.25 visual-slot overrides, patching or re-keying only retained render records — no `LayoutTree` or computed-panel regeneration per value change.
...
**Constraints from prior phases:** Phase 8 supplies `SliderDirection` and the applied-value contract; Phase 8.5 supplies the completed pointer lifecycle. Phase 7.25 supplies stable visual slots and batch re-keying. Phase 7.5 settles the public style-value convention and behavior/preset boundary; `SliderPreset` follows that convention where slider structure permits and does not reopen the material surface.

**Acceptance gate:** `cargo nextest run -p hana_diegetic` green with new tests: thumb/fill records track the applied value in all four directions; a value change dirties or re-keys only the expected visual slots and causes no relayout; preset builds under the behavior/preset boundary test. Extend and smoke-test the public path in `examples/widgets.rs` while preserving all existing diagnostic and input paths and expanding the measured readout instead of clipping it.
...
- **Default presentation:** `widgets/presets/tooltip.rs` supplies `TooltipPreset` / `TooltipStyle` through Phase 7.25's retained visual-slot boundary and follows the public style-value convention settled in Phase 7.5. Rich content remains ordinary panel content; the preset does not add behavior authority or mutate layout in response to hover/focus state.
...
**Constraints from prior phases:** Phase 10 supplies stable controller identity, template diff inputs, semantic relationships, panel-role ownership, and the settled placement builders/defaults. Phase 10.5 supplies the sole visibility state machine, hidden materialization path, readiness gates, duplicate-safe finalizer, resident hidden-panel contract, and settled tooltip picking policy. Phase 3 supplies the complete panel-role teardown contract; Phases 4/4.5 supply typed same-space attachment, demand, and detach-before-convert behavior; Phase 6.5 supplies absent-only `PanelPicking` installation and ownership-aware teardown; Phase 7.25 supplies retained visual slots, and Phase 7.5 settles their public preset style convention. Replacement and retargeting must call those existing paths rather than creating parallel lifecycle, attachment, picking, or presentation authorities.

**Acceptance gate:** `cargo nextest run -p hana_diegetic` green with new tests: a new blueprint completes role-removal teardown before installing the replacement, resets every old panel/widget/text/render/placement/picking component, rebuilds hidden on the same controller, and receives a fresh Hana-owned copy of the settled tooltip `PanelPicking`; an identical clone does nothing; policy-only and placement-only changes do not rebuild; placement-only replacement updates the checked attachment after its fence; same-space/same-unit retargeting updates ownership, inherited context, and attachment in place; cross-space or different-unit retargeting emits one hidden event if visible, detaches, and rematerializes the same controller without attempting an attached conversion; reveal waits for fresh layout and placement; repeated equal replacement or retargeting is a no-op; `TooltipPreset` patches only its retained visual slots and passes the behavior/preset boundary test. Extend and smoke-test the public path in `examples/widgets.rs` while preserving all existing diagnostic and input paths and expanding the measured readout instead of clipping it.
