...
- **Layout** (only phase-touched paths):
  ...
  - `crates/hana_diegetic/src/lib.rs` — curated public re-exports plus the public `HeadlessDiegeticUiPlugin` behavioral composition used by client tests.
- **Key files:**
  ...
  - `src/widgets/focus.rs` + `src/widgets/input.rs` — shipped per-window focus authority, public focus events, one ordered window-scoped `WidgetInput` message stream, private resolved widget intent, IME/disabled gating, and traversal over current computed preorder.
...
- **Canonical input paths:** later edits to `examples/widgets.rs` preserve both Phase 5.5 integration proofs: `hana_diegetic`'s built-in per-window `WidgetInputPlugin` bindings and the app-owned Bevy Kana action that sends the same Phase 5 message. Later behavior may extend either path but must not replace one with a second widget API.
...
- **Invariants:**
  ...
  - `HeadlessDiegeticUiPlugin` composes `HeadlessLayoutPlugin`, the private `WidgetsPlugin`, and the private `ImePlugin` without registering shaders, render assets, gizmos, or Bevy's render sub-app. `DiegeticUiPlugin` installs this same behavioral composition before adding text, screen-space, gizmo, and renderer integration. Future widget behavior registers through the private widget/IME plugins shared by both public entry points; renderer extraction and batching remain outside the headless composition. `WidgetInputPlugin` remains opt-in.
  ...
  - Tooltip authoring is separate from `Button`/`Slider` authoring. Reify creates a lightweight tooltip entity with `TooltipFor(target)`; first eligibility materializes that same entity into a hidden anchored panel. The semantic relationship exists before the placement relationship and does not itself create anchor-geometry demand. When the target is a panel or widget, the controller also records that owning panel role privately so component-only panel teardown cannot strand it; a general target has no invented panel owner.
- **Public contract ledger (fixed before delegation except where a phase-local pending-decision block names the unresolved surface):**
  - Plugin exports are `HeadlessLayoutPlugin`, `HeadlessDiegeticUiPlugin`, `DiegeticUiPlugin`, and the opt-in `WidgetInputPlugin`. `HeadlessLayoutPlugin` remains layout-only for benchmarks. `HeadlessDiegeticUiPlugin` adds widget, tooltip, focus, slider, and IME behavior without rendering so downstream clients can run their own deterministic tests. `DiegeticUiPlugin` composes that same behavior before installing renderer-facing plugins; private `WidgetsPlugin` and `ImePlugin` are not exposed as assembly pieces.
  - Phase 10 changes `El` to `El<L = Row, Role = LayoutOnly>` with public zero-sized marker roles `LayoutOnly` and `WidgetElement`. Authoring methods on `El<L, LayoutOnly>` are `button(self, id, Button) -> El<L, WidgetElement>` and `slider(self, id, Slider) -> El<L, WidgetElement>`; both assign the element id and crate-private widget variant atomically. Common layout/presentation builders and ordinary `LayoutBuilder` accept either role, while `.tooltip(...)` exists only on `El<L, WidgetElement>`. Tooltip layout authoring accepts only `LayoutOnly`. The named markers add no runtime storage and keep signatures and diagnostics self-documenting. `Button` is a private-field `Clone + Debug + PartialEq + Default` authoring builder with `new()` and Phase 7's `on_click(...)`; `Slider` is a private-field `Clone + Debug + PartialEq` validated authoring builder with no `Default` because range and initial value are required. Neither is an ECS component. Crate-private `WidgetSpec` is exactly `Button(Button) | Slider(Slider)`, while runtime slider data lives in `SliderState`, so no public `Spec` suffix is needed.
  ...
  - Interactivity/focus exports are `WidgetInteractivity`, `WidgetDisabled`, `PanelWidgetWriter`, `WidgetFocusable`, `WidgetFocused`, `RequestPanelFocus`, `RequestWidgetFocus`, `ClearWidgetFocus`, `WidgetFocusChanged`, and `WidgetFocusChangeCause`. Element authoring is `El::widget_interactivity(self, value: WidgetInteractivity) -> Self`. `PanelWidgetWriter::override_interactivity(widget, value)` and `inherit_interactivity(widget)` update the authoritative widget `El` by following `PanelWidget` + `WidgetOf`; an event target can be passed directly, while `(panel, id)` callers resolve it through `PanelWidgetReader`. The existing `CascadeEntityCommandsExt` gains `override_widget_interactivity(value)` and `inherit_widget_interactivity()` for panels and other ECS-authored cascade ancestors; raw `Cascade<T>` remains owned by `bevy_kana` and is not re-exported. The panel-focus request payload is `{ window, panel }`, the widget-focus request payload is `{ window, widget }`, clear is `{ window }`, and change is `{ window, previous, current, cause }`. `RequestPanelFocus` selects that panel for the window without focusing a widget, so the next traversal starts at the selected panel's first or last focusable widget. This gives applications with multiple panels an explicit choice instead of deriving keyboard ownership from spawn order. Cause variants are `Pointer`, `Traversal`, `Semantic`, `Application`, `ExplicitClear`, `WidgetRemoved`, `FocusabilityRemoved`, and `ScopeLost`; disable is intentionally absent. Pointer presses retain semantic `WidgetFocused` state but hide the private focus indicator; traversal and direct application requests show it. No additional public focus-visible type is introduced.
  - The exported `WidgetInput` message has `FocusNext`, `FocusPrevious`, `FocusFirst`, `FocusLast`, `Activate`, and `Cancel` variants; each carries `{ window: Entity }`. One message queue preserves write order between different operations. Phase 5 resolves the named window through private focus authority, applies IME and disabled gating, and emits one private entity-targeted `SemanticWidgetIntent`. The optional adapter exports `WidgetInputPlugin`, `WidgetInputMode`, `WidgetInputBindings`, `WidgetInputBindingsBuilder`, `WidgetInputBindingsError`, `WidgetInputDisabled`, and `WidgetControlSummary`. `WidgetInputMode` is configured independently on each `Window`: `Default` uses Tab / Shift+Tab / Enter or Space / Escape, `Bindings(WidgetInputBindings)` uses that window's custom bindings, and `Manual` creates no adapter-owned input context so the application can use its own Bevy Enhanced Input actions and Bevy Kana macros to write `WidgetInput`. Home and End remain available through custom `first` and `last` bindings but are not global defaults because the focused widget may own them. `WidgetInputBindings` is reflected, replaceable authored data and is the adapter-facing value a future keymap may produce; enhanced-input action, modifier, and trigger entities remain private runtime installation. `WidgetInputBindings::builder()` returns the public builder; its six fluent action methods each accept `impl Into<Binding>` and add alternatives, while `build()` deduplicates same-action repeats and returns `WidgetInputBindingsError::NoneBinding` or `ConflictingBinding(Binding)` for invalid input. Adding `WidgetInputPlugin` automatically gives every existing and newly created window `WidgetInputMode::Default` unless that window already specifies another mode. Keyboard and built-in gamepad actions run only for the operating-system-focused window; if there is not exactly one focused live window, built-in gamepad input emits no widget action. Other windows retain their remembered widget focus without responding. Adding or removing `WidgetInputDisabled` temporarily tears down or restores only that window's adapter-owned input entities without changing its selected mode or remembered widget focus. `WidgetInputMode::control_summary()` returns a display-only `WidgetControlSummary` with `next`, `previous`, `first`, `last`, `activate`, and `cancel` label lists; it is not runtime input state, and `Manual` produces empty lists.
  ...
  - Slider exports are `Slider`, `SliderState`, `SliderRange`, `SliderStep`, `SliderDirection`, `SliderConfigError`, `SliderGrabbed`, `SliderChangeRequested`, `SliderReleased`, `SliderCanceled`, `SliderCancelCause`, `RequestSliderAdjustment`, `SliderAdjustment`, and `slider_self_update`. Phase 10 appends `Tooltip`, `TooltipFor`, `Tooltips`, `TooltipDisabledPolicy`, `TooltipTarget`, `TooltipTargetEntity`, `MeshFace`, the mesh-anchor command extension, and `TooltipPlacementPolicy`; Phase 10.75 appends `TooltipShown` and `TooltipHidden`. Preset and theme APIs are deferred to [`widgets-deferred.md`](widgets-deferred.md).
  - `Tooltip` is the public authored value even though its `LayoutTree` blueprint is stored behind an `Arc` and panel creation is deferred. This matches the public `Button` and `Slider` names: lifecycle and storage do not add a `Template` or `Spec` suffix. Associated authoring is `.button(...)` or `.slider(...)` followed by `.tooltip(tooltip)`. A const widget typestate on `El` makes `.tooltip(...)` unavailable before the element becomes a widget, so there is no tooltip-specific panel-build error. Standalone authoring uses `TooltipCommandsExt::spawn_tooltip(target: impl TooltipTarget, tooltip) -> Entity` rather than inserting a raw `(Tooltip, TooltipFor)` tuple or accepting raw `Entity`. `PanelEntity<Space>`, `WidgetEntity<Space>`, the general `TooltipTargetEntity<Space>`, and application-owned typed handles may implement `TooltipTarget`; its associated space prevents a world target from being treated as a screen target. `TooltipFor` remains publicly queryable, `Tooltips::iter()` exposes reverse membership, and `Tooltips` uses `linked_spawn` to despawn related tooltip controllers with their target.
...
- **Authoring builders** (`widgets/button.rs` + `widgets/slider.rs`): create `Button` as the private-field `Clone + Debug + PartialEq + Default` builder with `new()`. Create the Phase 1 portion of the approved slider contract: `SliderDirection::{LeftToRight, RightToLeft, BottomToTop, TopToBottom}`; `SliderRange::new(start, end) -> Result<SliderRange, SliderConfigError>` for finite strictly ordered endpoints; `SliderStep::new(step) -> Result<SliderStep, SliderConfigError>` for finite positive steps; and the infallible private-field `Slider` builder `Slider::new(range)` with `value`, `step`, and `direction`, whose authored numbers are validated at panel build. `SliderConfigError` is exactly `NonFiniteRange | UnorderedRange | NonFiniteValue | NonPositiveStep`, derives `thiserror::Error`, and has exhaustive stable-message tests. These are authoring/configuration types, not ECS components; Phase 8 adds `SliderState`, requests, events, and runtime behavior.
...
- Traversal order is the current `ComputedWidgetRecord` preorder rebuilt in Phase 1, never `PanelWidgets` relationship insertion order. Next/previous/first/last stay within the active panel for that window and wrap deterministically; focusing a widget on another panel transfers the active panel. When no widget is focused, an application selects one of its live panels with `RequestPanelFocus { window, panel }`; the next traversal then starts at the appropriate end of that panel. Hana does not guess between multiple panels from entity or spawn order. Structural reorder changes traversal without respawning entities.
- Define `WidgetInput` with next, previous, first, last, activate, and cancel variants. Core focus requests/events and their observable routing do not depend on a binding library; Phase 5.5 only translates enhanced-input action edges into this core path.
...
Actual problem:
Given the approved per-window focus model, the public ledger must state whether input operations are binding-library action markers, app-sendable window-scoped requests, or entity-targeted events. Without that contract, Phase 5 cannot prove how activation reaches the currently focused widget, and Phase 5.5 cannot be a replaceable adapter.
...
**Goal:** An opt-in `bevy_enhanced_input` adapter gives each window an independent binding mode while feeding only Phase 5's core semantic path. Applications may instead keep their own enhanced-input contexts and Bevy Kana action macros and write the same `WidgetInput` message directly.
...
- Core focus and `WidgetInput` routing work without the adapter.
...
Decision:
Send built-in gamepad actions to the operating-system-focused window, matching keyboard behavior. If there is not exactly one focused live window, emit no widget action because the adapter cannot choose safely. An application that wants a different rule can use `WidgetInputMode::Manual` and write `WidgetInput` with its own selected window.
...
- `WidgetInputBindings` already stores native `bevy_enhanced_input::Binding` values, derives `Reflect`, and can be replaced per window without changing Phase 5's `WidgetInput` contract.
...
- The canonical example proves both paths: it explicitly selects the Widget Lab panel for initial keyboard traversal even though another widget panel exists, Hana's Tab controls move focus within that panel, and an app-owned Bevy Kana `P` action moves the same per-window widget focus.
...
- Phase 7.5 and Phase 10.75 now state that `WidgetSystems::FocusCommandsApplied` already includes both built-in and app-owned message effects; no adapter-specific fence is added.
...
- Slider state/request behavior, shared capture ownership, projection, and pointer lifecycle are split across Phases 8, 8.25, 8.5, and 8.75. Phase 8.75 consumes the already-extracted one-pointer/one-widget authority, and its tests use Bevy's real dispatcher and raw-message timing.
...
- Direct button and slider builders reuse the same private slot/index path and existing material/texture re-keying. Tooltip presentation remains the application-authored `Tooltip` tree.
...
- Updated tooltip phases so hidden tooltips retain their controller, panel, layout, and placement state while renderer rows may retire and rebuild; applications supply the ordinary `Tooltip` tree.
...
**Spec:**
- **Direct public authoring:** the normal surface remains the existing `El::background`, `El::border`, and `El::material` declaration. The element that declares the widget carries the per-state builders `hovered_background`, `pressed_background`, `focused_background`, and `disabled_background`; the same four state prefixes apply to `*_border_color`, `*_border_width`, and `*_material`. They exist only after `.button`/`.slider`/`.widget`/`.editable_field` flips the element's role, so an ordinary element never offers them. `focused_*` means the keyboard focus indicator is visible, not merely that the button remains the semantic focus target after a pointer press. No public `ButtonPreset`, `ButtonStyle`, appearance component, state enum, theme token, or custom/extended-material surface is introduced. `Handle<StandardMaterial>` remains the material boundary.
- **Root-surface boundary:** these builders affect only the element carrying `El::button`; arbitrary child text, images, icons, shapes, content, and layout stay as authored. A state background requires an authored normal background record; a state border color or border width requires an authored normal border, whose radii remain fixed and whose widths change only where a state replaces them; and a state material requires an authored root surface. A state border width applies to all four sides and grows inward from the authored outer bounds, so it changes no solved layout; a borderless-looking button opts in by authoring a zero-width border, which emits the record a focus width can widen. Validate missing targets during panel construction and `set_tree` with stable `PanelBuildError` variants rather than silently ignoring the builder. State changes never add/remove render roles or change layout geometry.
...
- **Focus command fence:** add `WidgetSystems::FocusCommandsApplied`, an explicit `ApplyDeferred` after Phase 5's `WidgetSystems::SemanticInput`. Button state presentation runs after this fence, so pointer-driven indicator removal and application/traversal indicator insertion are visible in the same frame. Phase 10.75 reuses this scheduling point for tooltip focus eligibility.
...
**What deviated from the plan:**
- Per-state appearance lives in `StateAppearance` behind `Option<Box<_>>` on the element, so an element without state presentation does not carry the larger color/material block.
...
- Phase 10.75 can reuse `WidgetSystems::FocusCommandsApplied` for tooltip focus eligibility without another adapter-specific fence.
...
- Phase 8.5 now owns deterministic projection helpers and the exact axis/thumb formula; Phase 8.75 carries the deterministic Bevy pointer-dispatch coverage and slider lifecycle.
...
- Phase 10 now names the `El` typestate and checked standalone command explicitly; its general-target provider/return-value decision remains deferred to its pre-dispatch check.
- Phase 10.75 owns the post-transform reveal schedule and deterministic pointer path; the materialization/behavior split is accepted, while Phase 10.5's picking-policy decision remains deferred to its pre-dispatch check.
...
- **Shipped construction contract:** Phase 1 provides `SliderDirection::{LeftToRight, RightToLeft, BottomToTop, TopToBottom}`; finite strictly ordered `SliderRange`; finite positive `SliderStep`; the infallible `Slider::new(range)` plus `value`, `step`, and `direction` builders validated at panel build; and the `thiserror`-derived `SliderConfigError::{NonFiniteRange, UnorderedRange, NonFiniteValue, NonPositiveStep}` with stable-message tests.
...
**Implications for remaining phases:**
- Phase 8.5 projection returns raw target values; Phase 8.75 emits those values unchanged and leaves every snap-and-clamp decision to `SliderState::set_value`.
...
- Keep the shared authority crate-private. Do not expose a capture component, resource, trait, event, or generic public terminal payload. Phase 8.75 consumes the private operations from slider behavior.
...
**What worked:**
- Private `WidgetCaptures` now owns checked pointer/widget claims, attempted-press observations, and sequence exhaustion while `ButtonCaptures` retains only button ids and terminal outcomes.
- Every existing button lifecycle regression remained green, and the real example produced one semantic click and one callback after the extraction.

**What deviated from the plan:**
- `widgets/reify.rs` and the panel lifecycle modules needed no source edit: their existing queued `ButtonPress` removal runs its hook before kind replacement, despawn, or owned cleanup, and that hook now releases shared occupancy.
- The smallest authority uses one pointer-keyed ownership map and a reverse lookup rather than maintaining two synchronized maps.

**Surprises:**
- The first extraction left checked availability and ownership mutation as separate operations; review consolidated them into `WidgetCaptures::try_capture` so future slider code cannot bypass either exclusivity rule.
- A missing shared owner is unreachable through current production paths, but the button terminal hook now warns instead of silently discarding that inconsistency.

**Implications for remaining phases:**
- Phase 8.75 must claim through `WidgetCaptures::try_capture`, store slider-specific payloads separately, and release shared occupancy before a later same-batch action can claim either side.
- Slider lifecycle cleanup can follow the same marker-removal hook boundary without adding direct shared-capture mutation to reify or panel lifecycle systems.

### Phase 8.25 Review

- Phase 8.5 now receives the live Bevy `RenderTarget` component plus the resolved primary-window entity so it can compare the camera's current normalized target with the target captured at press time.
- Phase 8.5 no longer names `widgets/picking.rs`; it creates projection helpers only, and Phase 8.75 owns their first production consumer.
- Phase 8.75 now names `SliderDrag`/`SliderCaptures` as the slider-specific marker and payload, with the same terminal-before-removal hook boundary proven by buttons.
- Phase 8.75 now moves the button-only raw reconciler into one private cross-widget dispatcher so same-batch button/slider handoff has one ordering authority.

### Phase 8.5 — Captured-camera slider projection  · status: done (`bdca98af`)
...
- When the slider has a marked thumb, let `C` be the content-box extent and `T` the thumb extent on the active axis. The usable visual travel is `max(C - T, 0)`. The presentation writer solves the desired center along that travel, subtracts the thumb's solved authored center, changes no cross-axis position, and preserves authored depth. Ordinary authoring therefore does not have to place the thumb at the directed range start.
...
**Spec:**
- **Ray boundary:** add crate-private `project_flat_panel_ray_hit` beside `project_flat_panel_hit`. Its inputs are the live captured-camera entity's `Camera`, `GlobalTransform`, and separate `RenderTarget` component, the resolved primary-window entity when one exists, the captured `NormalizedRenderTarget`, current pointer viewport position, target `DiegeticPanel`, and target panel `GlobalTransform`; it returns panel-layout coordinates or a private typed projection failure. It normalizes the live `RenderTarget` with the primary-window entity and verifies that it still equals the captured target, calls `Camera::viewport_to_world`, intersects that ray with the panel's two-sided local z=0 plane, rejects non-invertible transforms, parallel rays, intersections behind the camera, invalid panel scale, unavailable cameras, and missing/unresolvable/mismatched render targets, then delegates world-hit conversion to the existing `project_flat_panel_hit`. Surface-panel integration later replaces this one flat boundary.
...
- **Sequencing with anatomy:** Phase 9 does not yet supply live thumb anatomy. This phase tests the value helper with explicit synthetic content bounds/thumb extents. Phase 8.75 records the live slider-root content box and uses the shipped headless path (`thumb_extent = None`); Phase 9 adds live thumb geometry and the thumb-aware integration coverage without replacing either helper.
...
**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with deterministic tests for perspective and orthographic captured cameras, secondary and primary-window render targets, current `RenderTarget` normalization, two-sided world panels, missing/unresolvable/mismatched targets, unavailable cameras, non-invertible transforms, parallel/behind-plane rays, and the same world hit mapping through `project_flat_panel_hit`. Pure value tests cover all four directions, outside clamping, finite raw output, border/padding-excluded synthetic content bounds, exact endpoint centers, headless full-content travel, zero headless extent, and `T >= C`; off-lattice targets remain raw rather than being snapped.

#### Retrospective

**What worked:**
- `project_flat_panel_ray_hit` now re-derives a live perspective or orthographic camera ray only while its normalized render target still matches the press-time target, then maps the two-sided plane hit through `project_flat_panel_hit`.
- `project_pointer_value` implements the approved four-direction thumb-center endpoint table and keeps its result raw for application-owned acceptance.

**What deviated from the plan:**
- No public or runtime-visible behavior was added, as planned; the canonical example therefore received a startup/first-frame regression smoke instead of a new interaction exercise.

**Surprises:**
- The first delegate stopped while its compile check was still running, so the main verification reran every package gate and found the unused test import.
- Review tightened both helpers so non-finite panel transforms, panel scales, active-axis geometry, and thumb extents return typed projection failure instead of leaking non-finite coordinates or widening the travel interval.

**Implications for remaining phases:**
- Phase 8.75 can consume `CapturedCameraRay`, `FlatPanelRayError`, `project_flat_panel_ray_hit`, and the headless `project_pointer_value(..., None, ...)` path directly; it must remove their temporary unused/dead-code expectations when they become production-used.
- Phase 9 must feed the solved slider-root content box and live thumb active-axis extent into the same value helper and reuse its endpoint table for thumb translation.

### Phase 8.5 Review

- Phase 8.75 now assigns the slider-root slot and carries its solved border/content bounds through reify before pointer capture consumes them. This makes the approved padding/border-excluded headless projection implementable without recomputing layout during a drag.
- A press is projected before it claims shared occupancy. Initial projection failure emits nothing; only projection loss during an already-active drag produces `SliderCanceled { cause: ProjectionUnavailable, .. }`.
- Phase 9 adds only the optional thumb slot and presentation behavior. Its offset is the desired directed center minus the thumb's solved authored center, which preserves exact endpoints for any ordinary authored placement and after direction changes.
- Phase 8.75 names the exact captured camera, render-target, panel-transform, and root-slot inputs and maps their post-capture failures to one typed cancel path.
- Phase 10's general-target contract and Phase 10.5's keep-visible policy and work-order split are settled; only the materialized-tooltip picking policy remains at that boundary. Phase 12 remains an intentional discussion checkpoint rather than a delegate-ready implementation phase.

### Phase 8.75 — Slider pointer lifecycle  · status: done (`f183ba7a`)
...
**Spec:**
- **Press, drag, and release:** first project the press through the live camera and slider-root content box. Only a successful projection is an accepted press: it then claims shared occupancy, stores `press.hit.camera`, `press.pointer_location.target` as the captured `NormalizedRenderTarget`, and the successfully projected raw target; emits `SliderGrabbed { entity, id, pointer_id }`; and emits one non-final `SliderChangeRequested` carrying that raw target. If the initial projection is unavailable, do not claim the pointer or widget and emit no grab, value, release, or cancel event. A click without movement still reprojects the release location and proposes that clicked value. Every drag reprojects that event's current pointer location through Phase 8.5, replaces the stored raw target, and emits one non-final proposal. A valid release reprojects the release location, terminalizes and frees shared occupancy, emits one final raw proposal, then emits `SliderReleased { entity, id, pointer_id }`; a later raw action in the same unread batch may claim the freed pointer or widget. If an already-active drag later cannot project a drag or release location, cancel it with `ProjectionUnavailable` and emit no value proposal for that failed location.
- **Live projection inputs:** on every press/drag/release projection, query the stored camera entity's live `Camera`, `GlobalTransform`, and optional separate `RenderTarget`; resolve the current primary-window entity when exactly one exists; combine those with the captured normalized target and event viewport position in Phase 8.5's private `CapturedCameraRay`; and query the owning `DiegeticPanel` plus its live `GlobalTransform`. Read the slider-root content box from its reified `VisualSlotId::SLIDER_ROOT` record and pass that box with `thumb_extent = None` to `project_pointer_value`. Map any `FlatPanelRayError` or `ProjectionUnavailable` after capture to `SliderCancelCause::ProjectionUnavailable`. Remove Phase 8.5's temporary unused/dead-code expectations once these helpers become production-used.
- **Live root geometry:** `El::slider` assigns crate-private `VisualSlotId::SLIDER_ROOT`. Extend `ComputedVisualSlot`, carried through `ComputedWidgetRecord` into `WidgetVisualSlots`, with the slot element's solved border-box and padding/border-excluded content-box bounds. Compute both once beside the layout result in `layout/element.rs` using the layout engine's existing `content_box` formula. Pointer projection reads this reified root record; it does not reconstruct padding, inspect retained render batches, or wait for Phase 9's optional thumb anatomy. A headless slider therefore has the exact live content box required by the approved projection contract.
...
- **Slider-specific payload and marker:** private `SliderCaptures` stores slider-only payloads keyed by widget entity: panel-local id, captured camera entity and normalized render target, latest raw projected target, and typed terminal outcome. Private `SliderDrag` is the live marker. Every release, cancel, disable, kind-change, removal, and panel-teardown path first writes the terminal outcome, then removes `SliderDrag`; its remove/despawn hook takes the slider payload, releases `WidgetCaptures`, and emits exactly one typed terminal event. Reify and panel lifecycle continue to request typed cancellation rather than mutating shared occupancy directly.
- **Shared raw dispatcher:** move the button-only raw `PointerInput` reader/reconciler out of `button.rs` into `capture.rs`. One private dispatcher owns terminal-before-later-press ordering for all captured widgets, queries the current `WidgetKind`, and calls private button or slider terminal/recapture helpers; it does not introduce a public trait, generic terminal event, or shared behavior payload. This one path must handle button → slider, slider → button, and same-kind handoff in a single unread raw batch while preserving the existing button event sequence and `ButtonPress` timing.
- **Lifecycle:** reuse Phase 6's checked raw-action ordering and terminal-before-marker-removal pattern through the shared dispatcher. `SliderCanceled { entity, id, pointer_id, cause }` uses `SliderCancelCause::{PointerCanceled, PointerRemoved, CaptureLost, Disabled, ProjectionUnavailable, WidgetRemoved, WidgetKindChanged, Explicit}`. `WidgetDisabled` cancels an active drag and blocks pointer changes. Same-panel/same-id/same-kind refresh preserves capture and live applied value; removal, kind change, disable, projection/capture loss, explicit cancel, or owner-panel teardown terminates it exactly once.
...
- **Semantic cancel:** `SemanticWidgetIntent::Cancel` terminates an active slider drag on its resolved target exactly once with the stored `PointerId` and `Explicit` cause. It is a no-op without an active capture; built-in Escape and application-written `WidgetInput::Cancel` use this same path.
...
- **Initial anatomy boundary:** this phase assigns and records the slider-root slot but introduces no public anatomy builder. It exercises the headless full-content-box projection path. Phase 9 adds the optional thumb slot and proves the same lifecycle uses thumb-center endpoints without changing capture behavior.
...
- `src/layout/builder.rs`, `src/layout/element.rs`, `src/layout/engine/sizing.rs`, `src/widgets/visual.rs`, `src/widgets/id.rs` — assign the slider-root slot and carry its solved border/content bounds through computed output and reify
...
**Constraints from prior phases:** Phase 8 supplies applied state and raw application-controlled proposals; `SliderState::set_value` is the sole snap-and-clamp boundary. Phase 8.25 supplies crate-private `WidgetCaptures::try_capture`, reverse owner lookup, attempted-press observations, and checked sequence exhaustion while preserving button terminals and `ButtonPress` timing. Its button-only raw reconciler still lives in `button.rs`; this phase moves that dispatcher into `capture.rs` and adds typed slider callbacks without moving button ids or terminal outcomes into the shared resource. Phase 8.5 supplies `CapturedCameraRay`, `FlatPanelRayError`, `project_flat_panel_ray_hit`, and `project_pointer_value`; the ray helper re-normalizes the camera's current separate `RenderTarget` against the current primary window and rejects target drift, while the value helper rejects non-finite or non-positive active-axis geometry and keeps output raw. Phase 6.5 supplies the final per-face hit stream: `Interactive` and `WidgetsOnly` may target a slider, while `PanelOnly` and `PassThrough` cannot. Phase 4.5 supplies combined world/screen attachment cleanup; Phase 5 supplies the post-interactivity fence and paired panel lifecycle paths. Deterministic pointer integration feeds synthetic `PointerHits` and raw `PointerInput` through Bevy's real dispatcher; it never moves the operating-system pointer or substitutes directly triggered target events.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with tests driven through Bevy's real pointer dispatcher and message-maintenance timing: a successfully projected press immediately emits grabbed then one raw non-final proposal; a press whose initial projection is unavailable claims nothing and emits no lifecycle or value event; click-without-drag emits the raw release-location final proposal; each drag updates the stored raw target; projection loss after capture cancels exactly once with `ProjectionUnavailable`; final proposal precedes `SliderReleased` after occupancy is freed; rejecting all non-final proposals leaves `SliderState` unchanged while the final proposal still carries the raw release position; opt-in acceptance calls `set_value` once per proposal. The slider-root slot exists even without a thumb, and its solved border/content bounds survive computed output and reify; pointer projection demonstrably excludes authored border and padding. Cover perspective and orthographic cameras; primary and secondary render targets; camera target drift; missing camera, `GlobalTransform`, or `RenderTarget`; non-finite/non-invertible panel transforms and invalid panel scale; `Interactive`/`WidgetsOnly` versus `PanelOnly`/`PassThrough`; release-over/release-away; zero or invalid active-axis content geometry; beyond-panel clamping; pointer loss; disable; kind change; component-only role removal; full panel despawn; Escape; same-id refresh; and exact-once terminal behavior. Raw-batch regressions cover button → slider and slider → button handoff, same-pointer reuse, a second pointer competing for the freed widget, stale hover, `PickingPlugin` without `InteractionPlugin`, rejected later presses, raw cancel followed by another action, and sequence exhaustion. Extend and smoke-test `examples/widgets.rs` through BRP keyboard/state inspection or user-performed pointer input without moving the operating-system pointer programmatically.

#### Retrospective

**What worked:**
- The shared raw dispatcher preserves the existing button sequence while adding slider capture, cross-widget handoff, and exact-once terminal events.
- Reified slider-root content bounds let live projection use solved layout geometry without writing `SliderState`; the application remains authoritative over every proposal.

**What deviated from the plan:**
- Raw-batch reconciliation needed explicit per-action observation counts and boolean terminal-helper results so a later rejected action cannot suppress an earlier accepted release.

**Surprises:**
- A typed release helper must report whether it actually terminalized a stored payload; assuming success can free shared occupancy without recording the matching terminal outcome.

**Implications for remaining phases:**
- Phase 9 can add only the thumb slot and presentation writer; it reuses the shipped root geometry, capture lifecycle, and raw dispatcher unchanged.
- Phase 9's presentation gate must observe `SliderDrag` insertion and removal so pressed styling returns to normal after every terminal path.
...
**Spec:**
- **Direct anatomy authoring:** add `El::slider_thumb()` as a marker on one ordinary descendant of the nearest `El::slider`. It creates no ECS child and exposes no anatomy component; computed output associates that element's private stable slot with the owning slider. Phase 8.75 already assigns crate-private `VisualSlotId::SLIDER_ROOT`; this phase adds the distinct crate-private fixed identity `VisualSlotId::SLIDER_THUMB`. Zero marked thumbs leaves the headless slider valid with no automatic value visualization.
- **Solved geometry source:** consume the solved border/content bounds already carried by `ComputedVisualSlot` through `ComputedWidgetRecord` → `WidgetVisualSlots`. Phase 8.75's slider root slot supplies its content box; this phase marks the thumb slot so its border box supplies both its active-axis extent and its solved authored center. Remove the temporary `#[cfg(test)]` gate from the existing crate-private `WidgetVisualSlots::border_box` reader and use that production reader for both extent and center. Pointer projection and presentation read these reified slot records rather than recomputing padding or querying retained render batches.
- **Stable validation contract:** an orphan thumb returns `PanelBuildError::SliderThumbOutsideSlider(PanelElementId)` with "slider thumb `{0}` must be inside a slider subtree"; a second thumb returns `PanelBuildError::SliderHasMultipleThumbs(PanelElementId)` with "slider `{0}` contains more than one thumb". State builders share one element-level error set across every widget kind: `StateBackgroundRequiresBackground` displays "widget `{0}` state background requires an authored background"; `StateBorderColorRequiresBorder` displays "widget `{0}` state border color requires an authored border"; `StateBorderWidthRequiresBorder` displays "widget `{0}` state border width requires an authored border"; and `StateMaterialRequiresSurface` displays "widget `{0}` state material requires an authored background or border". A slider's authored range, value, or step reports through `SliderConfig(PanelElementId, SliderConfigError)`. Each carries the widget's `PanelElementId`. Panel construction and `set_tree` return the same variants before any partial update is queued.
...
- **Value presentation:** solve the desired thumb center from the approved content-box/thumb-center endpoint table and the normalized applied value, then subtract the thumb slot's solved authored center on the active axis. Keep this desired-center delta in layout points — layout coordinates with Y increasing downward — so presentation shares the solved slot geometry with pointer projection, then convert it once to the retained override's panel-local render frame through the owning panel's `points_to_world()` scale and Y-axis convention (X scales; Y scales and inverts) before writing `VisualSlotOverride::offset`. The conversion lives at one private shared boundary, not duplicated across the four retained routes; a missing owning panel or a non-finite or non-positive scale clears the thumb offset rather than manufacturing one. This makes the range endpoints exact even when the ordinary authored tree did not initially place the thumb at the directed range start. When the thumb's active-axis extent `T` is greater than or equal to the content extent `C`, center it on the content box's active axis while leaving pointer projection unavailable; preserve its cross-axis position and authored depth. Respect all four directions, preserve authored hit geometry, and perform no `LayoutTree` or `ComputedDiegeticPanel` regeneration per value change.
- **Direct root state builders:** the same element-level state builders cover the slider root — optional hovered, pressed, focused, and disabled background, border-color, border-width, and `Handle<StandardMaterial>` replacements. They use the same authored-target validation, property composition, and root-surface boundary as `Button`; child thumb/label appearance remains application-authored and constant in widgets v1.
- **Presentation ordering and gate:** reuse Phase 7.5's order: focus/applied state → slider state/thumb writer → `WidgetSystems::PresentationCommandsApplied` → `dispatch_visual_overrides` → retained renderer routes. Add a private slider presentation run condition covering changed `SliderState`, authored `WidgetSpec`/slots, `PickingInteraction`, focus, disabled, and the private drag/press marker, including removal edges back to normal. The condition and writer filter to live `WidgetKind::Slider` entities; an unrelated button change does not wake the all-slider walk. A quiet frame never walks all sliders. The writer still immutable-compares before mutable override access so an unchanged requested override produces no change tick or upload.

**Files:**
- `src/widgets/slider.rs` — direct root-state builders and value/state presentation writer
- `src/layout/builder.rs`, `src/layout/element.rs`, `src/widgets/id.rs` — `El::slider_thumb`, thumb-slot association, and validation while consuming Phase 8.75's solved slot geometry
...
- `src/render/fill_batch.rs` — integrated retained-batch proof for scaled, Y-inverted thumb translation
...
**Constraints from prior phases:** Phase 8 supplies `SliderDirection` and the accepted applied-value contract. Phase 8.5 supplies `project_pointer_value`, its raw finite-result contract, invalid-geometry rejection, and the approved four-direction endpoint table. Phase 8.75 supplies pointer lifecycle, private pressed/drag state, `VisualSlotId::SLIDER_ROOT`, and solved border/content bounds through `ComputedVisualSlot` and `WidgetVisualSlots`; its headless path calls `project_pointer_value(..., None, ...)`. This phase feeds the same helper `Some(thumb_extent)` from the solved thumb slot and uses the identical endpoint calculation for presentation. Phase 7.25 supplies stable visual slots and batch re-keying while preserving authored depth. Phase 7.5 supplies the direct root-state convention, role-specific private color overrides, shared immutable-before-mutable writer, and presentation command fence. Slider reuses those paths and does not introduce a second presentation component, theme/preset abstraction, or material surface.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: zero thumbs leaves headless behavior intact; orphan, duplicate-thumb, and missing root-surface targets return the exact stable construction and `set_tree` errors above; the slider-root and thumb slots remain distinct and carry current solved border/content bounds through reify; one thumb tracks the accepted applied value in all four directions even when its ordinary authored position is not the directed range start and after an authored direction change; live pointer projection and thumb presentation read the same slot geometry and approved endpoint table, including padding/border exclusion, exact center alignment at both endpoints, outside clamping, active-axis centering for `T >= C` with pointer projection unavailable, and zero-extent headless behavior; oversized-thumb centering preserves cross-axis position, authored depth, and slider hit geometry in all four directions; direct hover/press/focus/disabled root values follow Phase 7.5 precedence and target rules; first override insertion reaches dispatch through `WidgetSystems::PresentationCommandsApplied`; a value or state change dirties/re-keys only the expected root/thumb records and causes no relayout; an integrated retained-batch routing proof drives the real widgets + SDF pipeline on a world-panel slider with a visible SDF thumb and a vertical direction, then proves an applied-value change moves exactly the thumb retained record by the panel-local render delta converted from the solved slots through the live `points_to_world()` (with Y inverted) while the root/peer record and the widget hit/layout geometry stay unchanged — failing if raw layout points are written or vertical Y is not inverted; a direct run-condition test proves one slider-authored/state insertion, change, or removal re-arms presentation once, an unrelated button change does not, and a quiet frame skips the all-slider walk; repeated value/state leaves override change ticks and retained uploads untouched; authored thumb depth, slider hit bounds, and child content remain unchanged. Extend and smoke-test the public path in `examples/widgets.rs` while preserving all existing diagnostic and input paths and expanding the measured diagnostic readout.

#### Retrospective

**What worked:**
- `WidgetVisualSlots` and `WidgetVisualOverrides` carried the authored thumb geometry and presentation change without recomputing layout.
- `DirectedThumbTravel` now supplies the same endpoint calculation to pointer projection and thumb presentation.

**What deviated from the plan:**
- The retained-render proof in `src/render/fill_batch.rs` became the authoritative test for panel scale and vertical-axis conversion.

**Surprises:**
- Widget-level tests could pass while a layout-space Y delta was written into the render-space override with the wrong sign; the integrated retained-record test was needed to expose that boundary error.

**Implications for remaining phases:**
- Any tooltip behavior that crosses layout, attachment, and final-render coordinate frames needs an integration test at the final consumer, not only component-level state assertions.
...
**Spec:**
- **Semantic relationship:** every tooltip is its own entity with public `Tooltip` and `TooltipFor(target)` components; public `Tooltips` is the reverse target membership and is declared with `linked_spawn`, so target despawn owns tooltip-controller cleanup without hierarchy parenting. The relationship may name any live entity. It expresses what the tooltip describes, not how to place it; Phase 10.5 materializes only when the general anchoring layer can supply compatible geometry and presentation context.
- **Associated authoring typestate:** extend `El` with the defaulted named role generic `El<L = Row, Role = LayoutOnly>`. `.button(...)` and `.slider(...)` consume `El<L, LayoutOnly>` and return `El<L, WidgetElement>`; `.tooltip(tooltip: Tooltip)` exists only for `El<L, WidgetElement>`. Common element builders and ordinary `LayoutBuilder::with`/`with_root` accept either role through a sealed role trait. The tooltip declaration remains parallel to `CommonEl::widget`, never inside `Button`, `Slider`, or crate-private `WidgetSpec`. Repeating `.tooltip(...)` replaces the one declaration in ordinary last-builder-wins fashion. Layout output creates a private `ComputedTooltipRecord` keyed by `(panel, widget id)`. There is no `TooltipRequiresWidget` validation variant or runtime error.
- **Computed storage and tree diff:** carry associated `ComputedTooltipRecord`s through `ComputedDiegeticPanel` beside computed widgets and populate them from both full layout and cached-geometry refreshes in `compute_panel_layouts`. A tooltip declaration add/remove/replacement is `VisualOnly`; an identical `Tooltip` clone remains `Identical`. Controller indexes still follow source revision when an identical `set_tree` advances the source tree without changing computed output.
- **Standalone authoring:** `TooltipCommandsExt::spawn_tooltip(target: impl TooltipTarget, tooltip: Tooltip) -> Entity` reserves and returns the controller id, matching ordinary Bevy spawn ergonomics, but keeps relationship mutation private and does not accept raw `Entity`. `TooltipTarget` exposes the target entity and a compile-time `World` or `Screen` associated space; built-in panel/widget handles, the general `TooltipTargetEntity<Space>`, and application-owned typed handles may implement it. When commands apply, the operation verifies that the target is live and still supplies placement data for its declared space. An already-missing target removes the reserved controller in the same command flush, before another system can observe it and without producing an invalid-relationship warning. If the target disappears later, `Tooltips` linked-spawn cleanup despawns the controller. A live target whose placement data is not ready may carry a lightweight hidden controller, but no tooltip panel is materialized until placement is possible.
- **Tooltip is its visual-layout authoring context:** `Tooltip::new(root: El<L, LayoutOnly>) -> Tooltip` creates the `Fit × Fit` tooltip with the caller's visible root. It mirrors the existing closure-based visual layout operations as `with(el: El<L, LayoutOnly>, children)`, `text(...)`, and `image(el: El<L, LayoutOnly>, handle, tint)`. Every inserted element therefore passes through the tooltip context. A `.button(...)` or `.slider(...)` result is `El<_, WidgetElement>` and cannot be used as the root or passed to `Tooltip::with`/`image`; no completed arbitrary `LayoutTree` constructor, silent widget stripping, tooltip-content validation error, or public `TooltipPanel` type exists. Ordinary panels continue to accept either element role. Internally `Tooltip` authors its concrete `LayoutTree` behind copy-on-write `Arc` storage; once cloned or attached, that immutable blueprint remains the deferred panel source.
- **Public tooltip policy builders:** `Tooltip::new(...)` also installs `show_after = Duration::from_millis(500)`, `hide_after = Duration::ZERO`, `TooltipDisabledPolicy::Suppress`, source `Anchor::TopCenter`, target `Anchor::BottomCenter`, `PanelAnchorOffset::new(Px(0.0), Px(8.0))`, and `TooltipPlacementPolicy::KeepVisible`. Consuming `.show_after(...)`, `.hide_after(...)`, `.disabled_policy(...)`, `.source_anchor(...)`, `.target_anchor(...)`, `.offset(...)`, and `.placement_policy(...)` builders override those defaults. There is no separate public tooltip-policy component or `.tooltip_with_policy(...)` path.
  ```rust
...
- **General placement data:** arbitrary world targets publish local anchor points through `hana_valence::ResolvedAnchorGeometry`; arbitrary screen targets publish a rectangle plus window, camera order, render layers, and pixel-unit context through the equivalent general screen-target data. Built-in panel/widget adapters remain automatic. These are general anchoring inputs rather than tooltip variants, so other attachment consumers can reuse them. A general target has no invented `DiegeticPanel` owner: `TooltipFor` linked-spawn cleanup follows target-entity despawn, while `PanelOwned` is recorded only when a real panel role exists.
- **Easy mesh-face target:** add one checked mesh-anchor command that accepts any live `Mesh3d` entity plus `MeshFace::{PositiveX, NegativeX, PositiveY, NegativeY, PositiveZ, NegativeZ}` and returns `TooltipTargetEntity<World>`. The selected face is persistent general anchor authoring, not tooltip state. While attachment demand exists, Hana reads the entity's current `Mesh3d` and local `Aabb` components and derives the usual nine rectangular `Anchor` points plus their face frame into `ResolvedAnchorGeometry`. Authors never construct those points by hand. Missing `Mesh3d` or `Aabb` leaves the target pending and hidden with the bounded warning behavior; removing either component removes the derived geometry, and restoring usable components recovers it. An actual `Aabb` component change or selected-face change refreshes the derived local geometry, while ordinary transform changes require no rebuild because Valence composes the local points with the current transform. When an application replaces or mutates a mesh, that application owns keeping the entity's `Aabb` correspondingly correct. Hana uses the `Aabb` currently present and does not track mesh asset revisions, asset membership, revision generations, or later bounds ticks. The public call is intentionally one step before `spawn_tooltip`, approximately `let cube = commands.mesh_anchor_target(cube, MeshFace::PositiveZ);` followed by `commands.spawn_tooltip(cube, tooltip);`; settle only mechanical extension-trait naming during implementation.
...
- `src/layout/builder.rs`, `src/layout/element.rs` — named `LayoutOnly`/`WidgetElement` roles, separate tooltip declaration, and computed record
- `src/panel/diegetic_panel.rs`, `src/panel/compute_layout.rs` — carry computed tooltip records through full and cached-geometry panel output
- `src/panel/anchoring.rs`, `src/panel/valence_provider.rs`, `src/screen_space/anchoring/` — typed general targets, the AABB mesh-face provider, screen target data, and built-in panel/widget adapters
...
- `src/lib.rs` — curated tooltip, policy, relationship, and command-extension exports
...
**What worked:**
- Named `LayoutOnly` / `WidgetElement` roles and `trybuild` fixtures keep button and slider declarations out of tooltip trees at compile time.
- Associated and checked standalone authoring both converge on stable lightweight controllers behind `TooltipSystems::ControllerCommandsApplied`.
- The live widgets example produced exactly three controllers—button, slider, and cube—with one reverse relationship each and no materialized panel or tooltip-created anchor demand.

**What deviated from the plan:**
- `Tooltip` initially shared `Arc<LayoutBuilder>`; the shipped value shares copy-on-write `Arc<LayoutTree>` and keeps its authoring cursor outside the blueprint.
- Mesh-asset revision and bounds-tick tracking was removed. Hana consumes the current `Mesh3d` / `Aabb` components, while the application owns keeping bounds correct after mesh changes.

**Surprises:**
- A clone captured inside `Tooltip::with` needs to preserve automatic-id state but resume authoring at the root rather than retain the borrowed nesting cursor.
- General targets need only `TooltipFor`; only real panel and widget targets acquire `PanelOwned` cleanup ownership.
...
**Goal:** A private preparation request materializes the existing tooltip controller once as a hidden world- or screen-space panel, attaches and positions it, and reports readiness without running visibility timers or events.

**Settled keep-visible placement policy**

Actual problem:
The owner requires a policy for tooltips that do not fit inside their window. Screen tooltips have an explicit target window, while a world tooltip must be projected through the camera being used to interact with its panel before any screen-edge test is meaningful.

What is settled:
- Anchors and offset remain the authored preferred placement on `Tooltip`; collision handling does not overwrite them.
- `TooltipPlacementPolicy::KeepVisible` is the default. `Fixed` always uses the authored anchors and offset even when the result clips.
- `KeepVisible` first tests the authored side with the smallest along-edge shift that could keep the tooltip visible. It never shifts beyond the point where the tooltip ceases to overlap the target anchor in that axis. If the authored side still fails, test the opposite side, then the remaining side with more available room, then the final side. Equal remaining room uses a fixed implementation-documented tie break so placement is deterministic. Each candidate receives the same limited along-edge shift before it is rejected.
- Screen placement evaluates the target panel's window.
- World placement evaluates the current interaction camera. Pointer hits supply that camera. The existing per-window focus authority should retain it beside the active panel so later keyboard traversal reuses it without any per-tooltip camera configuration. Initial keyboard-only focus selects the highest-order active camera in that window that can render the active panel.
- The remembered camera governs placement only. V1 does not promise that other cameras sharing the world panel's render layers cannot also render the tooltip.
- When the natural `Fit × Fit` result is wider than the usable viewport, `KeepVisible` constrains only the tooltip panel's outer width to the usable width and reruns layout so wrappable content can reflow. It does not rewrite explicit sizes inside the application-authored tree. Placement uses the reflowed result. If that result still exceeds the viewport in either axis or no candidate can contain it, the controller remains hidden; v1 adds neither implicit clipping nor scrolling. `Fixed` retains the natural unconstrained size.
- `KeepVisible` keeps a fixed eight-logical-pixel margin inside every viewport edge after DPI conversion. V1 exposes no per-tooltip margin builder; `Fixed` ignores this margin. A later application-wide or per-window safe-area policy can replace the constant when actual use requires it.

**Settled pointer and semantic-interactivity policy**

Actual problem:
The owner requires a tooltip to remain an informational surface: it must never take pointer hover from its target, block a hit behind it, or contain a keyboard-focusable control.
...
- **Private request and readiness boundary:** Phase 10's controller has no panel or anchor demand. A private preparation request inserts a hidden `DiegeticPanel` on that same entity exactly once. `TooltipSystems::MaterializationCommandsApplied` makes the panel and synchronized `PanelSpace` queryable. A following system obtains current typed target handles and queues the checked same-space attachment; `TooltipSystems::AttachmentCommandsApplied` applies it before existing screen-demand and coordinate-specific placement fences. A private ready state is written only after current layout, attachment, placement, and final transform propagation all succeed. `Pending` means the hidden panel is not yet safe to reveal because layout, target geometry, presentation context, reflow, or attachment resolution is unavailable; it is not an extra ordinary-motion frame. Tests in this phase issue the private request directly; Phase 10.75 later issues it in response to hover or visible keyboard focus. A current diagnostic or unavailable provider leaves the panel hidden and not ready rather than placing it at a fallback transform.
- **Immutable blueprint handoff:** materialization clones the current `Arc<LayoutTree>` content into the owned panel tree without mutating or replacing the `Tooltip` blueprint pointer. Fit sizing and keep-visible reflow affect only materialized panel state; they never rewrite the shared authored blueprint.
- **Coordinate-space inheritance:** resolve a widget target through `WidgetOf`, read a panel target directly, or consume the settled general anchor-provider context. A panel/widget world target supplies its layout unit; a general world target reads the current global `PanelDefaults::layout_unit` when its tooltip panel is first materialized, exactly as other panel construction does, with no tooltip-specific hard-coded fallback. A panel/widget screen target supplies its window, camera order, render layers, and pixel layout unit. `ScreenAnchorTarget` supplies those same screen facts for a general target. Add it to `CandidateQueries` and the per-frame rectangle map, reject a target whose declared window is not live, and resolve its published bounds through the ordinary screen attachment path. World tooltips lower to `hana_valence::AnchoredTo`; screen tooltips retain private panel authoring and use the generalized screen target adapter. The materialized placement target must always match `TooltipFor`.
- **Presentation-camera input:** world `KeepVisible` readiness consumes a private compatible presentation-camera input on the controller. This phase's tests seed that input directly; a missing or incompatible camera leaves the tooltip hidden and not ready. Phase 10.75 selects or refreshes the input before it requests preparation. Screen placement continues to use the target's inherited window and camera order.
- **Lockstep dual-space placement:** layout is ready only after the materialized controller has a current `ComputedDiegeticPanel`. Screen placement first resolves the current target, chooses keep-visible placement, applies an attachment change when the chosen side differs, and reruns the screen attachment resolver only for that correction before `PanelSystems::PositionScreenSpace`. World placement refreshes current root target and camera globals from their local transform hierarchy after anchor animation, chooses placement, applies any attachment change before `AnchorSystems::Resolve`, and then uses ordinary transform propagation. A solvable target move therefore updates the tooltip and finishes `Ready` in the same frame, including a keep-visible side change. Exact-equality writes keep settled panel transforms and anchor sources quiet rather than manufacturing change ticks every frame.
- **Dual-space readiness:** screen placement is ready after `PanelSystems::PositionScreenSpace` when `ResolvedScreenPanelPosition::anchor_position` is `Some`; world placement is ready after `AnchorSystems::Resolve` when the controller retains its Valence attachment and no current-frame `ResolveDiagnostics` entry names it as skipped. The private ready state is finalized after `TransformSystems::Propagate`, so its `GlobalTransform` is current. Removing an attachment, target geometry, transform, or presentation input re-evaluates readiness in that frame; restoring valid input recovers it. Phase 10.75 may reveal only a ready controller.
- **Persistent hidden state and cleanup:** materialization starts world or screen anchor demand; later hiding retains the same panel, layout, and attachment without inactivity eviction. A mesh-face target derives geometry from the current application-maintained `Mesh3d` and `Aabb` only after that demand begins; this phase adds no asset-event, asset-revision, bounds-generation, or delayed-bounds machinery. Normal renderer routes may retire hidden GPU batch rows and rebuild them on show; that is not tooltip or panel respawn. Materialization snapshots any `Transform`, `GlobalTransform`, or `Visibility` already present on the controller. Hidden controller, target, component-only panel-role, and full panel despawn paths detach and clean the materialized state through existing lifecycle ordering, restoring those preexisting values instead of deleting application state, and emit no visibility events; Phase 10.75 extends those paths only for a tooltip that was actually visible.

**Files:**
- `src/widgets/tooltip.rs` — private preparation request/readiness, first materialization, and hidden cleanup
- `src/widgets/mod.rs` — materialization and attachment deferred fences
- `src/cascade/defaults.rs` — read the existing global panel layout-unit construction default; do not add a tooltip-specific default
- `src/panel/lifecycle.rs`, `src/panel/mod.rs` — hidden materialized-state cleanup through both panel-role teardown paths
...
- `src/screen_space/mod.rs`, `src/screen_space/anchoring/candidate.rs`, `src/screen_space/anchoring/rect.rs` — general-screen candidate query, window validation, and rectangle-map publication
...
- `src/render/fill_batch.rs` — prove the hidden panel's final world and screen placement inputs before reveal
- `crates/hana_valence/src/resolve.rs` — equality-gated anchor-source transform writes

**Constraints from prior phases:** Phase 10 supplies controllers containing `Tooltip` and `TooltipFor` plus optional `PanelOwned`, stable controller identity, an immutable copy-on-write `Arc<LayoutTree>` blueprint, placement builders/defaults, the checked standalone lifecycle, `ScreenAnchorTarget`, current-`Aabb` mesh geometry, general target providers, and `TooltipSystems::ControllerCommandsApplied`. Materialization must retain that blueprint pointer while the owned panel uses a content clone. Phase 3 supplies the owner-panel teardown contract; Phases 4 and 4.5 supply world/screen widget geometry, typed same-space placement, demand, and `finalize_widget_anchor_state`. Panel/widget branches mint current handles through `PanelEntityReader`/`PanelWidgetReader`; a general target uses Phase 10's provider path. A materialized tooltip is an active attachment even while hidden, so converting its source or target panel is rejected until it detaches; pre-materialized `TooltipFor` membership alone does not block conversion. Phase 9 showed that component-level state does not prove final coordinate conversion, so readiness tests inspect the propagated world/screen result. The application owns keeping a mesh target's `Aabb` synchronized with mesh changes; later phases consume the current component and do not recreate asset-version tracking.

**Acceptance gate:** `cargo nextest run -p hana_diegetic --lib` green with new tests: a private request materializes the same controller as a hidden `Fit × Fit` panel through panel fence → target acquisition → attachment fence; no request leaves the controller lightweight and creates no anchor demand; repeated requests reuse one panel and attachment; materialization retains the original tooltip blueprint `Arc` while the panel owns a content clone; world and screen panel/widget targets plus one mesh-face general target become ready only after current layout, placement, and propagated transform; a panel/widget target inherits its panel layout unit, while a general world target reads a non-default test value from the global `PanelDefaults::layout_unit` rather than hard-coding `Unit::Meters`; a missing provider, missing or incompatible presentation camera, or current resolver diagnostic remains hidden and not ready; removal and restoration of attachment or geometry invalidates and recovers readiness; a general screen target enters `CandidateQueries` and the rectangle map, rejects a missing declared window, and transfers bounds, window, camera order, render layers, and layout unit; a secondary-window target transfers the same presentation context; a world widget honors nonzero offset; mesh demand begins only with preparation and consumes the application's current `Mesh3d`/`Aabb` without asset-revision tracking; the settled keep-visible policy proves preferred/opposite/remaining-side order, bounded along-edge shifting, eight-logical-pixel margin, reflow-then-hide, off-viewport hiding, and interaction-camera projection; screen and world target moves, including a world keep-visible side change, update placement and remain ready in the same frame; a settled screen panel and anchor source keep quiet transform change ticks; the settled Hana-owned tooltip `PanelPicking` behavior is installed during materialization; hidden teardown paths detach and clean exactly once without visibility events and restore any controller transform/visibility state that existed before materialization. No test in this phase drives the operating-system pointer or implements show/hide timing.

#### Retrospective

**What worked:**
- The hidden controller, panel, checked attachment, placement result, and readiness state form one persistent materialization path for both coordinate spaces.
- Target-motion tests at final propagated geometry made same-frame behavior explicit instead of accepting a component update followed by a visible one-frame catch-up.

**What deviated from the plan:**
- Lockstep world placement required a narrow pre-resolve refresh of root anchor-provider and camera globals; the Valence resolver also changed to avoid rewriting an equal source transform.
- Materialized cleanup now snapshots and restores controller transform and visibility requirements because panel insertion may satisfy them on an application-created controller.

**Surprises:**
- The original screen positioner assigned the same numeric transform every frame, which still marked it changed and kept waking tooltip placement.
- A slider test camera supplied a seeded `GlobalTransform` but retained Camera's required default local `Transform`; same-frame global refresh exposed that inconsistent fixture, so the test now seeds both from the same pose.
...
**Goal:** Hover or visible keyboard focus requests preparation, runs the authored delays, reveals only a ready tooltip, emits visibility events, and thereafter hides/shows the same materialized panel without respawning it.

**Spec:**
- **Eligibility and ordering:** read target `PickingInteraction` plus private `WidgetFocusVisible` where applicable. Pointer focus alone does not keep a tooltip eligible after hover ends. Run after Phase 7.5's `WidgetSystems::FocusCommandsApplied` and Phase 10's `TooltipSystems::ControllerCommandsApplied`, so current interactivity, focus-indicator commands, and controllers are visible. Select or refresh the presentation camera, apply that private input, and only then issue Phase 10.5's preparation request; materialization and keep-visible placement must not run against the previous camera. `TooltipDisabledPolicy::Suppress` prevents or immediately ends visibility; `Show` leaves hover-or-visible-focus eligibility unchanged.
- **Interaction camera:** preserve the hit camera when pointer interaction establishes a window's active panel; keyboard traversal and semantic activation reuse it. Another-camera interaction replaces it. Initial keyboard-only focus selects the highest-order compatible active camera for the focused window/panel, and camera loss reselects by the same rule. This state populates Phase 10.5's private world `KeepVisible` presentation-camera input before preparation and adds no public camera field.
- **One private state machine:** `TooltipPhase::{Hidden, WaitingToShow(Timer), Visible, WaitingToHide(Timer)}` is authoritative. Entering the first show wait also issues Phase 10.5's private preparation request. `show_after` starts when eligibility becomes true and cancels on loss. `hide_after` starts when eligibility becomes false, cancels if eligibility returns, and hides on expiry; zero is immediate. A finished show timer still waits for Phase 10.5 readiness. `Visibility` is derived from this phase; no second visible marker or simultaneous timer components exist, and only waiting entities tick.
- **Two-stage reveal and events:** once the show delay and preparation readiness are both satisfied, a pre-batch system writes visible `Visibility` before `MaterialTableAppendReady`, allowing every retained route to create current records that frame. After `TransformSystems::Propagate`, emit public non-propagating `TooltipShown { entity }`, so observers read the final `GlobalTransform`. A visible→hidden edge writes hidden state and emits `TooltipHidden { entity }` exactly once before cleanup; canceled waits and redundant states emit nothing.
- **Persistent materialization:** hiding retains Phase 10.5's panel, layout, attachment, placement, and ready state. Later eligibility shows that same entity without another preparation request or panel spawn. Hidden GPU rows may retire and rebuild through normal renderer behavior; that is not tooltip or panel respawn.
- **Visible lifecycle finalization:** extend controller despawn, target-entity despawn, component-only target-panel role removal, and full target-panel despawn so a visible tooltip emits `TooltipHidden` before linked-spawn, ownership, or placement cleanup. The event observes `Tooltip`, its final transform, semantic target, and placement relation. One duplicate-safe finalizer gives every path at most one event; cleanup of a never-visible or already-hidden tooltip emits none.
- **Defaults:** `show_after` is 500 ms, `hide_after` is zero, disabled policy is `Suppress`, placement is `TopCenter → BottomCenter` with an eight-pixel target gap, and placement policy is `KeepVisible`. Rich content remains the ordinary application-authored tree.

**Files:**
- `src/widgets/tooltip.rs` — eligibility, timers, visibility state machine, events, and visible finalization
- `src/widgets/focus.rs` — retain and reuse the active panel's private interaction camera per window
- `src/widgets/mod.rs` — behavior ordering after focus and materialization readiness
- `src/panel/lifecycle.rs`, `src/panel/mod.rs` — visibility finalization before existing teardown
- `src/render/mod.rs`, `src/render/material_table.rs`, `src/render/fill_batch.rs` — pre-batch reveal and final retained-transform proof
- `src/lib.rs` — curated tooltip visibility-event exports
- `crates/hana_diegetic/examples/widgets.rs` — canonical timing, visibility-event, mesh-target, and retained-panel exercise
...
- **Decision:** Public authoring uses `Button` and `Slider`. An element declares one inline with `.button(id)` or `.slider(id, range)`, or adopts a pre-built declaration with `.widget(id, widget)`. The `Spec` suffix remains only in crate-private `WidgetSpec`; runtime slider data remains unambiguously `SliderState`.
...
### PD2 — Tooltip relationship and retained panel blueprint

- **Severity:** important
- **Source dimension:** architecture/ergonomics + type-system/changeability consensus
- **Class:** design-improvement
- **Relationship decision:** tooltips are separate lightweight entities with `TooltipFor(target)` / `Tooltips`, never fields inside `Button`, `Slider`, or `WidgetSpec`. Associated declarations reify a controller related to the widget; standalone tooltips use the same relation through a checked command. The semantic target may be any entity for which the general anchoring layer can supply placement context. First eligibility materializes the controller into an anchored panel. The linked-spawn relationship target owns entity-despawn cleanup; a private `PanelOwned` record mirrors an actual target panel role when one exists so removing only `DiegeticPanel` provides the same controller lifetime boundary.
...
- **Placement decision:** `Tooltip` stores source anchor, target anchor, offset, and `TooltipPlacementPolicy`. Authored defaults are `TopCenter → BottomCenter` with an eight-pixel downward gap and `KeepVisible`; `Fixed` preserves that literal placement even when clipped. `KeepVisible` tries the authored side with limited along-edge shift, then the opposite side, then the remaining side with more room, then the final side. Shifting stops before the tooltip ceases to overlap the target anchor. Screen edge handling uses the target window; world edge handling uses the interaction camera remembered automatically with the window's active panel. Pointer interaction supplies that camera and keyboard interaction reuses it. The camera governs placement only, not exclusive rendering.
- **Placement status:** accepted — 2026-07-23
- **Content-boundary decision:** core accepts arbitrary visual layout authored through `Tooltip` and has no Hana theme, command, or keymap knowledge. Hana may own a semantic title/shortcut/body helper and manager that resolves its current keymap/theme and authors an ordinary `Tooltip` through that context.
- **Content-boundary status:** revised and accepted — 2026-07-24
- **General-target decision:** standalone attachment accepts `TooltipTarget` typed handles rather than raw `Entity`. Panel and widget handles implement it automatically; applications may implement it for their own invariant-preserving handles. A checked one-call mesh-face adapter turns any bounded `Mesh3d` entity plus one explicit local `MeshFace` into a typed world target. Bevy supplies the local `Aabb`; Hana derives the face's nine anchors lazily and keeps them current without requiring manual `ResolvedAnchorGeometry` construction. General targets have no invented panel owner and remain hidden with a bounded diagnostic while required live data is unavailable.
- **General-target status:** accepted — 2026-07-23
- **Implementation-split decision:** Phase 10.5 owns a private preparation request, hidden panel construction, checked world/screen attachment, placement readiness, persistent hidden state, and hidden cleanup. Phase 10.75 owns hover/focus eligibility, interaction-camera capture, show/hide timers, pre-batch reveal, visibility events, and visible finalization. Both operate on the same controller and materialized panel; the split adds no public type or observable respawn.
- **Implementation-split status:** accepted — 2026-07-23
- **Interaction decision:** every materialized tooltip installs Hana-owned `PanelPicking::PASS_THROUGH` on both faces with no v1 override. It cannot take hover from its target or block lower hits. `Tooltip` accepts only `LayoutOnly` elements, so no button or slider can reify, receive keyboard focus, activate, or capture a pointer inside it; interactive floating content is deferred to a popover-style design.
- **Interaction status:** accepted — 2026-07-24
...
- **DP1 — Remove the button authoring value entirely · dropped.** `Button` remains the home for retained button behavior such as `.on_click(...)`; `.button(id)` constructs one inline and `El::on_click` forwards to it, while `.widget(id, button)` adopts one built away from the layout chain.
