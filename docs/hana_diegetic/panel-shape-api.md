...
`hana_diegetic`, the decisions that survived implementation, and the direction
...
`crates/hana_diegetic/examples/units.rs` now uses `PanelDraw::lines` for ruler
...
```text
crates/hana_diegetic/src/render/
  batch_key.rs
  analytic_paths/
    mod.rs
    atlas.rs
...
- Phase E superseded by `docs/hana_diegetic/callouts.md` (user-approved
...
Planar callout unification is now planned in `docs/hana_diegetic/callouts.md`,
...
See `docs/hana_diegetic/callouts.md` for the implementation outline and the
...
`docs/hana_diegetic/callouts.md`. Phase F reconciles this section.
