...
| Phase | What | Status |
|-------|------|--------|
| 1 | Placement - group fit geometry, camera-pose helpers, and overlay internals | Done — see `docs/hana_lagrange/as-built/free-cam.md` |
| 2 | Split `fit/triggers/look.rs` | Done — see `docs/hana_lagrange/as-built/free-cam.md` |
| 3 | Split `fit/geometry/solve.rs` | Done — see `docs/hana_lagrange/as-built/free-cam.md` |
| 4 | Split `fit/overlay/render/fit_target_bounds.rs` | Done — see `docs/hana_lagrange/as-built/free-cam.md` |

## Phase 1 - Placement

**Status:** implemented — see `docs/hana_lagrange/as-built/free-cam.md` (fit/ restructure, "How it works").
...
7. Run `cargo check -p hana_lagrange --all-targets`,
   `cargo check -p hana_lagrange --all-targets --features fit_overlay`, and
   `cargo nextest run -p hana_lagrange`.
...
5. Preserve the current `fit/triggers/mod.rs`, `fit/mod.rs`, and `lib.rs`
   re-export surface.
6. Run `cargo check -p hana_lagrange --all-targets`,
   `cargo check -p hana_lagrange --all-targets --features fit_overlay`, and
   `cargo nextest run -p hana_lagrange`.
