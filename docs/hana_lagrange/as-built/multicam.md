...
Source: `crates/hana_lagrange/examples/viewports_windows.rs`
...
- `RenderTarget::Window(WindowRef::Entity(..))` aims a third camera at a
  second OS window spawned via `hana_clerestory::ManagedWindow`.
...
(`crates/hana_lagrange/src/input/modes.rs`):
...
helpers (`crates/hana_lagrange/src/orbit_cam/presets.rs`) return an
...
(`crates/hana_lagrange/src/input/routing/mod.rs`, re-exported from
...
`AnimateToFit` (`crates/hana_lagrange/src/fit/triggers/animate.rs`) is triggered as an
