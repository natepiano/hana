# `hana_lagrange` input architecture

As-built overview of the `hana_lagrange` camera-input subsystem in
`crates/hana_lagrange/src/input/`. It describes the shipped types, data flow, and
...
Crate-root paths (`hana_lagrange::OrbitDelta`, etc.) resolve unchanged.
...
The public API is grouped under `hana_lagrange::input` and re-exported from the crate
...
`bevy_enhanced_input` and `bitflags` are direct `hana_lagrange` dependencies declared
