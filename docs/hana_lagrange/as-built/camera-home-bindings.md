...
`hana_lagrange` presets no longer bind a home/reset input by default. A freshly
...
a third call replaces the second (`crates/hana_lagrange/src/free_cam/input/bindings/preset.rs`,
`crates/hana_lagrange/src/orbit_cam/input/bindings/preset/*`):
...
(`crates/hana_lagrange/src/camera_home.rs`):
...
(`crates/hana_lagrange/src/input/events.rs`, re-exported at the crate root):
...
(`crates/hana_lagrange/src/input/events.rs`):
...
- `log_camera_homed` (`crates/hana_lagrange/examples/showcase/event_log.rs`) — logs
  the showcase entry.
...
(`crates/hana_lagrange/src/input/bindings/source_binding.rs`) because BEI 0.26
...
- **Control-summary home row** (`crates/hana_lagrange/src/input/control_summary.rs`):
