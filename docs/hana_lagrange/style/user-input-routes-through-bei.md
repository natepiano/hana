...
Any input a caller can bind or configure must be implemented as a bevy_enhanced_input action, not a direct `ButtonInput<KeyCode>` read. hana_lagrange's rebindability contract is built on BEI; bypassing it breaks context gating, modifier normalization, and `CameraInputInternalSet` scheduling — and silently fails for OS-intercepted keys like CapsLock on macOS.
