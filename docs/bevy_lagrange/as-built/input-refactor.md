...
> **Archived 2026-06-07 — completed.** Removed in 3495b50 ("remove completed
> input refactor plan"); revived as the asbuilt record of `bevy_lagrange`'s
> input architecture. The target structure below is the current `src/input/`
> module: `actions`, `adapter/`, `bindings/` (presets included), `context`,
> `intent`, `lifecycle`, `manual`, `metrics`, `modes`, `routing/`, `sources`,
> with `OrbitCamInput` / `OrbitCamBindings` / the input-mode components as the
> public surface. The transitional `legacy.rs` named in the phase plan was
> deleted when the refactor finished. Phase logs and review responses are
> retained in full; they record how each decision landed. The preset API was
> later extended — see [`../preset.md`](../preset.md).
...
the pinned enhanced-input API surface.
...
`Reflect` when their data can be represented one-to-one; non-generic reflected types rely
...
reflected one-to-one, keep it out of public component/resource state until it has a
...
| Rule | Example | Fix |
|------|---------|-----|
| Add bindings by behavior. | A custom builder tries to call `.held_mouse_orbit(...)` or `.zoom_keys(...)`. | Use `.orbit(...)`, `.pan(...)`, and `.zoom(...)`; let the binding value carry the device variant. |
...
without making private adapter actions public. The hooks should stay Lagrange-specific:
...
- Phase 07 now converts `OrbitCamInput` to store per-kind source sets before lifecycle
...
- Move private enhanced-input installation diagnostics here: context entity
  ...
  surface, and adapter entity visibility are meaningful only after real action/context
  entities exist.
...
- Convert `OrbitCamInput` to store per-kind source sets for orbit, pan, and zoom
...
- Phase 10 now targets the implemented animation-interrupt ordering:
...
- Test animation interruption through the implemented system ordering:
...
- The custom binding API now mirrors the pattern users already know from BEI:
...
- Add behavior-first `OrbitCamBindings` builder methods:
  `.orbit(...)`, `.pan(...)`, and `.zoom(...)`. Binding values carry the mouse,
  trackpad, wheel, pinch, button-drag, keyboard, or gamepad variant.
