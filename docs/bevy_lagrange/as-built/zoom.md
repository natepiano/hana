...
> **Archived 2026-06-07 — implemented.** The type model is the current code:
> `ZoomDirection { In, Out }`, `OrbitCamControlRow.zoom_direction`, and
> `with_zoom_direction` live in `src/input/control_summary.rs`, with the live
> direction surfaced to the fairy_dust camera control panel
> (`camera_control_panel/snapshot.rs`) for per-row highlighting.
