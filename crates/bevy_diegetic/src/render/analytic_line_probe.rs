...
impl Plugin for AnalyticLineProbePlugin {
    fn build(&self, app: &mut App) { app.add_systems(Update, build_analytic_lines); }
}

/// Rebuilds the analytic batch for every authored line whose data or placement
/// changed. Replacing the render components drops the previous handles, so the
/// old mesh/material/buffers are released; the per-edit asset churn is fine for
/// a probe.
fn build_analytic_lines(
    lines: Query<
        (Entity, &AnalyticLine, Option<&Transform>),
...
fn build_line(
    ...
    let linear = line.color.to_linear();
    let run = RunRecord {
