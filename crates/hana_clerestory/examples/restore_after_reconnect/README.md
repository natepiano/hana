...
- `hana_clerestory::monitor_probe` becomes `monitor-topology`.
- `hana_clerestory::recovery_probe` becomes `recovery-accepted`.
...
```sh
cargo check -p hana_clerestory --all-targets --all-features
cargo nextest run -p hana_clerestory --all-features
