...
pub(super) fn install(app: &mut App) {
    if app.world().contains_resource::<DiegeticDiagnosticsRegistered>() {
        return;
    }
...
fn publish_perf_diagnostics(perf: Res<DiegeticPerfStats>, mut diagnostics: Diagnostics) {
    ...
    diagnostics.add_measurement(&DIAG_LAYOUT_COMPUTE_PANELS, || perf.last_compute_panels as f64);
    diagnostics.add_measurement(&DIAG_TEXT_EXTRACT_MS, || f64::from(perf.last_text_extract_ms));
    ...
    diagnostics.add_measurement(&DIAG_ATLAS_PAGES_ADDED, || perf.last_atlas_pages_added as f64);
    diagnostics.add_measurement(&DIAG_ATLAS_DIRTY_PAGES, || perf.last_atlas_dirty_pages as f64);
    diagnostics.add_measurement(&DIAG_ATLAS_IN_FLIGHT_GLYPHS, || {
        perf.last_atlas_in_flight_glyphs as f64
    });
    diagnostics.add_measurement(&DIAG_ATLAS_ACTIVE_JOBS, || perf.last_atlas_active_jobs as f64);
    ...
    diagnostics.add_measurement(&DIAG_ATLAS_TOTAL_GLYPHS, || perf.last_atlas_total_glyphs as f64);
}
